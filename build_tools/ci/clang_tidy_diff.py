...
def ansiprint(text: str, color: str = "", bold: bool = False) -> str:
...
def print_diagnostic(
  ...
  diag_line = " ".join((
      ...
      ansiprint(diag.message, bold=True),
      ansiprint(f"[{diag.name}]", color=CYAN, bold=True),
  ))
  ...
  for linenum, line in enumerate(lines[start:end], start=start + 1):
    ...
    if linenum == diag.line_num:
      stream.write(f"{ansiprint(prefix, bold=True)}{line_content}\n")
      # Print caret
      spaces = "".join(
          "\t" if ch == "\t" else " " for ch in line_content[: diag.col_num - 1]
      )
      stream.write(
          f"{' ' * len(prefix)}{spaces}{ansiprint('^', color=GREEN, bold=True)}\n"
      )
    else:
...
class ClangTidyDiffFilter:
  ...
  def process_file(self, yaml_file: str) -> list[Diagnostic]:
    ...
    for touched_file in self.diff_ranges:
      if touched_file in yaml_file:
        self.seen_files.add(touched_file)
