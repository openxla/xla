...
class BantWorkspaceTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.output_base = Path(temporary.name) / "bazel cache"
  ...
  def test_uses_current_checkout_without_changing_cached_repositories(self):
    ...
    with run_dwyu.bant_workspace(self.root, self.output_base) as workspace:
      ...
      self.assertEqual((external / "tsl").resolve(),
                       self.root / "third_party/tsl")
      self.assertEqual((external / "dependency+").resolve(),
  ...
  def test_ignores_stale_bazel_out_symlink(self):
    ...
    self.assertEqual((self.root / "bazel-out").readlink(),
...
@unittest.skipUnless(_BANT,
                     "requires the BANT binary installed by the workflow")
...
''')
    self.write_file("xla/consumer/consumer.cc",
...
''')
    ...
    self.assertEqual(
        result.stdout,
        "buildozer 'add deps @xla//xla/platform:errors' @tsl//tsl/profiler:controller\n",
    )
...
''')
    ...
    self.assertEqual(
        result.stdout,
        "buildozer 'remove deps @xla//xla/platform:unused' @tsl//tsl/profiler:controller\n",
    )
...
''')
    ...
    self.assertEqual(
        result.stdout,
        "buildozer 'remove deps @xla//xla/platform:unused' @tsl//tsl/profiler:api\n",
    )
