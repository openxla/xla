...
def bant_workspace(root: Path, output_base: Path) -> Iterator[Path]:
  ...
  with tempfile.TemporaryDirectory(prefix="xla-dwyu-") as temporary:
    ...
    (repositories / "tsl").symlink_to(root / "third_party/tsl",
...
def run_dwyu(
    targets: Sequence[str], *, output_base: Path, bant: str = "bant"
) -> int:
  ...
  with bant_workspace(Path.cwd(), output_base) as workspace:
