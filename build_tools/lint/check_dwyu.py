...
def extract_targets(build_content: str,
                    allowed_rules: set[str]) -> list[tuple[str, set[str]]]:
...
def _group_changed_files_by_package(changed_files: list[str],
                                    packages: set[str]) -> dict[str, set[str]]:
...
def main(argv: Sequence[str]):
  ...
  parser.add_argument(
      "--base_ref",
      default="origin/main",
      help=("Git ref whose merge base with HEAD to diff against"
            " (default: origin/main)"),
  )
