...
""",
  ...
  def test_only_direct_source_owner(self):
    self.assertEqual(self.affected_targets(["pkg/foo.cc"]), ["//pkg:foo"])

  def test_header_does_not_select_dependents(self):
    self.assertEqual(self.affected_targets(["pkg/foo.h"]), ["//pkg:foo"])

  def test_build_only_change_selects_nothing(self):
    self.assertEqual(self.affected_targets(["pkg/BUILD"]), [])

  def test_build_and_source_change_selects_only_source_owner(self):
    self.assertEqual(self.affected_targets(["pkg/BUILD", "pkg/foo.cc"]),
                     ["//pkg:foo"])

  def test_unowned_file_selects_nothing(self):
    self.assertEqual(self.affected_targets(["pkg/README.md"]), [])

  def test_subdirectory_does_not_match_same_basename(self):
    self.assertEqual(self.affected_targets(["pkg/subdir/foo.cc"]), [])
...
cc_library(name = "full_label", srcs = ["//pkg:subdir/foo.cc"])
cc_library(name = "other", srcs = ["elsewhere/foo.cc"])
""",
    )
    self.assertEqual(
        self.affected_targets(["pkg/subdir/foo.cc"]),
        ["//pkg:relative", "//pkg:local_label", "//pkg:full_label"],
    )
...
""",
    ...
    self.assertEqual(self.affected_targets(["pkg/foo.cc"]), ["//pkg:bazel_foo"])
    self.assertEqual(self.affected_targets(["pkg/BUILD.bazel"]), [])
...
""",
    ...
    self.assertEqual(
        self.affected_targets(["pkg/nested/foo.cc", "pkg/bar.cc"]),
        ["//pkg:bar", "//pkg/nested:nested_foo"],
    )
...
""",
    ...
    self.assertEqual(
        ...
        ["//pkg:foo", "@tsl//:foo", "//third_party/tsl_extra:foo"],
    )
...
""",
    ...
    self.write_file("pkg/foo.cc", "// Original foo\n")
    self.write_file("pkg/bar.cc", "// Original bar\n")
    self.commit()

  def test_base_branch_changes_are_excluded(self):
    self.git("branch", "topic")
    self.write_file("pkg/bar.cc", "// Unrelated upstream change\n")
    self.commit()
    self.git("checkout", "-q", "topic")
    self.write_file("pkg/foo.cc", "// PR change\n")
    self.commit()
    self.assertEqual(check_dwyu.get_changed_files("main"), ["pkg/foo.cc"])
    output = io.StringIO()
    with contextlib.redirect_stdout(output):
      check_dwyu.main(["check_dwyu.py", "--base_ref", "main"])
    self.assertEqual(output.getvalue(), "//pkg:foo\n")

  def test_push_compares_with_previous_commit(self):
    self.write_file("pkg/foo.cc", "// Pushed change\n")
    self.commit()
    self.assertEqual(check_dwyu.get_changed_files("HEAD^"), ["pkg/foo.cc"])

  def test_rename_includes_old_and_new_paths(self):
    self.git("mv", "pkg/foo.cc", "pkg/new foo.cc")
    self.commit()
    self.assertEqual(
        check_dwyu.get_changed_files("HEAD^"),
        ["pkg/foo.cc", "pkg/new foo.cc"],
    )

  def test_empty_and_binary_files_do_not_require_diff_hunks(self):
    self.write_file("pkg/empty.cc", "")
    self.write_file("pkg/binary.cc", "\0data")
    self.commit()
    self.assertEqual(
        check_dwyu.get_changed_files("HEAD^"),
        ["pkg/binary.cc", "pkg/empty.cc"],
    )

  def test_deleted_file(self):
    self.git("rm", "pkg/foo.cc")
    self.commit()
    self.assertEqual(check_dwyu.get_changed_files("HEAD^"), ["pkg/foo.cc"])
