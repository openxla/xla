...
class XLAConfigOptions:
  ...
  def to_bazelrc_lines(
    ...
    if (dpav.clang_major_version is not None and dpav.clang_major_version < 16) or (
      dpav.gcc_major_version is not None and dpav.gcc_major_version < 13
    ):
      rc.append("build --define=xnn_enable_avxvnniint8=false")

    # Prevent XNNPACK from using `-mavx512fp16` (only available in clang-14+/gcc-12+)
    if (dpav.clang_major_version is not None and dpav.clang_major_version < 14) or (
      dpav.gcc_major_version is not None and dpav.gcc_major_version < 12
    ):
