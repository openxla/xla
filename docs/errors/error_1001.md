...
[--xla_tpu_scoped_vmem_limit_kib](https://openxla.org/xla/flags_guidance) and
...
* **Custom Kernel Scoped Vmem OOM**:
If the error points to a custom kernel → Jump to
[Retune the Kernel](#retune_the_kernel).
* **Non-Kernel Vmem Issues**:
If the Vmem OOM occurs due to a non-custom-kernel op, it is likely an internal
