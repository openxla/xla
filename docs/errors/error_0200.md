...
deliberate interrupt triggered by the XLA runtime safety checks.
...
hard hardware constraint (e.g., an instruction attempting to access an
...
### Scenario 3: XLA On-Device Check Failures

**Signature:** The error message contains specific details on the compiler
inject safety check that is failing. Look for the for the following keywords:

* `BoundsCheck`, `scheckne`, `scheckeq`, `schecklt`, `scheckge`, `scheckbetween`

This indicates that a compiler injected safety check into the compiled
...
* **Consistent Environment:** Check that all hosts are using identical
Docker images and libtpu versions.
* **Disable Enhanced Barrier (TPU v5):** If on TPU v5, try setting
