...
- **Retry the job:** If the issue was a transient voltage dip or network flap, a
  simple retry may work.
- **Identify and remove bad nodes:** If the error persists on the same specific
  task or host, the hardware is likely defective. Use your cluster management
  tools to "drain"/"cordon" the affected node and restart your job on healthy
  nodes.
...
- **File an XLA bug:** This is likely a compiler bug, the compiler should never
  emit instructions that violate hardware specs.
...
- **Debug custom kernels:** If using Pallas, check your index calculations. Use
