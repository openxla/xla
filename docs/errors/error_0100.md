...
- **Reduce your model's memory footprint**
  ...
  - **Parameter sharding:** For very large models, use techniques like model
    parallelism or sharding to distribute parameters across the HBM of multiple
    TPU cores or hosts.
  - **Shorten sequence/context length:** For models that operate on sequences
    (like language models), reducing the input sequence length can significantly
...
- **Address memory layout and padding**
  - TPU memory is allocated in chunks, and padding can increase the actual size
    of tensors.
