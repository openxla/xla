...
accept `default`, `off`, `on`, or `explicit`. `default` follows
optimization effort: it selects `on` at O1 or above, or when execution-time
optimization effort is at least 0.2; otherwise, it selects `off`. Any other
mode overrides optimization effort. All-gather and all-reduce default to
`default`, while reduce-scatter defaults to `on`. See
...
*   Collective pipelining uses `on` by default for operations commonly used
    for data-parallel communication. The `off` and `explicit` modes still
