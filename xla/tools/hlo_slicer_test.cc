...
TEST_F(HloSlicerTest, SingleComputationSlice) {
...

  {
    ...
    auto sliced_result = ForwardSliceModule(
        hlo_module.get(), relevant_instructions, hlo_selector);

    auto sliced_insts = sliced_result.sliced_insts;
    EXPECT_EQ(sliced_insts.size(), 1);
    EXPECT_EQ(sliced_insts[entry_comp].size(), 4);
    EXPECT_TRUE(sliced_insts[entry_comp].contains(p2));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(x));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(y));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(add1));

    EXPECT_EQ(sliced_result.num_frontier_insts(), 0);
  }

  {
    ...
    EXPECT_EQ(sliced_insts.size(), 1);
    EXPECT_EQ(sliced_insts[entry_comp].size(), 4);
    EXPECT_TRUE(sliced_insts[entry_comp].contains(add0));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(x));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(p3));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(add1));
...
TEST_F(HloSlicerTest, MultipleComputationSlice) {
...

  {
    ...
    EXPECT_EQ(sliced_insts.size(), 1);
    EXPECT_EQ(sliced_insts[entry_comp].size(), 4);
    EXPECT_TRUE(sliced_insts[entry_comp].contains(add2));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(add1));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(while1));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(loop_count));
...

  {
    ...
    auto sliced_insts = sliced_result.sliced_insts;
    EXPECT_EQ(sliced_insts.size(), 2);
    EXPECT_TRUE(sliced_insts.contains(entry_comp));
    EXPECT_TRUE(sliced_insts.contains(calculate_alpha_comp));
    EXPECT_FALSE(sliced_insts[entry_comp].contains(add1));
...

  {
    ...
    EXPECT_EQ(sliced_insts.size(), 2);
    EXPECT_TRUE(sliced_insts.contains(entry_comp));
    EXPECT_TRUE(sliced_insts.contains(while_body_comp));
    EXPECT_FALSE(sliced_insts.contains(while_cond_comp));
    EXPECT_FALSE(sliced_insts[entry_comp].contains(tuple1));
    EXPECT_FALSE(sliced_insts[entry_comp].contains(add1));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(add2));
    EXPECT_FALSE(sliced_insts[while_body_comp].contains(gte1));
...
TEST_F(HloSlicerTest, SingleComputationForwardFrontier) {
...

  {
    ...
    auto sliced_insts = sliced_result.sliced_insts;
    EXPECT_EQ(sliced_result.num_sliced_insts(), 4);
    EXPECT_EQ(sliced_insts.size(), 1);
    EXPECT_TRUE(sliced_insts[entry_comp].contains(add0));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(broadcast));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(y));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(x));

    EXPECT_EQ(sliced_result.num_frontier_insts(), 1);
    auto frontier_insts = sliced_result.frontier_insts;
    EXPECT_TRUE(frontier_insts[entry_comp].contains(x));
  }

  {
    ...
    auto sliced_insts = sliced_result.sliced_insts;
    EXPECT_EQ(sliced_result.num_sliced_insts(), 5);
    EXPECT_EQ(sliced_insts.size(), 1);
    EXPECT_TRUE(sliced_insts[entry_comp].contains(add0));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(y));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(x));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(p5));
    EXPECT_TRUE(sliced_insts[entry_comp].contains(sub1));

    EXPECT_EQ(sliced_result.num_frontier_insts(), 2);
    auto frontier_insts = sliced_result.frontier_insts;
    EXPECT_TRUE(frontier_insts[entry_comp].contains(x));
    EXPECT_TRUE(frontier_insts[entry_comp].contains(sub1));
  }
}
