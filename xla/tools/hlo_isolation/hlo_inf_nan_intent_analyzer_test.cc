...
ENTRY main {
  ...
  EXPECT_TRUE(IsIntentionalInfNan(*module));
  InfNanIntentOptions allow_unconstrained;
  allow_unconstrained.reject_unconstrained_ops = false;
  EXPECT_TRUE(IsIntentionalInfNan(*module, allow_unconstrained));
  ...
  InfNanIntentOptions reject_unconstrained;
  reject_unconstrained.reject_unconstrained_ops = true;
  EXPECT_FALSE(IsIntentionalInfNan(*module, reject_unconstrained));
}
...
ENTRY main {
  ...
  InfNanIntentOptions reject_unconstrained;
  reject_unconstrained.reject_unconstrained_ops = true;
  EXPECT_TRUE(IsIntentionalInfNan(*module, reject_unconstrained));
}
...
TEST(HloInfNanIntentAnalyzerTest,
     NonFloatingExtractionWithHazardDoesNotPropagateInStructuralMode) {
  const absl::string_view kHlo = R"hlo(
HloModule non_floating_extraction_with_hazard
ENTRY main {
  c_inf = f32[] constant(inf)
  s_data = s32[5] parameter(0)
  t = (f32[], s32[5]) tuple(c_inf, s_data)
  gte = s32[5] get-tuple-element(t), index=1
  param_float = f32[5] parameter(1)
  sq = f32[5] sqrt(param_float)
  ROOT out = (f32[5], s32[5]) tuple(sq, gte)
}
)hlo";
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
                       ParseAndReturnUnverifiedModule(kHlo));
  // In structural mode (hazard op present), non-floating extractions must not
  // be considered propagating paths.
  EXPECT_FALSE(IsIntentionalInfNan(*module));
}

TEST(HloInfNanIntentAnalyzerTest, VariadicMaskedReductionPasses) {
  const absl::string_view kHlo = R"hlo(
HloModule variadic_masked_reduction
%variadic_reducer (k1: f32[], v1: s32[], k2: f32[], v2: s32[]) -> (f32[], s32[]) {
  %k1 = f32[] parameter(0)
  %v1 = s32[] parameter(1)
  %k2 = f32[] parameter(2)
  %v2 = s32[] parameter(3)
  %cmp = pred[] compare(%k1, %k2), direction=GT
  %res_k = f32[] select(%cmp, %k1, %k2)
  %res_v = s32[] select(%cmp, %v1, %v2)
  ROOT %res = (f32[], s32[]) tuple(%res_k, %res_v)
}

ENTRY main {
  mask = pred[10] parameter(0)
  keys = f32[10] parameter(1)
  vals = s32[10] parameter(2)
  c_neg_inf = f32[] constant(-inf)
  b_neg_inf = f32[10] broadcast(c_neg_inf), dimensions={}
  masked_keys = f32[10] select(mask, keys, b_neg_inf)
  c_zero = s32[] constant(0)
  ROOT r = (f32[], s32[]) reduce(masked_keys, vals, c_neg_inf, c_zero), dimensions={0}, to_apply=%variadic_reducer
}
)hlo";
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
                       ParseAndReturnUnverifiedModule(kHlo));
...
TEST(HloInfNanIntentAnalyzerTest, IsStructuralOperandRejectsSelectCondition) {
  const absl::string_view kHlo = R"hlo(
HloModule test_select
ENTRY main {
  p = pred[10] parameter(0)
  a = f32[10] parameter(1)
  b = f32[10] parameter(2)
  ROOT sel = f32[10] select(p, a, b)
}
)hlo";
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
                       ParseAndReturnUnverifiedModule(kHlo));
  const HloInstruction* sel = module->entry_computation()->root_instruction();
  // Operand 0 is the predicate condition, which does not pass data.
  EXPECT_FALSE(internal::IsStructuralOperand(sel, sel->operand(0)));
  // Operands 1 and 2 are data branches.
  EXPECT_TRUE(internal::IsStructuralOperand(sel, sel->operand(1)));
  EXPECT_TRUE(internal::IsStructuralOperand(sel, sel->operand(2)));
}

TEST(HloInfNanIntentAnalyzerTest, ExtinguishingOpsDoNotPropagateToRoot) {
  const absl::string_view kHlo = R"hlo(
HloModule tanh_extinguishes_inf
ENTRY main {
  c_inf = f32[10] constant(inf)
  ROOT out = f32[10] tanh(c_inf)
}
)hlo";
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
                       ParseAndReturnUnverifiedModule(kHlo));
...
TEST(HloInfNanIntentAnalyzerTest, MaskedReductionWithInfOnTrueBranchPasses) {
  const absl::string_view kHlo = R"hlo(
HloModule masked_reduction_on_true
%max_reducer (x: f32[], y: f32[]) -> f32[] {
  %x = f32[] parameter(0)
  %y = f32[] parameter(1)
  ROOT %maximum = f32[] maximum(%x, %y)
...
ENTRY main {
  is_pad = pred[10] parameter(0)
  data = f32[10] parameter(1)
  data_sqrt = f32[10] sqrt(data)
  c_neg_inf = f32[] constant(-inf)
  b_neg_inf = f32[10] broadcast(c_neg_inf), dimensions={}
  sel = f32[10] select(is_pad, b_neg_inf, data_sqrt)
  ROOT r = f32[] reduce(sel, c_neg_inf), dimensions={0}, to_apply=%max_reducer
}
)hlo";
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
