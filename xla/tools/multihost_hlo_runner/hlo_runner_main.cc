...
int main(int argc, char** argv) {
  ...
  bool enable_mock_gpu = false;
  int mock_nccl_bandwidth = 0;
  ...
  std::vector<tsl::Flag> flag_list = {
      ...
      tsl::Flag("enable_mock_gpu", &enable_mock_gpu,
                "Should we simulate multi-hosts run with mock gpus?"),
      tsl::Flag("mock_nccl_bandwidth", &mock_nccl_bandwidth,
                "The mock bandwidth of NCCL collectives in GB/s"),
  };
  ...
  if (enable_mock_gpu) {
    ...
    client = xla::FunctionalHloRunner::CreateMockGpuClient(mock_nccl_bandwidth,
                                                           num_nodes);
  } else {
