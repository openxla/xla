...
Status FunctionalHloRunner::PrepareHloModuleForCompilation(
  ...
  if (preproc_options.flatten_while_loop() ||
    ...
    int max_outer_loop_count =
        preproc_options.max_outer_loop_count.value_or(while_execution_count);
    int max_loop_count =
        preproc_options.max_loop_count.value_or(while_execution_count);
