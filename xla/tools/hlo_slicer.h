 ...
 public:
  SliceOutput(absl::flat_hash_map<const HloComputation*,
              ...
              absl::flat_hash_map<const HloComputation*,
                                  absl::flat_hash_set<const HloInstruction*>>
                  frontier_insts)
      : sliced_insts_(sliced_insts), frontier_insts_(frontier_insts) {}

  const absl::flat_hash_map<const HloComputation*,
                            absl::flat_hash_set<const HloInstruction*>>&
  sliced_insts() const {
    return sliced_insts_;
  }

  const absl::flat_hash_map<const HloComputation*,
                            absl::flat_hash_set<const HloInstruction*>>&
  frontier_insts() const {
    return frontier_insts_;
  }

  // Return the total number of the sliced instructions
  int num_sliced_insts() const { return count_map_of_set_(sliced_insts_); }

  // Return the total number of the frontier instructions
  int num_frontier_insts() const { return count_map_of_set_(frontier_insts_); }
