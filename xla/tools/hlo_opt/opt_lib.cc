...
static auto& provider_map ABSL_GUARDED_BY(provider_mu) = *new ProviderMap();
...
/*static*/ void OptProvider::RegisterForPlatform(
  ...
  CHECK(!provider_map.contains(platform));
  provider_map[platform] = std::move(translate_provider);
}

/*static*/ OptProvider* OptProvider::ProviderForPlatform(
    se::Platform::Id platform) {
  absl::MutexLock l(&provider_mu);
  auto it = provider_map.find(platform);
