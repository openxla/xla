...
// TODO(ecg): remove when there are no users left.
struct OldEncodedBufferInfo {
  uint64_t packed_kind_and_size = 0;
  uint64_t entry_param_number = -1;
};
...
class BufferInfo {
 public:
  ...
  // TODO(ecg): replace this constructor with one that takes a
  // const EncodedBufferInfo&.
  ...
  // TODO(ecg): remove when there are no users left of OldEncodedBufferInfo.
  explicit BufferInfo(const OldEncodedBufferInfo& encoded)
      : entry_param_number_(encoded.entry_param_number),
        result_param_number_(-1) {
    Kind kind;
    uint64_t size;
  ...
  // TODO(ecg): remove when there are no users left of OldEncodedBufferInfo.
  OldEncodedBufferInfo EncodeOld() const {
    static_assert(sizeof(*this) == 16, "");
    OldEncodedBufferInfo ret;
    ret.packed_kind_and_size = Pack(kind(), size_);
    ret.entry_param_number = entry_param_number_;
    return ret;
  }
