...
void HloModuleImporter::ImportFrontendAttributes(const xla::HloModule& hlo_module, mlir::ModuleOp module) {
  if (!hlo_module.frontend_attributes().map().empty()) {
    ...
    if (!frontend_attributes.empty()) {
      module->setAttr(kFrontendAttributesAttr, builder_.getDictionaryAttr(frontend_attributes));
    }
  }
}
