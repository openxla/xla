...
void CompareStack(xla::StackFrameIndexProto& proto,
  ...
  while (root_index != BaseStackFrameIndexBuilder::kInvalidIndex) {
...
TEST(BaseStackFrameIndexBuilder, CallStacksAreAddedAndDeduplicatedCorrectly) {
  BaseStackFrameIndexBuilder builder;
  ...
  int result_index = BaseStackFrameIndexBuilder::kInvalidIndex;
  ...
  // same result index
  int compare_result_index = BaseStackFrameIndexBuilder::kInvalidIndex;
  for (auto& frame : user_frames) {
  ...
  compare_result_index = BaseStackFrameIndexBuilder::kInvalidIndex;
  for (auto& frame : user_frames) {
    compare_result_index = builder.AddStackFrameAndReturnId(
        frame.file_name, frame.line_number, frame.function_name,
        frame.column_number, compare_result_index);
