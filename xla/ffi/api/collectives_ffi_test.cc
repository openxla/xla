...
static void* const kFakeCommunicator = reinterpret_cast<void*>(0xC0FFEE);
...
struct FakeBackend {
  ...
  XLA_FFI_GroupMode requested_group_mode = XLA_FFI_GROUP_CROSS_REPLICA;
...
TEST(CollectivesFfiTest, RequestAndGetCommunicator) {
  ...
  auto handler = Ffi::Bind().Ctx<Extension<Collectives>>().To(
      [&](Communicator comm) -> Error {
        ...
        ErrorOr<void*> comm_or = comm.GetCommunicator(
            GroupMode::kFlattenedId, {{0, 1}}, /*communication_id=*/7);
