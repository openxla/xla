...
// An owning, type-safe interface to a set of FFI attributes.
//
...
//   ABSL_ASSIGN_OR_RETURN(auto dims, attrs.Get<absl::Span<const
//   int64_t>>("dims")); ABSL_ASSIGN_OR_RETURN(auto mode,
//   attrs.Get<absl::string_view>("mode"));
//
...
class Attributes {
 public:
  ...
  static Attributes Create(const AttributesMap& attrs) {
    return Attributes(AttributesStorage::Create(attrs));
  }
  ...
  Dictionary dict() const ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return Dictionary(&storage_->ffi_attrs());
  }
