...
#ifndef XLA_MLIR_BACKENDS_EXPERIMENTAL_CONVERSION_DE_BUFFERIZATION_H_
#define XLA_MLIR_BACKENDS_EXPERIMENTAL_CONVERSION_DE_BUFFERIZATION_H_
...
// of IREE we convert it from LMHLO dialects to IREEInput dialect. The key
// difference is that at LMHLO level program operates on buffers (memrefs), and
// at IREEInput level it uses value semantics and tensors.
//
...
// longer term we'll need to add tensor/buffer layouts to Experimental/IREE.
...
#endif  // XLA_MLIR_BACKENDS_EXPERIMENTAL_CONVERSION_DE_BUFFERIZATION_H_
