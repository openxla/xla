...
#include "xla/mlir/backends/experimental/conversion/convert_compiled_ops.h"
#include "xla/mlir/backends/experimental/conversion/convert_library_ops.h"
#include "xla/mlir/backends/experimental/conversion/convert_memref_ops.h"
...
#define GEN_PASS_DECL_CONVERTTOEXPERIMENTAL
#include "xla/mlir/backends/experimental/transforms/passes.h.inc"

#define GEN_PASS_DEF_CONVERTTOEXPERIMENTAL
#include "xla/mlir/backends/experimental/transforms/passes.h.inc"
...
static std::string toString(ExperimentalBackend backend) {
  switch (backend) {
    case ExperimentalBackend::kHAL:
      return "hal";
    case ExperimentalBackend::kStreamExecutor:
      return "streamexecutor";
  }
}

static FailureOr<ExperimentalBackend> parseExperimentalBackend(
    std::string_view str) {
  if (str == "hal") {
    return ExperimentalBackend::kHAL;
  } else if (str == "streamexecutor") {
    return ExperimentalBackend::kStreamExecutor;
  }
  return failure();
}
...
class ConvertToExperimentalPass
    : public ::impl::ConvertToExperimentalBase<ConvertToExperimentalPass> {
 public:
  ConvertToExperimentalPass(ThunkSequence *thunk_sequence,
                            std::optional<ExperimentalBackend> backend)
  ...
  void runOnOperation() override {
    ...
    auto compiled_ops_backend = parseExperimentalBackend(backend);
    ...
    // Convert all memrefs back to tensors, as the Experimental compilation
    // pipeline accepts input IR with value semantics. We rely on tied operands
    // to pass "output tensors" to be used as a storage for results.
    ...
    // Ensure all HLO and memref operations get lowered to IREEInput and
    // Experimental runtime. For this we have to de-bufferize the IR and
    // correctly tie operands with results write into the destination buffers.
    ...
    if (failed(applyPartialConversion(getOperation(), target,
      ...
      getOperation().emitError()
          << "conversion from Hlo to Experimental failed";
      return signalPassFailure();
    }
  }
...
std::unique_ptr<OperationPass<ModuleOp>> createConvertToExperimentalPass(
    ThunkSequence *thunk_sequence, std::optional<ExperimentalBackend> backend) {
  return std::make_unique<ConvertToExperimentalPass>(thunk_sequence, backend);
}
