...
int main(int argc, char **argv) {
  ...
  // `MlirOptMain` below.
  int32_t argc1 = 1;
  tsl::port::InitMain("Xla Experimental Backend Pass Driver", &argc1, &argv);
  ...
  // Lowering to Experimental vruntime.
  xla::gpu::registerExperimentalPases();

  return failed(MlirOptMain(
      argc, argv, "Xla Experimental Backend Pass Driver\n", registry));
}
