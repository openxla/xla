...
cublasLtPointerMode_t AsCublasLtPointerMode(gpu::BlasLt::PointerMode pointer_mode) {
  switch (pointer_mode) {
  ...
  TF_RETURN_IF_ERROR(
      SetAttr(cu_layout, CUBLASLT_MATRIX_LAYOUT_ORDER,
          int32_t{(m.order == gpu::MatrixLayout::Order::kRowMajor) ? 
                CUBLASLT_ORDER_ROW : CUBLASLT_ORDER_COL}));
...
auto BlasLt::MatmulPlan::GetAlgorithms(size_t max_algorithm_count) const ->
  ...
  std::vector<cublasLtMatmulHeuristicResult_t> results(max_algorithm_count);
  {
    ...
    Owned<cublasLtMatmulPreference_t> preference(cu_preference, 
    ...
    SE_CUBLAS_RETURN_IF_ERROR(cublasLtMatmulAlgoGetHeuristic(
        ...
        b_desc_.get(), c_desc_.get(), d_desc_.get(),
...
auto BlasLt::GetMatmulPlan(const gpu::GemmConfig& cfg,
         gpu::BlasLt::Epilogue epilogue) const -> tsl::StatusOr< MatmulPlanPtr >
{
  ...
  return std::make_unique< MatmulPlan >(*this,
      std::move(op_desc), std::move(a_desc),
      std::move(b_desc), std::move(c_desc),
      std::move(d_desc),
      cfg.alpha, cfg.beta, must_swap_operands);
}

tsl::Status BlasLt::MatmulPlan::ValidateInputs(blas::DataType scale_type,
        bool alpha_on_device, bool beta_on_device, blas::DataType A_type, 
        blas::DataType B_type, blas::DataType C_type, blas::DataType D_type) const 
{
...
tsl::Status BlasLt::MatmulPlan::DoMatmul(Stream* stream, const void* alpha, 
                             DeviceMemoryBase a,
...

  {
    ...
    if(auto palgo = std::any_cast< cublasLtMatmulAlgo_t >(&algorithm.opaque_algo)) {
      ...
      SE_CUBLAS_RETURN_IF_ERROR(cublasLtMatmul(
          ...
          c_desc_.get(), d.opaque(), d_desc_.get(), palgo,
...
tsl::Status BlasLt::MatmulPlan::ExecuteOnStream(
    Stream* stream, DeviceMemoryBase a,
    DeviceMemoryBase b, DeviceMemoryBase c,
  ...
  std::tuple operand_types{a_desc_.type(), b_desc_.type(),
                    c_desc_.type(), d_desc_.type()};
...
#define TYPED_MATMUL(SCALENTYPE, ATYPE, BTYPE, CTYPE, DTYPE)                \
  if (operand_types == std::make_tuple(ATYPE, BTYPE, CTYPE, DTYPE)) {       \
    return gpu::BlasLt::MatmulPlan::DoMatmul<SCALENTYPE,                    \
          CudaToNativeT<ATYPE>::type, CudaToNativeT<BTYPE>::type,           \
          CudaToNativeT<CTYPE>::type, CudaToNativeT<DTYPE>::type>(          \
        stream, alpha_, a, b, beta_, c, d, bias,                            \
        aux, a_scale, b_scale, c_scale,                                     \
