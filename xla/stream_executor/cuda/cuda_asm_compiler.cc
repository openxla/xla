...
absl::StatusOr<std::vector<uint8_t>> CompileAndLinkUsingLibNvJitLink(
  ...
  nvJitLinkResult linking_result = [&] {
    // There is a memory leak in nvJitLink that occurs in some cases.
    // TODO(b/319271534): Figure out what to do with this leak.
    absl::LeakCheckDisabler disable_leak_checks{};
    return nvJitLinkComplete(link_handle);
  }();
