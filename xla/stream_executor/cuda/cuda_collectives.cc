...
/* static */ absl::StatusOr<void*> CudaCollectives::CollectiveMemoryAllocate(
  ...
  if (xla::GetDebugOptionsFromFlags().xla_gpu_enable_nvshmem()) {
    return xla::gpu::NvshmemApi::Default().Allocate(bytes);
  }
...
/* static */ absl::Status CudaCollectives::CollectiveMemoryDeallocate(
  ...
  if (xla::GetDebugOptionsFromFlags().xla_gpu_enable_nvshmem()) {
    return xla::gpu::NvshmemApi::Default().Deallocate(location);
  }
