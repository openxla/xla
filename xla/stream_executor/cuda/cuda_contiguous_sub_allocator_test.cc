...
TEST_F(CudaContiguousSubAllocatorTest, GrowthPreservesLiveDeviceBuffers) {
  ASSERT_OK_AND_ASSIGN(auto sub,
                       CreateCudaContiguousSubAllocator(executor_, 128 << 20));
  ...
  // The free arena coalesces across its original physical mapping boundary.
  // Fill and read the whole span to exercise both backing handles.
  void* spanning = allocator.AllocateRaw(page, 6 * page, upper);
  ASSERT_EQ(spanning, collective);
  EXPECT_EQ(allocator.GetStats()->pool_bytes, 6 * page);
  ASSERT_EQ(cuMemsetD8(reinterpret_cast<CUdeviceptr>(spanning), 0x5a, 6 * page),
            CUDA_SUCCESS);
  for (size_t offset : {size_t{0}, 4 * page - sizeof(result), 4 * page,
                        6 * page - sizeof(result)}) {
    ASSERT_EQ(
        cuMemcpyDtoH(&result, reinterpret_cast<CUdeviceptr>(spanning) + offset,
                     sizeof(result)),
        CUDA_SUCCESS);
    EXPECT_EQ(result, 0x5a5a5a5a);
  }
  allocator.DeallocateRaw(spanning);
}
