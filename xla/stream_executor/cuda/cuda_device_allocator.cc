...
DeviceAllocate(StreamExecutor* executor,
               const CudaDeviceAllocator::Options& options, uint64_t size) {
...
class CudaDeviceMemoryAllocation : public MemoryAllocation {
 public:
  ...
  ~CudaDeviceMemoryAllocation() final {
    if (ptr_ != nullptr) {
      DeviceDeallocate(executor_, ptr_, padded_size_, handle_);
    }
  }
...
absl::StatusOr<std::unique_ptr<MemoryAllocation>> CudaDeviceAllocator::Allocate(
  ...
  ASSIGN_OR_RETURN(auto result, DeviceAllocate(executor_, options_, size));
