...
// Maps physical allocations at successive addresses without changing any
// existing mapping. Physical memory is obtained only when Alloc is called.
absl::StatusOr<std::unique_ptr<ContiguousSubAllocator>>
CreateCudaContiguousSubAllocator(
    StreamExecutor* executor, size_t capacity,
