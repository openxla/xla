...
TEST(XlaHloModuleConfig, ToAndFromC) {
  ...
  ASSERT_OK_AND_ASSIGN(xla::HloModuleConfigProto in_config_proto,
                       in_config.ToProto());
  ASSERT_OK_AND_ASSIGN(xla::HloModuleConfigProto out_config_proto,
                       out_config.ToProto());
...
TEST(XlaHloModule, ToAndFromC) {
  ...
  ASSERT_OK_AND_ASSIGN(xla::HloModuleProtoWithConfig in_module_proto,
                       in_module.ToProtoWithConfig());
  ASSERT_OK_AND_ASSIGN(xla::HloModuleProtoWithConfig out_module_proto,
                       out_module.ToProtoWithConfig());

  out_module_proto.mutable_hlo_module()->set_id(
      in_module_proto.hlo_module().id());

  google::protobuf::util::MessageDifferencer diff;
  diff.set_message_field_comparison(
      google::protobuf::util::MessageDifferencer::EQUIVALENT);
  EXPECT_TRUE(diff.Equals(in_module_proto, out_module_proto));
...
TEST(HostCommandHandler, ToC) {
  ...
  HostCommandHandler cpp_handler = [&side_effect](uint32_t command,
    ...
    ss << "stack_offset: ";
    ss.width(2 * sizeof(stack_offset));
    ss << stack_offset;
    side_effect = ss.str();
  };
  SE_TpuHostCommandHandler c_handler;
  ToC(cpp_handler, &c_handler);
...
TEST(HostCommandHandler, FromC) {
  ...
  HostCommandHandler in_handler = [&side_effect](uint32_t command,
    ...
    ss << std::hex << std::showbase;
    ss << "command: " << command << std::endl;
    ss << "stack_offset: " << stack_offset;
    side_effect = ss.str();
  };
  SE_TpuHostCommandHandler c_handler;
  ToC(in_handler, &c_handler);
  ASSERT_NE(c_handler.handler_func, nullptr);
  ASSERT_NE(c_handler.context, nullptr);

  in_handler(kCommand, kStackOffset);
  std::string in_side_effect = side_effect;
  ASSERT_EQ(kExpected, in_side_effect);
