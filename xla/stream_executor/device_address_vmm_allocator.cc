...
absl::StatusOr<std::optional<DeviceAddressVmmAllocator::PendingDeallocationKey>>
DeviceAddressVmmAllocator::TryMapWithoutWaiting(
    PerDeviceState& state, DeviceAddressBase addr,
    MemoryReservation& reservation, uint64_t reservation_offset, uint64_t size,
    DeviceAddressBase reservation_address) {
  auto allocation_it = state.records_by_allocator_address.find(addr.opaque());
  if (allocation_it == state.records_by_allocator_address.end() ||
      !allocation_it->second->allocator_active() ||
  ...
  if (auto stale_reservation_overlap = FindOverlappingRecord(
    ...
    if (stale_record.raw_allocation() == raw_allocation &&
      ...
      ErasePendingDeallocation(state, PendingDeallocationKind::kMap,
                               reservation_address);
      return std::optional<PendingDeallocationKey>();
    }

    // The reservation range is exact but belongs to a different raw allocation
    // or record. Ask the caller to drain the old mapping before retrying.
    return std::optional<PendingDeallocationKey>(PendingDeallocationKey{
  ...
  if (source_record->reservation_stale()) {
    ...
    return std::optional<PendingDeallocationKey>(PendingDeallocationKey{
        PendingDeallocationKind::kMap, source_record->reservation_stale_seqno(),
...
absl::Status DeviceAddressVmmAllocator::Map(int device_ordinal,
  ...
  // a third attempt can install the requested mapping.
  constexpr int kMaxStaleMappingWaits = 2;
  for (int wait_count = 0; wait_count <= kMaxStaleMappingWaits; ++wait_count) {
    ASSIGN_OR_RETURN(
        std::optional<PendingDeallocationKey> stale_mapping,
        TryMapWithoutWaiting(*state, addr, *reservation, reservation_offset,
                             size, reservation_address));
    if (!stale_mapping.has_value()) {
      return absl::OkStatus();
    }
    ...
    RETURN_IF_ERROR(
        WaitAndCompleteStaleReservationMapping(*state, *stale_mapping));
  }

  return absl::AbortedError(
      "Map() allocator state changed repeatedly while waiting for stale "
      "reservation mappings; retry Map()");
}
