...
absl::Status DeviceAddressVmmAllocator::Deallocate(int device_ordinal,
  ...
  const bool use_standalone_marker =
      reclaimable_bytes > kMaxOpenDeallocationBatchBytes;
  uint64_t seqno;
  if (use_standalone_marker) {
    // An indivisible entry larger than the byte limit cannot join an open
    // batch. Flush any earlier batch first, then enqueue this entry's marker
    // before mutating allocator bookkeeping so an enqueue error is atomic.
    RETURN_IF_ERROR(FlushOpenDeallocationBatch(
        *state, DeallocationBatchFlushReason::kByteLimit));
    ...
    // Assign this deferred Deallocate to the current per-device trailing
    // batch. One stream marker will be enqueued for the whole batch when it is
    // flushed.
    seqno = GetOrCreateOpenDeallocationBatchSeqno(*state);
  }
