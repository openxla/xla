...
using namespace stream_executor::wrap;
...
void GpuGraphSupport::DestroyGraph::operator()(GpuGraphHandle graph) {
  ...
  CHECK(err == GpuSuccess)
      << "Failed to destroy gpu graph: "
      << GpuGetErrorString(err);
}

void GpuGraphSupport::DestroyGraphExec::operator()(GpuGraphExecHandle instance) {
  auto err = GpuGraphExecDestroy(instance);
  CHECK(err == GpuSuccess)
      << "Failed to destroy gpu graph instance: "
      << GpuGetErrorString(err);
}
...
tsl::StatusOr<OwnedGpuGraph> CaptureGpuGraph(
  ...
  VLOG(3) << "Capture gpu graph on a stream: "
  ...
  RETURN_IF_GPU_GRAPH_ERROR(GpuStreamBeginCapture(gpu_stream,
  ...
  if (!captured.ok())
    return tsl::errors::Internal("failed to capture gpu graph: ", captured.message());
...
tsl::StatusOr<bool> IsStreamCapturing(stream_executor::Stream* stream) {
  ...
  RETURN_IF_GPU_GRAPH_ERROR(GpuStreamIsCapturing(
