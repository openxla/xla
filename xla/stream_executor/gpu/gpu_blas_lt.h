...
struct MatrixLayout {  // plain MatrixLayout which is extended with create functions in matmul_utils.h
  enum class Order {
...
struct GemmConfig {  // plain GemmConfig which is extended with create functions in matmul_utils.h
...
// using CudaToNativeT = typename ChooseType< What, CUDA_R_8F_E4M3, tsl::float8_e4m3fn,
...
struct BlasLt {
  ...
  struct MatmulPlan {
    ...
    virtual tsl::Status ExecuteOnStream(
        Stream* stream, DeviceMemoryBase a_buffer,
        DeviceMemoryBase b_buffer, DeviceMemoryBase c_buffer,
        DeviceMemoryBase d_buffer,
  ...
  protected:
    ...
    template <typename Scale, typename A, typename B = A, typename C = A, typename D = A>
    tsl::Status DoMatmul(Stream* stream, xla::complex128 alpha,
        DeviceMemoryBase a, DeviceMemoryBase b, double beta,
        DeviceMemoryBase c, DeviceMemoryBase d, DeviceMemoryBase bias,
      ...
      return DoMatmul(stream, HostOrDeviceScalar<Scale>(salpha),
          DeviceMemory<A>(a), DeviceMemory<B>(b),
          HostOrDeviceScalar<Scale>(sbeta), DeviceMemory<C>(c),
          output, algorithm, scratch_allocator, 
          DeviceMemory<C>(bias),
          aux, DeviceMemory<Scale>(a_scale),
          DeviceMemory<Scale>(b_scale),
          DeviceMemory<Scale>(c_scale),
          DeviceMemory<Scale>(d_scale),
          DeviceMemory<Scale>(d_amax), profile_result);
    }

    // used internally by template DoMatmul function to validate inputs
    virtual tsl::Status ValidateInputs(blas::DataType scale_type,
        bool alpha_on_device, bool beta_on_device, blas::DataType A_type, 
        blas::DataType B_type, blas::DataType C_type, blas::DataType D_type) const = 0;

    virtual tsl::Status DoMatmul(Stream* stream, const void* alpha, DeviceMemoryBase a,
  ...
  virtual tsl::StatusOr< MatmulPlanPtr > GetMatmulPlan(const GemmConfig& cfg,
                              Epilogue epilogue) const = 0;
