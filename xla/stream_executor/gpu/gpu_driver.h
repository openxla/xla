...
//
// The order of parameters is generally kept symmetric with the underlying CUDA/ROCm
// driver API.
//
...
class GpuDriver {
 public:
  // Wraps a call to cuInit/hipInit with logging to help indicate what has gone wrong in
  ...
  // CUDA/HIP events can explicitly disable event TSC retrieval for some presumed
  ...
  // Deallocates a location created by HostAllocate, via cuMemFreeHost/hipHostFree.
  ...
  // Registers a memory region at location of size bytes via cuMemHostRegister/hipHostRegister.
