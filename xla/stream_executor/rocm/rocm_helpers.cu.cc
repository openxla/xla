...
__global__ void launchInplaceBiasActivation_kernel(T* c_data, const T* bias_data, 
  ...
  if (x >= n || y >= m)
      return;
  float v = static_cast<float>(c_data[x + y * ldc]) + static_cast<float>(bias_data[x]);
  if (act_mode == 1)
      v = sigmoid(v);
  else if (act_mode == 2)
      v = v > 0.0f ? v : 0.0f;
...
template <typename T>
void launchInplaceBiasActivation(hipStream_t stream, void* c_data, const void* bias_data, 
                                int activation_mode, uint64_t m, uint64_t n, int64_t ldc, 
                                float param) {
  ...
  hipLaunchKernelGGL(kernel,
    dim3(gx, gy, 1),
    dim3(bx, by, 1), 0, stream, 
    static_cast<T*>(c_data),
    static_cast<const T*>(bias_data),
    m, n, ldc, param);
}

template void launchInplaceBiasActivation<__half>(hipStream_t stream, void* c_data, 
                                                const void* bias_data, 
                                                int activation_mode, uint64_t m, 
                                                uint64_t n, int64_t ldc, float param);

template void launchInplaceBiasActivation<hip_bfloat16>(hipStream_t stream, void* c_data, 
                                                      const void* bias_data, 
                                                      int activation_mode, uint64_t m, 
                                                      uint64_t n, int64_t ldc, float param);

template void launchInplaceBiasActivation<float>(hipStream_t stream, void* c_data, 
                                                const void* bias_data, int activation_mode, 
                                                uint64_t m, uint64_t n, int64_t ldc, 
                                                float param);

template void launchInplaceBiasActivation<double>(hipStream_t stream, void* c_data, 
                                                const void* bias_data, int activation_mode, 
                                                uint64_t m, uint64_t n, int64_t ldc, 
                                                float param);
