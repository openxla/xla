...
__global__ void launchInplaceBiasActivation_kernel(T* c_data,
                                                   const Tbias* bias_data,
                                                   const T* side_input_data, float side_input_scale,
  ...
  if(!transpose)
    v = static_cast<float>(c_data[addr]) +
            static_cast<float>(bias_data[x]);
  else
    v = static_cast<float>(c_data[addr]) +
            static_cast<float>(bias_data[y]);
  if(side_input_data != 0)
    v += float(side_input_data[addr]) * side_input_scale;
  if (act_mode == 1)
    v = sigmoid(v);
  else if (act_mode == 2)
    v = v > 0.0f ? v : 0.0f;
...
void launchInplaceBiasActivation(hipStream_t stream, void* c_data,
  ...
  hipLaunchKernelGGL(kernel, dim3(gx, gy, batch), dim3(bx, by, 1), 0, stream,
                     static_cast<T*>(c_data), static_cast<const Tbias*>(bias_data),
...
template void launchInplaceBiasActivation<X, Y>(                            \
    ...
    const void* side_input_data, float side_input_scale,                    \
    int activation_mode, uint64_t batch, uint64_t m, uint64_t n, int64_t ldc, float param);
