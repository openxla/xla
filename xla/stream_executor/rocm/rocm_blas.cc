...
absl::StatusOr<rocblas_datatype> AsRocBlasType(blas::DataType type) {
  ...
  default:
    return absl::InternalError(absl::StrFormat("Unsupported blas data type: %d",
        (int)type));
  }
}
...
absl::StatusOr<rocblas_datatype> AsRocBlasComputeType(
  ...
  default:
    return absl::InternalError(absl::StrFormat("Unsupported compute type: %d",
        (int)type));
  }
}
