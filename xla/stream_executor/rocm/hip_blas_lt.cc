...
/*static*/ tsl::StatusOr<BlasLt::MatrixLayout> BlasLt::MatrixLayout::Create(
  ...
  VLOG(2) << "BlasLt::MatrixLayout::Create type: " << (int)type << " rows: " << m.num_rows
          << " cols: " << m.num_cols << " batch_size: " << m.batch_size
          << " leading_dim_stride: " << *leading_dim_stride << " batch_stride: " << *batch_stride;
