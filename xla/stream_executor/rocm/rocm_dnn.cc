...
template <typename T>
miopenStatus_t miDestroyObject(T obj) { return miopenStatusSuccess; }

template<> miopenStatus_t miDestroyObject(miopenTensorDescriptor_t obj) {
  return wrap::miopenDestroyTensorDescriptor(obj);
}

template<> miopenStatus_t miDestroyObject(miopenConvolutionDescriptor_t obj) {
  return wrap::miopenDestroyConvolutionDescriptor(obj);
}

template<> miopenStatus_t miDestroyObject(miopenPoolingDescriptor_t obj) {
  return wrap::miopenDestroyPoolingDescriptor(obj);
}

template<> miopenStatus_t miDestroyObject(miopenLRNDescriptor_t obj) {
...
struct ScopedDescriptor {
  ...
  ~ScopedDescriptor() {
    ...
    auto status = miDestroyObject(handle_);//wrap::miopenDestroyTensorDescriptor(handle_);
    if (status != miopenStatusSuccess) {
...
using ScopedConvolutionDescriptor = ScopedDescriptor<miopenConvolutionDescriptor_t>;
...
absl::StatusOr<ScopedTensorDescriptor> scope(const BatchDescriptor& batch_descriptor, 
  ...
  switch (batch_descriptor.layout()) {
    ...
    case dnn::DataLayout::kBatchDepthYX: {
      ...
      if (status != miopenStatusSuccess) {
        return absl::InternalError("could not convert BatchDescriptor "
...
absl::StatusOr<ScopedFilterDescriptor> scope(const FilterDescriptor& filter_descriptor, 
                              miopenDataType_t data_type) {
...
absl::StatusOr<ScopedConvolutionDescriptor> scope(const ConvolutionDescriptor& convolution_descriptor) {
    ...
    status = wrap::miopenInitConvolutionNdDescriptor(
        obj.handle_, convolution_descriptor.ndims(), padding.data(), strides.data(),
...
absl::StatusOr<ScopedPoolingDescriptor> scope(const PoolingDescriptor& pooling_descriptor) {
...
absl::StatusOr<ScopedNormalizeDescriptor> scope(const NormalizeDescriptor& normalize_descriptor) {
    ...
    status = wrap::miopenSetLRNDescriptor(obj.handle_, miopenLRNCrossChannel, lrn_N,
...
// around it
struct ScopedActivationDescriptor: ScopedDescriptor<miopenActivationDescriptor_t>
{
  static absl::StatusOr<ScopedActivationDescriptor> 
  Create(dnn::ActivationMode activation_mode, double alpha=0.0) {
    ScopedActivationDescriptor obj;
    obj.alpha_ = alpha;
    auto status = wrap::miopenCreateActivationDescriptor(&obj.handle_);
    if (status != miopenStatusSuccess) {
      return absl::InternalError("call to miopenCreateActivationDescriptor failed: "
                 + ToString(status));
    } else {
      ...
      status = wrap::miopenSetActivationDescriptor(
          obj.handle_, obj.miopen_activation_mode_, obj.alpha_, obj.beta_, obj.gamma_);
      if (status != miopenStatusSuccess) {
        return absl::InternalError("call to miopenSetActivationDescriptor failed: "
                   + ToString(status));
      }
    }
    return obj;
...
class ScopedFusionPlanConvolutionBiasActivation : public ScopedFusionPlanBase {
 public:
  ScopedFusionPlanConvolutionBiasActivation(miopenHandle_t miopen_handle, 
      miopenTensorDescriptor_t input_descriptor)
      : ScopedFusionPlanBase(miopen_handle,
                             miopenVerticalFusion,
                             input_descriptor) {}

  ScopedFusionPlanConvolutionBiasActivation(ScopedFusionPlanConvolutionBiasActivation&& other)
  ...
  static absl::StatusOr<ScopedFusionPlanConvolutionBiasActivation> Create(
    ...
    ScopedFusionPlanConvolutionBiasActivation obj(miopen_handle, input_descriptor);
    ...
    if (!is_compiled) {
      ...
      obj.fusion_plan_compiled_ = !CachedFusionPlans::IsUnsupportedFusionPlan(hash);
    }
    return obj;
  }
...
MIOpenSupport::ConvolveRunnerFromDesc(
  ...
  TF_ASSIGN_OR_RETURN(auto scoped_input_desc, scope(input_descriptor, ToMIOpenDataType(input_type)));
  TF_ASSIGN_OR_RETURN(auto scoped_output_desc, scope(output_descriptor, ToMIOpenDataType(output_type)));
  TF_ASSIGN_OR_RETURN(auto scoped_filter_desc, scope(filter_descriptor, ToMIOpenDataType(input_type)));
...
bool MIOpenSupport::GetMIOpenConvolveAlgorithms(
    dnn::ConvolutionKind kind, dnn::DataType input_type, dnn::DataType output_type,
    Stream* stream,
    const dnn::BatchDescriptor& input_descriptor, DeviceMemoryBase input_data,
    const dnn::FilterDescriptor& filter_descriptor,
    DeviceMemoryBase filter_data, const dnn::BatchDescriptor& output_descriptor,
...
absl::Status MIOpenSupport::GetMIOpenConvolveAlgorithmsImmediateMode(
  ...
  switch (kind) {
    ...
    case dnn::ConvolutionKind::FORWARD_BIAS_ACTIVATION: {
      ...
      if (status != miopenStatusSuccess) {
        return absl::InternalError("call to miopenConvolutionForwardGetSolutionCount failed: "
            + ToString(status));
      }
      break;
    }
    case dnn::ConvolutionKind::BACKWARD_DATA: {
      ...
      if (status != miopenStatusSuccess) {
        return absl::InternalError("call to miopenConvolutionBackwardDataGetSolutionCount "
  ...
  switch (kind) {
    case dnn::ConvolutionKind::FORWARD: {
      ...
      if (status != miopenStatusSuccess) {
        return absl::InternalError("call to miopenConvolutionForwardGetSolution failed: "
                   + ToString(status));
      }
      ...
      for (size_t i = 0; i < solutionCount; i++) {
        ...
        if (status != miopenStatusSuccess) {
          return absl::InternalError("call to miopenConvolutionForwardCompileSolution failed: "
              + ToString(status));
        }
    ...
    case dnn::ConvolutionKind::BACKWARD_DATA: {
      ...
      if (status != miopenStatusSuccess) {
        return absl::InternalError("call to miopenConvolutionBackwardDataGetSolution failed: "
            + ToString(status));
      }
      ...
      for (size_t i = 0; i < solutionCount; i++) {
        ...
        if (status != miopenStatusSuccess) {
          return absl::InternalError(" call to miopenConvolutionBackwardDataCompileSolution "
                        "failed: " + ToString(status));
        }
    ...
    case dnn::ConvolutionKind::BACKWARD_FILTER: {
      ...
      if (status != miopenStatusSuccess) {
        return absl::InternalError("call to miopenConvolutionBackwardWeightsGetSolution failed: "
            + ToString(status));
      }
      ...
      for (size_t i = 0; i < solutionCount; i++) {
        ...
        if (status != miopenStatusSuccess) {
          return absl::InternalError("call to miopenConvolutionBackwardWeightsCompileSolution "
                 "failed: " + ToString(status));
        }
    ...
    default: {
      return absl::InternalError("Unexpected convolution kind " + std::to_string(static_cast<int>(kind)));
    }
  }
...
absl::Status MIOpenSupport::GetMIOpenConvolveAlgorithmsFindMode(
  ...
  switch (kind) {
    ...
    case dnn::ConvolutionKind::FORWARD_BIAS_ACTIVATION: {
      ...
      if (status != miopenStatusSuccess) {
        return absl::InternalError("call to miopenConvolutionForwardGetWorkspaceSize failed: "
            + ToString(status));
      }
      break;
    }
    case dnn::ConvolutionKind::BACKWARD_DATA: {
      ...
      if (status != miopenStatusSuccess) {
        return absl::InternalError("call to miopenConvolutionBackwardDataGetWorkspaceSize failed: "
            + ToString(status));
      }
      break;
    }
    case dnn::ConvolutionKind::BACKWARD_FILTER: {
      ...
      if (status != miopenStatusSuccess) {
        return absl::InternalError("call to miopenConvolutionBackwardWeightsGetWorkspaceSize "
               "failed: " + ToString(status));
      }
      break;
    }
    default: {
      return absl::InternalError("Unexpected convolution kind " + static_cast<int>(kind));
      break;
    }
  }
  ...
  if (scratch_memory_size != 0) {
    if (scratch_allocator == nullptr) {
      return absl::InternalError("An allocator must be specified "
  ...
  switch (kind) {
    ...
    case dnn::ConvolutionKind::FORWARD_BIAS_ACTIVATION: {
      ...
      if (status != miopenStatusSuccess) {
        return absl::InternalError("call to miopenFindConvolutionForwardAlgorithm failed: "
                   + ToString(status));
      }
      break;
    }
    case dnn::ConvolutionKind::BACKWARD_DATA: {
      auto status = wrap::miopenFindConvolutionBackwardDataAlgorithm(
          ...
          scratch_memory_size, exhaustiveSearch);
      if (status != miopenStatusSuccess) {
        return absl::InternalError("call to miopenFindConvolutionBackwardDataAlgorithm failed: "
            + ToString(status));
      }
      break;
    }
    case dnn::ConvolutionKind::BACKWARD_FILTER: {
      auto status = wrap::miopenFindConvolutionBackwardWeightsAlgorithm(
          ...
          scratch_memory_size, exhaustiveSearch);
      if (status != miopenStatusSuccess) {
        return absl::InternalError("call to miopenConvolutionBackwardWeightsAlgorithm "
                      "failed: " + ToString(status));
      }
      break;
    }
...
bool MIOpenSupport::DoBatchNormalizationForward(
  ...
  return DoBatchNormalizationForwardImpl<Eigen::bfloat16, float>(
      ...
      batch_var, saved_mean, saved_inv_var, is_training).ok();
}
...
absl::Status MIOpenSupport::DoBatchNormalizationForwardImpl(
  ...
  if (status != miopenStatusSuccess) {
    return absl::InternalError("failed to enqueue forward batch normalization on stream: "
               + ToString(status));
  }
  return absl::OkStatus();
}
...
absl::Status MIOpenSupport::DoBatchNormalizationBackwardImpl(
  ...
  TF_ASSIGN_OR_RETURN(auto scale_offset_descriptor,
      scope(scale_offset_desc, static_cast<miopenDataType_t>(miopen_scale_type)));
  miopenBatchNormMode_t mode = miopenBNSpatial;
  float one = 1.0;
  float zero = 0.0;
  ...
  if (status != miopenStatusSuccess) {
    return absl::InternalError("failed to enqueue backward batch normalization on stream: "
               + ToString(status));
  }
  return absl::OkStatus();
}
...
void launchInplaceBiasActivation(hipStream_t stream, void* c_data,
                                 ...
                                 int activation_mode,
                                 uint64_t batch, uint64_t m, uint64_t n, int64_t ldc,
                                 float param);
...
absl::Status InplaceBiasActivation(Stream* stream, DeviceMemoryBase c_data,
  ...
  launchInplaceBiasActivation<CT,CTbias>(
      ...
      static_cast<int>(activation_mode)+(transpose?10:0), batch, m, n, ldc, param);
  return absl::OkStatus();
}

template <typename Ta, typename Tb, typename ...Args>
absl::Status InplaceBiasActivation(Stream* stream, 
                                   DeviceMemory<Ta> c_data,
                                   DeviceMemory<Tb> bias_data,
                                   Args... args) {
  return InplaceBiasActivation<Ta, Tb>(stream,
      DeviceMemoryBase(c_data), DeviceMemoryBase(bias_data), args...);
}
...
absl::Status ROCmFusedMatmulRunner::operator()(
  ...
  if (_input_type == dnn::DataType::kFloat)
    return InplaceBiasActivation<float>(stream, c_data, bias_data, side_input, 0.0f,
                                        _activation_mode, 1, _m, _n, _ldc, 0.0f);
  else if (_input_type == dnn::DataType::kHalf)
    return InplaceBiasActivation<Eigen::half>(
        stream, c_data, bias_data,  side_input, 0.0f, _activation_mode, 1, _m, _n, _ldc, 0.0f);
  else if (_input_type == dnn::DataType::kBF16)
    return InplaceBiasActivation<Eigen::bfloat16>(
        stream, c_data, bias_data,  side_input, 0.0f, _activation_mode, 1, _m, _n, _ldc, 0.0f);
  else if (_input_type == dnn::DataType::kDouble)
    return InplaceBiasActivation<double>(stream, c_data, bias_data, side_input, 0.0f,
                                         _activation_mode, 1, _m, _n, _ldc, 0.0f);
  else
    return absl::InvalidArgumentError("Unsupported input type");
}
...
public:
  ...
  absl::Status operator()(Stream* stream, dnn::ProfileResult* profile_result,
      ...
      bool do_unfused = (side_input_scale_ != 0.0) 
                        || !fusion_plan_.CompilationSucceeded();
 ...
 public:
  ...
  static absl::StatusOr<std::unique_ptr<const dnn::FusedConvRunner>> Create(
      ...
      dnn::ActivationMode activation
        ...
        TF_ASSIGN_OR_RETURN(auto input_nd_, scope(input_nd, ToMIOpenDataType(input_type, input_nd.layout())));
        TF_ASSIGN_OR_RETURN(auto output_nd_, scope(output_nd, ToMIOpenDataType(input_type, input_nd.layout())));
        TF_ASSIGN_OR_RETURN(auto filter_, scope(filter, ToMIOpenDataType(input_type)));
        TF_ASSIGN_OR_RETURN(auto bias_nd_, scope(bias_nd, ToMIOpenDataType(bias_type)));
        TF_ASSIGN_OR_RETURN(auto conv_, scope(conv));

        TF_ASSIGN_OR_RETURN(auto activation_desc,
          ScopedActivationDescriptor::Create(activation, leakyrelu_alpha));

        TF_ASSIGN_OR_RETURN(auto fusion_plan,
            ScopedFusionPlanConvolutionBiasActivation::Create(
                miopen->GetHandle(parent, stream).handle(),
                input_nd_.handle(), filter_.handle(),
        ...
        VLOG(2) << "True workspace size " << workspace_size_1 << " " << true_workspace_size;

        auto obj = new RocmFusedConvRunner(parent, stream, miopen, 
          int64_t (solutions[0].solution_id),
          true_workspace_size,
          input_type,
          bias_type,
          conv_scale,  side_input_scale,
          leakyrelu_alpha,
          input_nd, output_nd,
          filter, bias_nd,
          conv,
 ...
 private:
  ...
  RocmFusedConvRunner(GpuExecutor* parent, Stream* stream, MIOpenAccess* miopen, 
                            ...
                            double conv_scale, double side_input_scale,
                            double leakyrelu_alpha,
                            BatchDescriptor dnn_input_nd, BatchDescriptor dnn_output_nd,
                            FilterDescriptor dnn_filter, BatchDescriptor dnn_bias_nd,
                            ConvolutionDescriptor dnn_conv,
                            dnn::ActivationMode activation,
      ...
      : parent_(parent),
        ...
        side_input_scale_f32_(float(side_input_scale)),
  ...
  absl::Status execute_unfused(Stream* stream, dnn::ProfileResult* profile_result,
                         DeviceMemoryBase scratch_memory,
                         DeviceMemoryBase input_data,
                         DeviceMemoryBase filter_data,
                         DeviceMemoryBase side_input_data,
      ...
      if(rank != 4 && rank != 5)
        return absl::InternalError("RocmFusedConvRunner expects 4d or 5d descriptors");
      int d1 = 1, d2 = 1;
      ...
      auto inplace_call = [&](auto out, auto bias) {
        return InplaceBiasActivation(stream,
              out, bias,
              side_input_data,
      ...
      if (input_type_ == dnn::DataType::kFloat && bias_type_ == dnn::DataType::kFloat)
        biasActStatus = inplace_call(DeviceMemory<float>(output_data), DeviceMemory<float>(bias_data));
      else if (input_type_ == dnn::DataType::kHalf && bias_type_ == dnn::DataType::kFloat)
        biasActStatus = inplace_call(DeviceMemory<Eigen::half>(output_data), DeviceMemory<float>(bias_data));
      else if (input_type_ == dnn::DataType::kHalf && bias_type_ == dnn::DataType::kHalf)
        biasActStatus = inplace_call(DeviceMemory<Eigen::half>(output_data), DeviceMemory<Eigen::half>(bias_data));
      else if (input_type_ == dnn::DataType::kBF16 && bias_type_ == dnn::DataType::kFloat)
        biasActStatus = inplace_call(DeviceMemory<Eigen::bfloat16>(output_data), DeviceMemory<float>(bias_data));
      else if (input_type_ == dnn::DataType::kBF16 && bias_type_ == dnn::DataType::kBF16)
        biasActStatus = inplace_call(DeviceMemory<Eigen::bfloat16>(output_data), DeviceMemory<Eigen::bfloat16>(bias_data));
      else 
