...
ROCmFusedMatmulRunner::ROCmFusedMatmulRunner(Stream* stream, dnn::DataType input_type, 
                                            dnn::DataType bias_type, dnn::DataType output_type, 
                                            bool trans_a, bool trans_b, uint64_t m, uint64_t n, 
                                            uint64_t k, int64_t lda, int64_t ldb, int64_t ldc,
                                             dnn::ActivationMode activation_mode) 
                                            : _stream(stream), _input_type(input_type), 
                                            _bias_type(bias_type), _output_type(output_type), 
                                            _trans_a(trans_a), _trans_b(trans_b), _m(m), _n(n), 
                                            _k(k), _lda(lda), _ldb(ldb), _ldc(ldc),
...
tsl::Status ROCmFusedMatmulRunner::gemm(Stream* stream, 
      ...
      DeviceMemoryBase b_data,
      DeviceMemoryBase c_data) const {
        blas::Transpose ta = _trans_a ? 
                        blas::Transpose::kTranspose : blas::Transpose::kNoTranspose;
        blas::Transpose tb = _trans_b ? 
                        blas::Transpose::kTranspose : blas::Transpose::kNoTranspose;
...
template <typename T>
tsl::Status InplaceBiasActivation(Stream* stream, 
                                  DeviceMemoryBase c_data, 
                                  DeviceMemoryBase bias_data, 
                                  dnn::ActivationMode activation_mode, 
                                  uint64_t m, uint64_t n, int64_t ldc, float param){
                                  
  typedef typename std::conditional<std::is_same_v<T,Eigen::half>, __half, 
  ...
  if (activation_mode == dnn::ActivationMode::kReluX || 
    ...
    return tsl::Status(absl::StatusCode::kInvalidArgument,
                      "ROCm InplaceBiasActivation can't be used with parametric activations yet");

  launchInplaceBiasActivation<CT>(AsGpuStreamValue(stream), 
                                  c_data.opaque(), bias_data.opaque(), 
...
tsl::Status ROCmFusedMatmulRunner::operator()(Stream* stream, dnn::ProfileResult* prof,
                                  DeviceMemoryBase scratch_memory, DeviceMemoryBase a_data,
                                  DeviceMemoryBase b_data, DeviceMemoryBase bias_data,
                                  DeviceMemoryBase c_data) const {
    tsl::Status status;
    ...
    if (_input_type == dnn::DataType::kFloat)
      ...
      return InplaceBiasActivation<Eigen::half>(stream, c_data, bias_data, 
                                              _activation_mode, _m, _n, _ldc, 0.0f);
    else if (_input_type == dnn::DataType::kBF16)
      return InplaceBiasActivation<Eigen::bfloat16>(stream, c_data, bias_data, 
                                                _activation_mode, _m, _n, _ldc, 0.0f);
    else if (_input_type == dnn::DataType::kDouble)
...
tsl::Status MIOpenSupport::GetFusedMatmulRunners(
    ...
    std::vector<std::unique_ptr<const dnn::FusedMatmulRunner>>* out_exec_plans) {      
  ...
  auto runner_ptr = new ROCmFusedMatmulRunner(stream, input_type, bias_type,
                                              output_type, trans_a, trans_b, m, n, k, 
