 ...
 public:
  ...
  class MatmulDesc {
   public:
    ...
    hipblasLtComputeType_t compute_type() const { return HIPBLASLT_COMPUTE_F32; }
    hipblasDatatype_t scale_type() const { return HIPBLAS_R_32F; }
    hipblasPointerMode_t pointer_mode() const { return HIPBLAS_POINTER_MODE_HOST; }
  ...
  struct MatmulPlan : public gpu::BlasLt::MatmulPlan {
    ...
    MatmulPlan(const BlasLt& blas_lt_ref, MatmulDesc&& op_desc,
        ...
        blas_lt_ref_(blas_lt_ref), op_desc_(std::move(op_desc)),
        a_desc_(std::move(a_desc)), b_desc_(std::move(b_desc)),
        c_desc_(std::move(c_desc)), d_desc_(std::move(d_desc)),
        alpha_(alpha), beta_(beta), must_swap_operands_(must_swap_operands)
    {} 
