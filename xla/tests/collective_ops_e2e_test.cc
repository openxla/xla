...
TEST_P(AsyncCollectiveOps, AsyncCollectivePermuteUsingPut) {
  ...
  ENTRY test_computation {
    ...
    sum = u32[] add(replica, ten)
    p = u32[2] broadcast(sum), dimensions={}
    permute = u32[2] collective-permute(p), source_target_pairs={{3,0}, {0,1}, {1,2}, {2,3}}
    ROOT copy = u32[2] copy(permute)
  }
  )";
  const int64_t kNumReplicas = 4;
  ...
  const std::vector<Literal>& results = execution_result.results;
  ASSERT_EQ(results.size(), kNumReplicas);
  LiteralTestUtil::ExpectR1Equal<uint32_t>({13, 13}, results[0]);
  LiteralTestUtil::ExpectR1Equal<uint32_t>({10, 10}, results[1]);
  LiteralTestUtil::ExpectR1Equal<uint32_t>({11, 11}, results[2]);
  LiteralTestUtil::ExpectR1Equal<uint32_t>({12, 12}, results[3]);
}
