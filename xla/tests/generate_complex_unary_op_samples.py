...
def disable(op, re, im):
    # Return True to disable samples (re, im) that are know to be
...
def main():
    ...
    for opname in ['Log1p']:
        ...
        size_re, size_im = dict(Log1p=(7, 7)).get(opname, (default_size, default_size))
        ifblocks = []
        ...
        for dtype in [np.complex64, np.complex128]:
            ...
            cnan = {np.float32: 'std::nanf("")', np.float64: 'std::nan("")'}[float_dtype]
            ...
            def _tostr(v):
                if v == pi: return 'pi'
                if v == -pi: return '-pi'
                if v == h_pi: return 'pi_2'
                if v == -h_pi: return '-pi_2'
                if v == q_pi: return 'pi_4'
                if v == -q_pi: return '-pi_4'
                if v == tq_pi: return 'pi3_4'
                if v == -tq_pi: return '-pi3_4'
                if v == finfo.max: return 'max'
                if v == -finfo.max: return '-max'
                if v == finfo.tiny: return 'min'
                if v == -finfo.tiny: return '-min'
            ...
            for x, y in zip(sample.flatten(), values.flatten()):
                ...
                if values.dtype.kind == 'c':
                    ...
                    rows.append(f'{prefix}{{ {{ {re_x}, {im_x} }}, {{ {re_y}, {im_y} }}, {scale} }}')
                else:
                    ...
                    scale = tostr(np.ldexp(1.0, -np.frexp(abs(y))[1]))
                    rows.append(f'{prefix}{{ {{ {re_x}, {im_x} }}, {tostr(y)}, {scale} }}')
            rows = ',\n          '.join(rows)
...
''')
    ...
    output_filename = os.path.join(os.path.dirname(__file__), "complex_unary_op_samples.h")
