...
class ComplexUnaryOpTest : public ClientLibraryTestBase {
 protected:
  ...
  std::vector<T> get_column(const std::vector<std::tuple<Types...>>& table) {
    std::vector<T> column;
    std::transform(table.cbegin(), table.cend(), std::back_inserter(column),
                   [](const auto& item) {
  ...
  void UnaryTestHelper(XlaOp (*Op)(const XlaOp operand)) {
    ...
    float atol;
    // log(10)/log(2) = 3.3219...
    constexpr int precision_deficiency = static_cast<int>(C::dps_deficiency * 3.3219280948873626);
