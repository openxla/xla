        ...
        if (client->ShouldStageHostToDeviceTransfers(src, transfer_size)) {
          ...
          if (staging_buffer != nullptr) {
            ...
            ABSL_RETURN_IF_ERROR(
                stream->DoHostCallback(copy_to_staging_buffer));
            ABSL_RETURN_IF_ERROR(stream->Memcpy(
                &sub_buffer, staging_buffer.get(), transfer_size));
          } else {
            ABSL_RETURN_IF_ERROR(
                stream->Memcpy(&sub_buffer, src, transfer_size));
          }
        } else {
      ...
      if (transfer_size > 0) {
        if (client->ShouldStageHostToDeviceTransfers(dst, transfer_size)) {
          ...
          staging_buffer = client->GetHostMemoryAllocator()->Allocate(
              transfer_size, alloc_opts);
          if (staging_buffer != nullptr) {
            ABSL_RETURN_IF_ERROR(stream->Memcpy(staging_buffer.get(),
                                                sub_buffer, transfer_size));
            ...
            ABSL_RETURN_IF_ERROR(
                stream->Memcpy(dst, sub_buffer, transfer_size));
          }
        } else {
