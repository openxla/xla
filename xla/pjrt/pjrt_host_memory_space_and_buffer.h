...
#ifndef XLA_PJRT_PJRT_HOST_MEMORY_SPACE_AND_BUFFER_H_
#define XLA_PJRT_PJRT_HOST_MEMORY_SPACE_AND_BUFFER_H_
...
// Represents the host memory attached to a PjRtDevice.
class HostMemorySpace : public PjRtMemorySpace {
 public:
  HostMemorySpace(int id, PjRtClient* client, PjRtDevice* device);
  ...
  absl::string_view memory_space_kind() const override { return "host"; }
...
class TfrtCpuBuffer final : public PjRtBuffer {
 public:
  ...
  ABSL_DEPRECATED(
      "TfrtCpuBuffer is bound to HostMemorySpace, use memory_space() instead "
      "when possible.")
  PjRtDevice* device() const override;
  PjRtClient* client() const override { return memory_space_->client(); }
  HostMemorySpace* memory_space() const override { return memory_space_; }
 ...
 private:
  ...
  HostMemorySpace* const memory_space_;
...
#endif  // XLA_PJRT_PJRT_HOST_MEMORY_SPACE_AND_BUFFER_H_
