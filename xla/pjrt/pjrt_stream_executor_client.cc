...
StatusOr<std::unique_ptr<PjRtStreamExecutorBuffer>> AllocateDestinationBuffer(
  ...
  if (is_uninitialized_create) {
    // if the caller provided a definition event then we record that. Also put
    // it as the first definition event so that we can guarantee only the first
    // one might not have event recorded.
    if (definition_event) {
...
PjRtStreamExecutorClient::CreateErrorBuffer(Status error, const Shape& shape,
  ...
  definition_event->SetSequencingEvent(std::move(event).value(), stream.get(),
                                       error);
  local_device->ReturnStreamToPool(std::move(stream));
...
PjRtStreamExecutorClient::CreateViewOfDeviceBuffer(
  ...
  TF_ASSIGN_OR_RETURN(
      LocalDeviceState * local_device,
      tensorflow::down_cast<PjRtStreamExecutorDevice*>(se_device)
          ->GetLocalDeviceState());
...
PjRtStreamExecutorBuffer::CopyToDeviceHelper(
  ...
  auto async_copy_to_device = [src_device_buffer,
    ...
    copy_event->SetSequencingEvent(std::move(event_or).value(), transfer_stream,
                                   defined_status);
...
PjRtFuture<Status> PjRtStreamExecutorBuffer::GetReadyFuture() {
  ...
  if (device_buffer) {
    ...
    if (stream != nullptr) {
      ...
      // All events are already complete; set the `definition_promise` with the
      // status of the buffer's first definition event which may have error
      // status to propagate.
