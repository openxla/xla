...
PjRtMemorySpace* TfrtCpuDevice::host_memory_space() const {
  CHECK_EQ(memory_spaces_.size(), 1);
  CHECK_EQ(memory_spaces_[0]->memory_space_kind(), "host");
  return memory_spaces_[0];
}
...
void TfrtCpuDevice::AttachMemorySpace(PjRtMemorySpace* memory_space) {
  ...
  CHECK_EQ(memory_space->memory_space_kind(), "host") << absl::StrFormat(
      "Could not attach a TfrtCpuDevice to a memory space with kind: %s.",
      memory_space->memory_space_kind());
  CHECK(memory_spaces_.empty())
      << "The TfrtCpuDevice is already attached to a memory space.";
...
TfrtCpuClient::TfrtCpuClient(
  ...
  for (int idx = 0; idx < addressable_devices_.size(); ++idx) {
    ...
    memory_spaces_storage_.push_back(
        std::make_unique<HostMemorySpace>(idx, this, cpu_device));
    auto* host_memory_space = memory_spaces_storage_[idx].get();
    memory_spaces_.push_back(host_memory_space);
    cpu_device->AttachMemorySpace(host_memory_space);
  }
...
StatusOr<std::unique_ptr<TfrtCpuBuffer>> AllocateDestinationBuffer(
  ...
  CHECK_EQ(device->memory_spaces().size(), 1);
  if (!on_device_shape.IsTuple()) {
    ...
    return std::make_unique<TfrtCpuBuffer>(
        ...
        tensorflow::down_cast<HostMemorySpace*>(device->memory_spaces()[0]));
  }
  // Tuple case.
