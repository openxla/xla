...
std::optional<std::vector<OpSharding>> PjRtCApiExecutable::GetOutputShardings()
  ...
  const PJRT_Api* c_api = pjrt_c_api();
  pjrt::LogFatalIfPjrtError(c_api->PJRT_Executable_OutputShardings(&args),
                            c_api);

  if (!args.has_shardings) {
    return std::nullopt;
  }
  ...
  out.reserve(args.num_outputs);
  for (int i = 0; i < args.num_outputs; ++i) {
    std::string sharding_str(args.output_shardings[i],
                             args.output_sharding_sizes[i]);
    out.emplace_back(OpSharding());
    CHECK(out.back().ParseFromString(sharding_str));
  }
  return out;
}
