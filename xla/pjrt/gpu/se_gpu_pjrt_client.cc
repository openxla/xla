...
class AsyncHostToDeviceTransferManager
 ...
 public:
  ...
  Status TransferLiteralToBuffer(
    ...
    VLOG(1) << "AsyncHostToDeviceTransferManager::TransferLiteralToBuffer";
  ...
  void SetBufferError(int buffer_index, Status error) override {
    auto stream = device_->local_device_state()->BorrowStreamFromPool();

    auto event =
        device_->local_device_state()->event_pool().ThenAllocateAndRecordEvent(
            stream.get());
    {
      absl::MutexLock l(&mu_);
      ...
      definition_events_[buffer_index]->SetSequencingEvent(
          std::move(event).value(), stream.get(), error);
    }
    VLOG(1) << "PjRt buffer error: " << error;
    device_->local_device_state()->ReturnStreamToPool(std::move(stream));
  }
