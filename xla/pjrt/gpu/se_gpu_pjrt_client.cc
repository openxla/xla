...
StatusOr<std::unique_ptr<PjRtClient>> GetStreamExecutorGpuClient(
    ...
    PjRtClient::KeyValueGetCallback kv_get,
    PjRtClient::KeyValuePutCallback kv_put, bool enable_mock_gpu,
    int mock_nccl_bandwidth) {
  ...
  if (enable_mock_gpu) {
    ...
    TF_RET_CHECK(mock_nccl_bandwidth > 0);
    gpu_run_options->set_mock_nccl_bandwidth(mock_nccl_bandwidth);
  }
  if (num_nodes > 1) {
