...
absl::Status MultiProcessAllReduceTestBody(int node_id) {
  ...
  if (node_id == 0) {
    ASSIGN_OR_RETURN(service, xla::GetDistributedRuntimeService(
                                  "127.0.0.1:12349",
                                  xla::CoordinationServiceImpl::Options{
  ...
  ASSIGN_OR_RETURN(auto results, executable->Execute({{}}, ExecuteOptions()));
  TF_RET_CHECK(results.size() == 1);
  TF_RET_CHECK(results[0].size() == 1);
  RETURN_IF_ERROR(results[0][0]->GetReadyFuture().Await());

  ASSIGN_OR_RETURN(std::shared_ptr<xla::Literal> result_literal,
...
int main(int argc, char* argv[]) {
  ...
  } else if (test_to_run == "MultiProcessAllReduceHelper") {
    result = xla::MultiProcessAllReduceTestBody(node_id);
  } else {
