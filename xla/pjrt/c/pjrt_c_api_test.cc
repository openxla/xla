...
class TestCApiFactory {
 private:
  ...
  std::optional<ExpectedOutputs> expected_outputs_;
 ...
 public:
  void Register(std::function<const PJRT_Api*()> factory,
    ...
    platform_name_ = absl::AsciiStrToLower(platform_name);
...
class PjrtCApiTest : public PjrtCApiTestBase {
 protected:
  ...
  // Runs the default executable created in PjrtCApiTpuExecutableTest:SetUp
...
TEST_F(PjrtCApiTest, DeviceDebugString) {
  ...
  ASSERT_EQ(error, nullptr);
  CHECK_EQ(expected_outputs_->expected_device_debug_string, args.debug_string);
}
...
TEST_F(PjrtCApiTest, DeviceToString) {
  ...
  ASSERT_EQ(error, nullptr);
  CHECK_EQ(expected_outputs_->expected_device_to_string, args.to_string);
}
