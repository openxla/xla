...
PJRT_Error* PJRT_Client_Create(PJRT_Client_Create_Args* args) {
  ...
  const auto kExpectedOptionNameAndTypes =
      absl::flat_hash_map<std::string, PJRT_NamedValue_Type>({
          ...
          {"override_boot_id", PJRT_NamedValue_Type::PJRT_NamedValue_kString},
      });
  ...
  if (auto it = create_options.find("override_boot_id");
      it != create_options.end()) {
    override_boot_id = std::get<std::string>(it->second);
  }
  ...
  options.override_boot_id = override_boot_id;
