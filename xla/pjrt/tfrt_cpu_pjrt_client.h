...
class TfrtCpuDevice final : public PjRtDevice {
 public:
  ...
  PjRtMemorySpace* host_memory_space() const override;
...
class TfrtCpuClient final : public PjRtClient {
 public:
  ...
  tsl::thread::ThreadPool* pjrt_client_thread_pool() const override {
 ...
 private:
  ...
  // Includes all addressable memory spaces.
  std::vector<std::unique_ptr<PjRtMemorySpace>> memory_spaces_storage_;
