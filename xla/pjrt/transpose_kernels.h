...
#if HWY_MIN_BYTES >= 32
    if constexpr (row_bytes == 2 * kBlockBytes) {
...
void TransposeMicroKernelDispatch(const char* a, int64_t lda, char* b,
                                  int64_t ldb) {
  HWY_STATIC_NAMESPACE::TransposeMicroKernel<T, bs>(a, lda, b, ldb);
}

template <typename T, int bs>
struct TransposeMicroKernel {
  static void Apply(const char* a, int64_t lda, char* b, int64_t ldb) {
    TransposeMicroKernelDispatch<T, bs>(a, lda, b, ldb);
  }
};
}  // namespace xla
#endif  // HWY_ONCE
