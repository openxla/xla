...
void HostCallbackContext::Receive(int res_num,
  ...
  auto chunk = result_channel->Pop();
  chunk.AndThen([this, res_num, metadata, chunk,
                 stream = std::move(stream)]() mutable {
    auto recv_chunk = std::move(*chunk);
    if (!use_major_to_minor_data_layout_for_callbacks_) {
      ...
      auto statusor_linearized =
          host_memory_for_device_manager_->ToDeviceLayout(
              recv_chunk.data(), recv_chunk.size(), host_shape, device_shape);
      recv_chunk = std::move(statusor_linearized.value());
    }

    stream->AddChunk(std::move(recv_chunk)).OnReady([](Status s) {
      TF_CHECK_OK(s);
    });
  });
}
