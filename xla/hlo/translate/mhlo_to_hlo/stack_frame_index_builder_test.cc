...
// Two inlined ops share their caller through the op name wrappers the inliner
// keeps; the callee chain seen again as a root gets frames of its own.
TEST(StackFrameIndexBuilderTest, SharedWrappedCaller) {
  MLIRContext ctx;
  Location a = MakeFrameLoc(&ctx, "A", "a.py", 1);
  Location b = MakeFrameLoc(&ctx, "B", "b.py", 2);
  Location c = MakeFrameLoc(&ctx, "C", "c.py", 3);
  ...
  Location callee_op_a =
      NameLoc::get(StringAttr::get(&ctx, "add:"),
                   NameLoc::get(StringAttr::get(&ctx, "jit(f)/add"),
                                CallSiteLoc::get(a, b)));
  Location callee_op_e =
      NameLoc::get(StringAttr::get(&ctx, "mul:"),
                   NameLoc::get(StringAttr::get(&ctx, "jit(f)/mul"),
                                CallSiteLoc::get(e, b)));
  Location caller_op =
      NameLoc::get(StringAttr::get(&ctx, "call:"),
                   NameLoc::get(StringAttr::get(&ctx, "jit(f)/call"),
                                CallSiteLoc::get(c, d)));
  Location inlined_a = CallSiteLoc::get(callee_op_a, caller_op);
  Location inlined_e = CallSiteLoc::get(callee_op_e, caller_op);

  StackFrameIndexBuilder builder;
  EXPECT_EQ(builder.AddCallStackAndGetFirstFrameId(inlined_a).last_frame_id, 4);
  EXPECT_EQ(builder.AddCallStackAndGetFirstFrameId(inlined_e).last_frame_id, 5);
  EXPECT_EQ(builder.AddCallStackAndGetFirstFrameId(caller_op).last_frame_id, 2);
  EXPECT_EQ(builder.AddCallStackAndGetFirstFrameId(callee_op_a).last_frame_id,
            7);
  EXPECT_EQ(builder.AddCallStackAndGetFirstFrameId(inlined_a).last_frame_id, 4);
  EXPECT_EQ(builder.AddCallStackAndGetFirstFrameId(inlined_e).last_frame_id, 5);

  constexpr absl::string_view kExpected = R"pb(
    file_names: [ "d.py", "c.py", "b.py", "a.py", "e.py" ]
    function_names: [ "D", "C", "B", "A", "E" ]
    file_locations { file_name_id: 1 function_name_id: 1 line: 4 end_line: 4 }
    file_locations { file_name_id: 2 function_name_id: 2 line: 3 end_line: 3 }
    file_locations { file_name_id: 3 function_name_id: 3 line: 2 end_line: 2 }
    file_locations { file_name_id: 4 function_name_id: 4 line: 1 end_line: 1 }
    ...
    stack_frames { file_location_id: 5 parent_frame_id: 3 }
    stack_frames { file_location_id: 3 parent_frame_id: 0 }
    stack_frames { file_location_id: 4 parent_frame_id: 6 }
  )pb";
  EXPECT_THAT(builder.Build(), EqualsProto(kExpected));
}
