...
class HloEvaluatorTypedVisitor : public ConstDfsHloVisitorWithDefault {
 ...
 public:
  ...
  absl::Status HandleRaggedDotBatchModeWithLiterals(const HloInstruction* dot,
    ...
    auto lhs_batch = dot_dims.lhs_batch_dimensions();
    // We must find a match because we are in batch mode.
    std::optional<int64_t> ragged_dim_as_batch_dim;
    for (int64_t i = 0; i < lhs_batch.size(); ++i) {
      if (lhs_ragged_dim == lhs_batch[i]) {
        ragged_dim_as_batch_dim = i;
        break;
      }
    }

    const int64_t ragged_dim_size_in_gs =
        gs_literal.shape().dimensions(ragged_dim_as_batch_dim.value());
    ...
    TF_RETURN_IF_ERROR(result.PopulateParallel<ReturnT>(
        [&](absl::Span<const int64_t> result_index, int /*thread_id*/) {
          ...
          for (int i = 0; i < ragged_dim_size_in_gs; ++i) {
            group_index[ragged_dim_as_batch_dim.value()] = i;
            batches_handled += gs_literal.Get<int64_t>(group_index);
          }
          if (lhs_index[dot_dims.lhs_batch_dimensions(
                  ragged_dim_as_batch_dim.value())] >= batches_handled) {
            return static_cast<ReturnT>(0);
          }
