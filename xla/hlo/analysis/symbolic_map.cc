...
SymbolicMap SymbolicMap::SetNumDimensions(int num_new_dims) const {
  ...
  auto replaced_map_exprs = this->Replace(sym_replacements).GetResults();
  return this->Get(ctx_, num_new_dims, GetNumSymbols(),
                   llvm::SmallVector<SymbolicExpr>(replaced_map_exprs));
}
