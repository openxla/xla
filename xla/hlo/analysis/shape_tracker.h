...
class ShapeTracker {
 public:
  ...
  // Returns true if the memory of the specified input dimensions is contiguous
  // in the output shape (i.e. they are not swapped, and there are no other
  // non-degenerate dimensions between them in the output layout).
  bool IsMemoryContiguous(absl::Span<const int64_t> input_dims) const;
