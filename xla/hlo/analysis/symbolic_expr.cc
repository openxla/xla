...
class Parser {
 ...
 private:
  ...
  SymbolicExpr MaybeParseVariableFromMap() {
    size_t len = 0;
    while (len < remaining_str_.size() &&
           (absl::ascii_isalnum(remaining_str_[len]) ||
            remaining_str_[len] == '_')) {
      len++;
    }
    if (len > 0) {
      auto it =
          variable_map_->find(llvm::StringRef(remaining_str_.data(), len));
  ...
  SymbolicExpr MaybeParseDimAndSymbolVariables() {
    ...
    if (num_dims_ >= 0 && absl::ConsumePrefix(&remaining_str_, "s")) {
      if (num_dims_ <= 0) {
        return ReportError(
      ...
      if (!sym_id.has_value()) {
        return ReportError("Symbol cannot be parsed.");
      }
      return CreateSymbolicVariable(num_dims_ + sym_id.value(), context_);
    }
    return SymbolicExpr();
  }
