...
class Parser {
 ...
 private:
  ...
  SymbolicExpr MaybeParseVariableFromMap() {
    size_t var_len = 0;
    while (var_len < remaining_str_.size() &&
           (absl::ascii_isalnum(remaining_str_[var_len]) ||
            remaining_str_[var_len] == '_')) {
      var_len++;
    }
    if (var_len > 0) {
      auto it =
...
SymbolicExpr ParseSymbolicExpr(absl::string_view expr_str,
                               mlir::MLIRContext* mlir_context,
                               int64_t num_dims) {
  return ParseSymbolicExpr(&expr_str, mlir_context, num_dims);
}
