...
absl::StatusOr<bool> NormalizeSingleOperandAllReduce(
  ...
  if (!ar_shape.has_layout()) {
    *ar_shape.mutable_layout() = LayoutUtil::GetDefaultLayoutForShape(ar_shape);
  }
  ...
  *padded_shape.mutable_layout() =
      LayoutUtil::GetDefaultLayoutForShape(padded_shape);
  ...
  *reshape_shape.mutable_layout() =
      LayoutUtil::GetDefaultLayoutForShape(reshape_shape);
  ...
  std::vector<hlo_sharding_util::FormattingStep> formatting_steps(
      {hlo_sharding_util::FormattingStep{.input_shape = ar_shape,
       ...
       hlo_sharding_util::FormattingStep{
