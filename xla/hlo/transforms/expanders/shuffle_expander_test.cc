...
TEST_F(ShuffleExpanderTest, ExpandsMultiRotate) {
  const char* hlo_string = R"(
HloModule shuffle_module

ENTRY main {
  p0 = f32[3,2]{1,0} parameter(0)
  ROOT shuffle = f32[3,2]{1,0} shuffle(p0), dimensions={0}, mode=multi_rotate, multi_shifts=s32[1,2]{1,0} { { 1, 2 } }
}
)";
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
                       ParseAndReturnVerifiedModule(hlo_string));
  ...
  // The shifts are expanded into the indices of the permute that moves the
  // elements the same way: every slice of dimension 0 takes its values from the
  // coordinate that its own shift rotates it to, and dimension 1 keeps its own
  // coordinate via an iota.
  auto* root = module->entry_computation()->root_instruction();
  EXPECT_THAT(
      root, m::Gather(m::Parameter(0),
                      m::Concatenate(m::Broadcast(m::Constant()), m::Iota())));
  ExpectElementwiseGather(root);
  ...
  auto* broadcast = concatenate->operand(0);
  EXPECT_THAT(broadcast, m::Shape("s32[3,2,1]"));
  EXPECT_THAT(broadcast->dimensions(), ElementsAre(0, 1));
  EXPECT_EQ(broadcast->operand(0)->literal(),
            LiteralUtil::CreateR2<int32_t>({{1, 2}, {2, 0}, {0, 1}}));
...
TEST_F(ShuffleExpanderTest, ExpandsMultiRotateOfRank1) {
  const char* hlo_string = R"(
HloModule shuffle_module

ENTRY main {
  p0 = f32[4]{0} parameter(0)
  ROOT shuffle = f32[4]{0} shuffle(p0), dimensions={0}, mode=multi_rotate, multi_shifts=s32[1]{0} { -1 }
}
)";
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
                       ParseAndReturnVerifiedModule(hlo_string));
  ...
  // A rank 1 operand holds a single slice and so takes a single shift, which
  // wraps around into one index per element.
  auto* root = module->entry_computation()->root_instruction();
  EXPECT_THAT(root, m::Gather(m::Parameter(0), m::Broadcast(m::Constant())));
  ExpectElementwiseGather(root);
  ...
  EXPECT_THAT(broadcast->dimensions(), ElementsAre(0));
  EXPECT_EQ(broadcast->operand(0)->literal(),
            LiteralUtil::CreateR1<int32_t>({3, 0, 1, 2}));
}

TEST_F(ShuffleExpanderTest, ExpandsMultiRotateOfInnermostDimension) {
  const char* hlo_string = R"(
HloModule shuffle_module

ENTRY main {
  p0 = f32[3,2]{1,0} parameter(0)
  ROOT shuffle = f32[3,2]{1,0} shuffle(p0), dimensions={1}, mode=multi_rotate, multi_shifts=s32[3,1]{1,0} { {1}, {0}, {1} }
}
)";
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
                       ParseAndReturnVerifiedModule(hlo_string));
  ...
  // The mirror image of `ExpandsMultiRotate`: the rotated dimension contributes
  // the *last* component of the gather's start indices and the iota of the
  // dimension left in place comes first, so the concatenation orders its
  // components by the dimension they belong to rather than by the shuffle.
  auto* root = module->entry_computation()->root_instruction();
  EXPECT_THAT(
      root, m::Gather(m::Parameter(0),
                      m::Concatenate(m::Iota(), m::Broadcast(m::Constant()))));
  ExpectElementwiseGather(root);
  ...
  auto* iota = concatenate->operand(0);
  EXPECT_THAT(iota, m::Shape("s32[3,2,1]"));
  EXPECT_EQ(Cast<HloIotaInstruction>(iota)->iota_dimension(), 0);

  auto* broadcast = concatenate->operand(1);
  EXPECT_THAT(broadcast, m::Shape("s32[3,2,1]"));
  EXPECT_THAT(broadcast->dimensions(), ElementsAre(0, 1));
  EXPECT_EQ(broadcast->operand(0)->literal(),
