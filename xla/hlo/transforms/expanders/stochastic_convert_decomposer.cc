...
absl::Status DecomposeStochasticConvert(HloComputation* comp,
  ...
  RETURN_IF_ERROR(ShapeInference::InferStochasticConvertShape(
  ...
  if (primitive_util::IsSignedIntegralType(to_type)) {
    ASSIGN_OR_RETURN(HloInstruction * operand_sign,
                     MakeUnaryHlo(HloOpcode::kSign, operand));
    ASSIGN_OR_RETURN(HloInstruction * should_neg,
                     MakeCompareHlo(Comparison::Direction::kLt, operand_sign,
                                    MakeScalarLike(operand_sign, 0)));
    ASSIGN_OR_RETURN(HloInstruction * operand_abs,
                     MakeUnaryHlo(HloOpcode::kAbs, operand));
    ASSIGN_OR_RETURN(HloInstruction * truncated_fp,
    ...
    auto min = static_cast<int64_t>(
        ...
        << (to_bits - 1));
    ASSIGN_OR_RETURN(HloInstruction * is_min,
    ...
    ASSIGN_OR_RETURN(HloInstruction * is_max,
    ...
    RETURN_IF_ERROR(instruction->ReplaceAllUsesWith(result));
    RETURN_IF_ERROR(comp->RemoveInstruction(instruction));
...
absl::StatusOr<bool> StochasticConvertDecomposer::RunImpl(
  ...
  for (HloComputation* computation :
    ...
    for (HloInstruction* instruction :
      ...
      RETURN_IF_ERROR(DecomposeStochasticConvert(computation, instruction));
      changed = true;
    }
  }
  return changed;
