  ...
  switch (shuffle->mode()) {
    case ShuffleMode::kPermute: {
      ABSL_ASSIGN_OR_RETURN(
          indices, shuffle::GetPermuteIndices(shuffle->shuffle_mode()));
      break;
    }
    case ShuffleMode::kMultiRotate: {
      ...
      ABSL_ASSIGN_OR_RETURN(indices,
                            shuffle::GetMultiRotatePermuteIndices(
...
absl::StatusOr<HloInstruction*> ShuffleExpander::ExpandInstruction(
  ...
  switch (shuffle->mode()) {
    case ShuffleMode::kRotate: {
      ...
      for (size_t idx = 0; idx < dimensions.size(); ++idx) {
        ...
        ABSL_ASSIGN_OR_RETURN(
            ...
            MakeSliceHlo(result, start_indices_0, limit_indices_0, strides,
                         &shuffle->metadata()));
        ABSL_ASSIGN_OR_RETURN(
            HloInstruction * slice1,
            MakeSliceHlo(result, start_indices_1, limit_indices_1, strides,
                         &shuffle->metadata()));
    ...
    case ShuffleMode::kPermute:
      ...
      // element of the operand per element of the result expresses, and so
      // does a multi-rotate, which moves every slice by its own shift.
