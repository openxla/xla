...
absl::Status AlgebraicSimplifierVisitor::HandleAllReduceOrReduceScatter(
  ...
  switch (*reduction_kind) {
    ...
    case ReductionKind::SUM: {
      ...
      HloInstruction* group_size_inst =
          computation_->AddInstruction(HloInstruction::CreateConstant(
              LiteralUtil::CreateR0(element_type, group_size)));

      HloInstruction* mul = computation_->AddInstruction(
          HloInstruction::CreateBinary(constant->shape(), HloOpcode::kMultiply,
                                       constant, group_size_inst));

      return ReplaceWithNewInstruction(
          collective,
          HloInstruction::CreateBroadcast(collective->shape(), mul, {}));
    }
    default:
      return absl::OkStatus();
  }
