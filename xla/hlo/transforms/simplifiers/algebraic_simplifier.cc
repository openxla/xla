...
absl::Status AlgebraicSimplifierVisitor::HandleBitcastConvert(
  ...
  if (options_.is_layout_sensitive() &&
    ...
    auto last_dim_is_minor_most = [](const Shape& shape) {
      if (!shape.has_layout()) {
        return true;
      }
      return LayoutUtil::MinorToMajor(shape).front() ==
             shape.dimensions().size() - 1;
    };

    auto last_dim_is_contiguous = [&last_dim_is_minor_most](
                                      const Shape& shape) {
      if (!last_dim_is_minor_most(shape)) {
        return false;
      }
