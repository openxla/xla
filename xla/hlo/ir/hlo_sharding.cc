...
HloSharding::IotaTileAssignment::Create(absl::Span<const int64_t> dims) {
...
HloSharding::IotaTileAssignment::Create(absl::Span<const int64_t> dims,
  ...
  absl::InlinedVector<int64_t, kInlinedDims> canonicalized_dims(
      reshape_dims.begin(), reshape_dims.end());
  absl::InlinedVector<int, kInlinedDims> canonicalized_perm(
      transpose_perm.begin(), transpose_perm.end());
...
HloSharding::IotaTileAssignment::IotaTileAssignment(IotaTileAssignment&& other)
    : ndims_(other.ndims_),
      ...
      out_of_line_(other.out_of_line_) {
  SetPointers();
  if (IsInlined()) {
    std::memcpy(dims_, other.dims_, size_bytes());
  } else {
    other.out_of_line_ = nullptr;
  }
}
...
HloSharding::IotaTileAssignment& HloSharding::IotaTileAssignment::operator=(
  ...
  ndims_ = other.ndims_;
  reshape_ndims_ = other.reshape_ndims_;
  const int new_size = size_bytes();
  if (new_size > kInlinedBytes) {
    out_of_line_ = malloc(new_size);
  }
  SetPointers();
  std::memcpy(dims_, other.dims_, new_size);
  return *this;
}
...
HloSharding::IotaTileAssignment& HloSharding::IotaTileAssignment::operator=(
  ...
  swap(reshape_ndims_, other.reshape_ndims_);
  if (IsInlined()) {
    auto* old_out_of_line = out_of_line_;
    std::memcpy(dims_, other.dims_, size_bytes());
    if (!other.IsInlined()) {
      other.out_of_line_ = old_out_of_line;
    }
  } else {
    swap(out_of_line_, other.out_of_line_);
  }
  SetPointers();
  return *this;
}
...
HloSharding::IotaTileAssignment::IotaTileAssignment(
  ...
  std::memcpy(reshape_dims_, reshape_dims.data(),
              reshape_ndims_ * sizeof(int64_t));
  std::memcpy(transpose_perm_, transpose_perm.data(),
...
HloSharding::IotaTileAssignment::IotaTileAssignment(int ndims,
    ...
    : ndims_(ndims),
      ...
      out_of_line_(MaybeAllocateOutOfLineStorage()) {
  SetPointers();
}

void HloSharding::IotaTileAssignment::SetPointers() {
  dims_ = static_cast<int64_t*>(IsInlined() ? inlined_ : out_of_line_);
  DCHECK(dims_ != nullptr);
  reshape_dims_ = dims_ + ndims_;
  transpose_perm_ = reinterpret_cast<int*>(reshape_dims_ + reshape_ndims_);
}

std::optional<HloSharding::IotaTileAssignment>
HloSharding::IotaTileAssignment::Transpose(absl::Span<const int> perm) const {
  ...
  for (int i = 0; i < ndims_; ++i) {
    const int64_t dim = dims_[i];
    if (dim == 1) {
      one_to_non_one[i] = -1;
      continue;
    }
    if (non_one_dims.size() >= reshape_ndims_ ||
        reshape_dims_[transpose_perm_[non_one_dims.size()]] != dim) {
      is_pure_transpose = false;
    }
    one_to_non_one[i] = non_one_dims.size();
    non_one_dims.push_back(dims_[i]);
  }
  if (is_pure_transpose) {
    ...
    for (int i = 0; i < ndims_; ++i) {
      if (dims_[perm[i]] == 1) continue;
      new_perm.push_back(transpose_perm_[one_to_non_one[perm[i]]]);
    }
...
std::string HloSharding::IotaTileAssignment::ToString() const {
...
int64_t HloSharding::IotaTileAssignment::value_at(
  ...
  absl::InlinedVector<int64_t, kInlinedDims> reshape_index(reshape_ndims_);
  for (int64_t i = reshape_ndims_ - 1; i >= 0; --i) {
    int dim = transpose_perm_[i];
    int dim_size = reshape_dims_[dim];
  ...
  if (needs_merging) {
    ...
    TileAssignment new_tile_assignment = [&] {
      if (tile_assignment.iota_) {
        CHECK_GE(tile_assignment.iota_->ndims(), perm.size());
        auto transposed_iota =
            tile_assignment.iota_->ndims() == perm.size()
                ? tile_assignment.iota_->Transpose(perm)
                : IotaTileAssignment::Create(
                      tile_assignment.iota_->dims().subspan(0, perm.size()),
                      tile_assignment.iota_->reshape_dims(),
                      tile_assignment.iota_->transpose_perm())
                      .Transpose(perm);
        if (transposed_iota) {
          return TileAssignment(std::move(*transposed_iota));
        }
      }
