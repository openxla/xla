...
class DummyTest : public HloHardwareIndependentTestBase {
 public:
...
static void BM_HashStandard(::testing::benchmark::State& state) {
  DummyTest test_base;
  absl::string_view hlo_string = R"(
HloModule module
...
ENTRY entry {
  ...
  auto module_or_status = test_base.ParseModule(hlo_string);
  auto module = std::move(module_or_status).value();
  auto computation = module->entry_computation();
  for (auto s : state) {
    absl::HashOf(*computation);
  }
}
BENCHMARK(BM_HashStandard);

static void BM_HashWithControlEdges(::testing::benchmark::State& state) {
  DummyTest test_base;
  absl::string_view hlo_string = R"(
HloModule module
...
ENTRY entry {
  ...
  add0 = f32[100] add(p0, p1)
  mul0 = f32[100] multiply(p0, add0)
  ROOT div0 = f32[100] divide(p1, mul0), control-predecessors={add0}
})";
  auto module_or_status = test_base.ParseModule(hlo_string);
  auto module = std::move(module_or_status).value();
  auto computation = module->entry_computation();
...
BENCHMARK(BM_HashWithControlEdges);

static void BM_HashWithUnusedInstructions(::testing::benchmark::State& state) {
  DummyTest test_base;
  absl::string_view hlo_string = R"(
HloModule module
...
BENCHMARK(BM_HashWithUnusedInstructions);
