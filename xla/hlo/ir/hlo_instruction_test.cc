...
TEST_F(HloInstructionTest, AreOperandsAndOutputFullyBound_FullyBoundStart) {
  const char* const hlo = R"(
HloModule test
...
ENTRY main {
  ...
  EXPECT_TRUE(Cast<HloAsyncInstruction>(start)
                  ->AreOperandsAndOutputFullyBound({1})
                  .value());
}

TEST_F(HloInstructionTest, AreOperandsAndOutputFullyBound_FullyBoundUpdate) {
  const char* const hlo = R"(
HloModule test
...
ENTRY main {
  ...
  EXPECT_FALSE(start->AreOperandsAndOutputFullyBound({0}).value());
  EXPECT_FALSE(start->AreOperandsAndOutputFullyBound({1}).value());
  EXPECT_FALSE(start->AreOperandsAndOutputFullyBound({0, 0}).value());
  HloAsyncInstruction* update =
      Cast<HloAsyncInstruction>(FindInstruction(module.get(), "update"));
  ASSERT_NE(update, nullptr);
  EXPECT_TRUE(update->AreOperandsAndOutputFullyBound({1}).value());
}

TEST_F(HloInstructionTest, AreOperandsAndOutputFullyBound_PartiallyBoundStart) {
  const char* const hlo = R"(
HloModule test
...
TEST_F(HloInstructionTest, AreOperandsAndOutputFullyBound_FullyBoundDone) {
  const char* const hlo = R"(
HloModule test
...
ENTRY main {
  ...
  EXPECT_TRUE(Cast<HloAsyncInstruction>(done)
                  ->AreOperandsAndOutputFullyBound({1})
                  .value());
}
...
ENTRY main {
  ...
  EXPECT_FALSE(Cast<HloAsyncInstruction>(start)
                   ->AreOperandsAndOutputFullyBound({1, 0})
                   .ok());
}
