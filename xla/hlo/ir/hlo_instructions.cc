...
void HloScanInstruction::PrintExtraAttributesImpl(
  ...
  if (is_associative_ != TRI_STATE_MAYBE) {
    printer.Next([this](Printer* printer) {
...
std::unique_ptr<HloInstruction> HloScanInstruction::CloneWithNewOperandsImpl(
  ...
  return std::make_unique<HloScanInstruction>(
      shape, new_operands.subspan(0, num_carries),
      new_operands.subspan(num_carries, new_operands.size() - num_carries),
      to_apply(), scan_dimension(), is_reverse(), is_associative());
}
