...
TEST_F(MathTest, Digamma) {
  XlaBuilder builder(TestName());
  auto x =
      ConstantR1<float>(&builder, {1.0, 0.5, 1 / 3.0, 0.25, 1 / 6.0, 0.125, 2.0,
                                   3.0, 4.0, 6.0, 8.0, 9.0, 0.0, -1.0});
  Digamma(x);
...
TEST_F(MathTest, PolygammaNegativeInputs) {
  ...
  // catastrophically wrong results because Zeta's Euler-Maclaurin expansion
  // requires q > 0.  The fix applies the reflection formula
  //   psi^(n)(x) = (-1)^n * psi^(n)(1-x) - g_n(x)
  // so that Zeta is only called with a positive argument.
  //
  // Golden values generated by scipy.special.polygamma.
  XlaBuilder builder(TestName());
  // n=1 (trigamma) at several negative non-integer inputs.
  auto n1 = ConstantR1<float>(&builder, {1, 1, 1, 1, 1});
  auto x1 = ConstantR1<float>(&builder, {-0.5f, -3.3f, -8.5f, -9.5f, -16.5f});
  Polygamma(n1, x1);
  std::vector<float> expected_n1 = {8.934802f, 14.81774f, 9.758607f, 9.769687f,
                                    9.810798f};
  // Use a relaxed tolerance: the reflection formula introduces one extra
  // trigonometric evaluation, so float32 accuracy is ~1e-4.
  ErrorSpec tol(/*aabs=*/1e-2, /*arel=*/1e-4);
  ComputeAndCompareR1<float>(&builder, expected_n1, {}, tol);

  // n=2 (tetragamma).
  XlaBuilder builder2(TestName());
  auto n2 = ConstantR1<float>(&builder2, {2, 2});
  auto x2 = ConstantR1<float>(&builder2, {-0.5f, -8.5f});
  Polygamma(n2, x2);
  std::vector<float> expected_n2 = {-0.8287966f, -0.01230785f};
  ComputeAndCompareR1<float>(&builder2, expected_n2, {}, tol);

  // n=3 (pentagamma).
  XlaBuilder builder3(TestName());
  auto n3 = ConstantR1<float>(&builder3, {3, 3});
  auto x3 = ConstantR1<float>(&builder3, {-0.5f, -3.3f});
  Polygamma(n3, x3);
  std::vector<float> expected_n3 = {193.4091f, 768.9857f};
  ComputeAndCompareR1<float>(&builder3, expected_n3, {}, tol);

  // Positive inputs must be unchanged by the fix.
  XlaBuilder builder4(TestName());
  auto n4 = ConstantR1<float>(&builder4, {1, 1, 2});
  auto x4 = ConstantR1<float>(&builder4, {0.5f, 2.0f, 1.5f});
  Polygamma(n4, x4);
  std::vector<float> expected_pos = {4.934802f, 0.6449341f, -0.8287966f};
  ComputeAndCompareR1<float>(&builder4, expected_pos, {}, kErrorSpec);
}
