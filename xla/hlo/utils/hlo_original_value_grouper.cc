...
void HloOriginalValueGrouper::OnOriginalTensorReady(
  ...
  auto attr_it = absl::c_find_if(debug_attributes, [](const auto& attr) {
    return attr.log_mode ==
               HloModule::DebugAttributes::DebugLogMode::kFusionDebugger ||
           attr.log_mode == HloModule::DebugAttributes::DebugLogMode::kDefault;
  });
  if (attr_it == debug_attributes.end()) {
    LOG(INFO) << "callback_id not found in debug_attributes";
    return;
  }
  int64_t current_callback_id = attr_it->callback_id;
  int64_t op_id = attr_it->op_id;
      ...
      for (const auto& [orig_array, attrs_vec] :
        ...
        for (const auto& attr : attrs_vec) {
          if (attr.callback_id == callback_id) {
            ...
            if (scopes_match_suffix(cand_relative_key.scope_instructions,
                                    current_scopes)) {
              max_op_id = std::max(max_op_id, attr.op_id);
              ...
              if (is_addressable &&
                  (skip_recoverability_check_ ||
                   analysis_->IsOriginalAbsoluteTensorKeyRecoverable(
                       new_absolute_key))) {
                group.expected_op_ids.insert(attr.op_id);
              }
            }
          }
        }
    ...
    if (callback_id == current_callback_id) {
      if (op_id >= 0 && op_id < group.literals.size()) {
        LOG(INFO) << "Received op_id: " << op_id
                  << " for current callback_id=" << current_callback_id;
        if (recovered_data != nullptr) {
          group.literals[op_id] = recovered_data;
        }
        group.received_op_ids.insert(op_id);
        group.expected_op_ids.insert(
            op_id);  // Ensure it's expected if received
      }
    }

    // Case 1 handling: if callback_id != current_callback_id and group is
    // incomplete, we skip it by erasing it.
    if (callback_id != current_callback_id &&
