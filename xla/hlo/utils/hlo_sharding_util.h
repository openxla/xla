...
//
// For tiled sharding, the new dimension to its right of the passed down size
// will be of size `original_dim_size / new_dim_size`. The original dimension
// size needs to be divisible by `new_dim_size`.
//
// For named sharding, the dimension sharding at `dimension` is split into two.
...
// Merges the dimension at `dimension` with the dimension at `dimension + 1`.
//
// For tiled sharding, the new dimension at `dimension` will be of size
// `dimensions[dimension]` * `dimensions[dimension+1]`.
//
// For named sharding, the axes of the dimension at `dimension + 1` are appended
// to the axes of the dimension at `dimension`.
