...
class HandlerBase {
 protected:
  ...
  // we also take an optional expected output dim map as an argument, to verify
  // that sharding propagation in fact infers the sharding we expect (and to
  // crash if it doesn't). As we build more confidence in this, we can remove
  ...
  void MaybeAppend(
    ...
    if (auto output_spec = GetShardingFromUser(lhs_spec, rhs_spec);
  ...
  std::optional<HloSharding> GetShardingFromUser(const HloSharding& lhs_spec,
    ...
    auto ins_clone = ins_->Clone();
    auto lhs_clone = lhs_->Clone();
    auto rhs_clone = rhs_->Clone();
