...
def main():
  ...
  if fa_version < (0, 3, 1):
    warnings.warn("functional_algorithm version 0.3.1 or newer is required,"
                  f" got {fa.__version__}")
    return

  output_file = os.path.join(
      os.path.dirname(__file__), "math_impl.h"
  )
  ...
  for xlaname, fname, args in [
    ...
    if func is None:
      ...
      continue
    ctx = fa.Context(paths=[fa.algorithms], enable_alt=True,
