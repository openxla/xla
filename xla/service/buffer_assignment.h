...
class BufferAllocation {
 public:
  ...
  BufferAllocation(Index index, int64_t size, HloValue::Color color)
      : index_(index),
        size_(size),
        color_(color),
        page_id_(0),
  ...
  HloValue::Color color() const { return color_; }

  void set_color(HloValue::Color color) { color_ = color; }
  struct OffsetSize {
    int64_t offset = 0;
    int64_t size = 0;
  };
...
class BufferAssignment {
 public:
  ...
  BufferAllocation* NewEmptyAllocation(int64_t size, HloValue::Color color);
 ...
 private:
  ...
  HloValue::AlignmentFunction color_alignment_;
...
class BufferAssigner {
 public:
  ...
  struct Options {
    ...
    std::function<int64_t(HloValue::Color)> color_memory_limit;
  };
 ...
 private:
  ...
  // color for stability.
  std::vector<HloValue::Color> SortColorsForCanUseAllocation(
  ...
  absl::StatusOr<int64_t> ReuseCompatibleTempHeaps(
      ...
      HloValue::Color buffer_color, BufferAssignment* assignment);
  ...
  absl::flat_hash_map<HloValue::Color, absl::flat_hash_set<const HloValue*>>
  SplitBuffersByColor(
