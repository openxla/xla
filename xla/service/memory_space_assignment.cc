...
Status MemorySpaceAssignment::SlicedCopyAllocation::Process() {
  ...
  // ProcessCopyLikeAllocationUses() inserts bitcasts after copy-like
  // allocations, as needed, to avoid an issue that might otherwise place a
  // bitcast-use in alternate memory. For SlicedCopyAllocations, this can result
  // in the slices being calculated on a bitcast shape change that ends up not
  // being applied until after the slice. To avoid the situation, we apply
  // needed bitcasts before the slice, to make sure the shape used for slice
  // calculations is the same shape upon which we create the slices.
