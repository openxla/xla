...
void AlternateMemoryBestFitHeap::ImportRepackedAllocations() {
  ...
  for (RepackAllocationBlock& allocation_block : repack_allocation_blocks_) {
    ...
    allocation_block.allocation->replace_offset(allocation_block.offset);
...
void MemorySpaceAssignment::Allocation::replace_offset(int64_t offset) {
...
void MemorySpaceAssignment::SlicedCopyAllocation::replace_offset(
    int64_t offset) {
  int64_t diff = chunk().offset - offset;
  Allocation::replace_offset(offset);
  for (SliceDetail& slice_detail : slice_details_sorted_by_start_time_) {
