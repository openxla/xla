...
inline bool IsOneDnnConvolutionInstr(HloInstruction* instr) {
...
TransformationType GetTransformationConfig(
  ...
  return std::visit(
      [&](auto&& config) -> TransformationType {
        ...
        return PrimitiveTrait<T::config_val, TransformationType>::
            GetTransformationConfig(
                GetKernelConfig<T::config_val>(backend_config));
      },
...
OptimizationConfigPointer GetOptimizationConfig(
    absl::StatusOr<BackendConfig>* backend_config) {
  return GetTransformationConfig<OptimizationConfigPointer>(backend_config);
}
 ...
 public:
  ...
  absl::Status HandleAdd(HloInstruction* instr) override {
    ...
    if (Match(instr, pattern)) {
      ...
      auto optimization_config = GetOptimizationConfig(&backend_config);
  ...
  absl::Status HandleMaximum(HloInstruction* instr) override {
    ...
    if (Match(instr,
              m::MaximumAnyOrder(ElementwiseSafeIntermediates(
                                     ...
                                     .WithOneUser(),
                                 BcastConstScalar(0)))) {
      return FuseActivation(contraction, OneDnnFusionConfig::RELU, instr,
                            intermediate_instr, optional_bitcast);
    }
    return absl::OkStatus();
  }
  ...
  absl::Status HandleSelect(HloInstruction* instr) override {
    ...
    if (ELUActivation(instr, &src)) {
      if (Match(src, ElementwiseSafeIntermediates(
                         &intermediate_instr, &optional_bitcast,
                         OneDnnFusibleInstr(&contraction)))) {
        return FuseActivation(contraction, OneDnnFusionConfig::ELU, instr,
                              intermediate_instr, optional_bitcast);
      }
    }
    return absl::OkStatus();
  ...
  absl::Status HandleTanh(HloInstruction* instr) override {
    ...
    if (Match(instr, m::Tanh(ElementwiseSafeIntermediates(
                                 ...
                                 OneDnnFusibleInstr(&contraction))
                                 .WithOneUser()))) {
      return FuseActivation(contraction, OneDnnFusionConfig::TANH, instr,
                            intermediate_instr, optional_bitcast);
    }
    return absl::OkStatus();
  }
  ...
  absl::Status HandleClamp(HloInstruction* instr) override {
    ...
    if (Match(instr, m::Clamp(BcastConstScalar(0),
                              ElementwiseSafeIntermediates(
                                  ...
                                  .WithOneUser(),
                              BcastConstScalar(6)))) {
      return FuseActivation(contraction, OneDnnFusionConfig::RELU6, instr,
                            intermediate_instr, optional_bitcast);
    }
    return absl::OkStatus();
  }
  ...
  absl::Status HandleMultiply(HloInstruction* instr) override {
    ...
    if (activation != OneDnnFusionConfig::UNDEFINED) {
      ...
      if (Match(src, ElementwiseSafeIntermediates(
                         &intermediate_instr, &optional_bitcast,
                         OneDnnFusibleInstr(&contraction)))) {
        return FuseActivation(contraction, activation, instr,
                              intermediate_instr, optional_bitcast);
      }
    }
  ...
  absl::Status HandleDivide(HloInstruction* instr) override {
    ...
    if (SigmoidActivation(instr, &src)) {
      if (Match(src, ElementwiseSafeIntermediates(
                         ...
                         OneDnnFusibleInstr(&contraction))
                         .WithOneUser())) {
        return FuseActivation(contraction, OneDnnFusionConfig::SIGMOID, instr,
                              intermediate_instr, optional_bitcast);
      }
    }
    return absl::OkStatus();
  ...
  absl::Status FuseActivation(HloInstruction* contraction,
