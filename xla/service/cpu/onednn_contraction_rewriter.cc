...
// original Dequanitze pattern but are added by the layout assignment pass):
...
class OneDnnContractionRewriteVisitor : public DfsHloRewriteVisitor {
 public:
  ...
  absl::Status HandleAddInternal(HloInstruction* contraction,
    ...
    std::vector<HloInstruction*> new_operands(contraction->operands().size() + 1);
    int idx = 0;
