 ...
 public:
  ...
  absl::Status HandleConvolution(HloInstruction* conv) override {
    ...
    for (auto dim : conv->operand(1)->shape().dimensions()) {
      conv_config->mutable_kernel()->mutable_filter()->add_shape(dim + 1);
    }
...
class OneDnnPostRewriteVisitor : public DfsHloRewriteVisitor {
 public:
  ...
  absl::Status HandleCustomCallInternal(HloInstruction* custom_call) {
    auto scratch_add = AddScratch<config>(custom_call);
    if (scratch_add.ok()) {
    ...
    auto weights_prepack = PrepackWeights<config>(custom_call);
