...
TEST_P(QuantizedConvolutionTest, DequantizeConv2D) {
  ...
  ENTRY quant.convolution.test {
    param_inp = $quant_dtype[1,3,224,224] parameter(0)
    convert.194 = s32[1,3,224,224] convert(param_inp)
    constant.65 = s32[] constant(-4)
    broadcast.1 = s32[1,3,224,224] broadcast(constant.65), dimensions={}
    add = s32[1,3,224,224] add(convert.194, broadcast.1)
    convert.196 = $dtype[1,3,224,224] convert(add)
    constant.48 = $dtype[] constant(0.5)
    broadcast.186 = $dtype[1,3,224,224] broadcast(constant.48), dimensions={}
    multiply.197 = $dtype[1,3,224,224] multiply(convert.196, broadcast.186)
    transpose = $dtype[1,224,224,3] transpose(multiply.197), dimensions={0,2,3,1}
    param_wei = s8[64,3,7,7] parameter(1)
    convert.205 = s32[64,3,7,7] convert(param_wei)
    constant.66 = s32[] constant(0)
    broadcast.3 = s32[64,3,7,7] broadcast(constant.66), dimensions={}
    add.1 = s32[64,3,7,7] add(convert.205, broadcast.3)
    convert.207 = $dtype[64,3,7,7] convert(add.1)
    broadcast.163 = $dtype[64,3,7,7] broadcast(constant.48), dimensions={}
    multiply.208 = $dtype[64,3,7,7] multiply(convert.207, broadcast.163)
    transpose.1 = $dtype[7,7,3,64] transpose(multiply.208), dimensions={2,3,1,0}
  ...
  RunCompareAndMatchOptimizedHlo(module_str, {}, operand0_types, operand1_types,
                                 result_types);
}
...
TEST_P(QuantizedConvolutionTest, DequantizeConv2DBiasRequantize) {
  const char* module_str = R"(
  HloModule quant.convolution.test
  ...
  ENTRY quant.convolution.test {
    ...
    constant.49 = $dtype[] constant(0.1)
    ...
    param_wei.1 = s8[3,3,64,64] parameter(3), parameter_replication={false}
    convert.65 = s32[3,3,64,64] convert(param_wei.1)
