...
ENTRY main {
  p0 = u32[2,2]{0,1} parameter(0)
  ROOT out = u64[2]{0} bitcast-convert(u32[2,2]{0,1} p0), metadata={op_name="test"}
}
)";

  CheckLayoutNormalization(hlo, R"(
// CHECK: u64[2]{0} bitcast-convert({{.*}}), metadata={op_name="test"}
)");
}
...
ENTRY main {
  p0 = u64[2]{0,1} parameter(0)
  ROOT out = u32[2,2]{0,1} bitcast-convert(u64[2]{0} p0), metadata={op_name="test"}
}
)";
