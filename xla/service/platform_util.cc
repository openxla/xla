...
static bool IsInterpreter(const se::Platform* p) {
...
absl::StatusOr<std::vector<se::Platform*>> GetSupportedPlatforms() {
  return se::PlatformManager::PlatformsWithFilter(
      [](const se::Platform* platform) {
        auto sor = Compiler::GetForPlatform(platform);
        if (!sor.ok()) {
          LOG(INFO) << "Platform " << platform->Name() << " present but no "
                    << "XLA compiler available: " << sor.status().message();
        }
        return sor.ok();
      });
}
...
absl::StatusOr<se::Platform*> PlatformUtil::GetDefaultPlatform() {
  if (const char* allow_default = getenv("XLA_ALLOW_GET_DEFAULT_PLATFORM")) {
    if (strcmp(allow_default, "true") != 0) {
  ...
  return InvalidArgument(
      "Plaform must be specified - Multiple platforms found: %s.",
      platforms_string);
}
...
// by XLA.
static absl::Status IsDeviceSupported(se::StreamExecutor* executor) {
...
static absl::StatusOr<se::StreamExecutor*> ExecutorForDevice(
...
static absl::StatusOr<std::vector<int>> GetDeviceOrdinals(
  ...
  if (allowed_devices.has_value()) {
    for (int ordinal : *allowed_devices) {
      TF_RET_CHECK(ordinal >= 0 && ordinal < device_count);
      device_ordinals.push_back(ordinal);
    }
  } else {
...
PlatformUtil::GetStreamExecutors(
  ...
  std::vector<absl::StatusOr<se::StreamExecutor*>> se_sors(
      ...
      absl::UnknownError("StreamExecutor not initialized"));
  {
    ...
    for (int i = 0; i < device_ordinals.size(); ++i) {
      thread_pool.Schedule([platform, &se_sors, &device_ordinals, i]() {
        se_sors[i] = ExecutorForDevice(platform, device_ordinals[i]);
      });
    }
  ...
  for (int i = 0; i < se_sors.size(); ++i) {
    absl::StatusOr<se::StreamExecutor*>& se_sor = se_sors[i];
    if (se_sor.ok()) {
      out.push_back(se_sor.value());
    } else {
