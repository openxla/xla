...
std::vector<int64_t> BufferIntervalTree::MemoryUsedByTime(int64_t start,
                                                          int64_t end) const {
  std::vector<const BufferIntervalTreeNode*> nodes =
      NodesOverlappingInTime(start, end);
  int64_t total_time = end - start + 1;
  std::vector<int64_t> memory_used_by_time(total_time, 0);
  for (const BufferIntervalTreeNode* node : nodes) {
    for (int64_t time = std::max(node->start - start, int64_t{0});
         time <= std::min(node->end - start, end - start); ++time) {
      memory_used_by_time[time] += node->chunk.size;
    }
  }
  return memory_used_by_time;
}
