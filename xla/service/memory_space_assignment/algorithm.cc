  ...
  for (const HloValue* maybe_sliced_value : block_prefetched_values) {
    ...
    if (pinned_allocation_it ==
      ...
      AddRequiredAssignment(defining_position, MemorySpace::kDefault,
                            "block prefetched position", nullptr, false);
    ...
    for (const HloValue* aliased_value : buffer.values()) {
      ...
      for (const HloUse& use : aliased_value->GetUses()) {
        AddRequiredAssignment(use, MemorySpace::kDefault,
                              "block prefetched buffer use", nullptr, false);
      }
      AddRequiredAssignment(aliased_value->defining_position(),
                            MemorySpace::kDefault, "block prefetched buffer",
                            nullptr, false);
      FinalizeValue(aliased_value);
    }
  }
