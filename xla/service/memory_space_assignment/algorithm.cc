...
void FixAllocationSequenceAfterPostAllocationTransformation(
  ...
  VLOG(5) << "Fixing AllocationSequence after post-allocation transformation";
...
absl::StatusOr<HeapSimulator::Result<HloValue>> MsaAlgorithm::Finish() {
  ...
  if (options_.post_allocation_transformation_fn) {
    ...
    VLOG(5) << "Running post allocation transformation on module";
    for (HloComputation* comp : alias_analysis_.dataflow_analysis()
      ...
      for (HloInstruction* instr : comp->MakeInstructionPostOrder()) {
        ...
        // If the any of the operands of the instruction has in-place user, we
    ...
    VLOG(5) << "Post allocation transformation info: \n"
