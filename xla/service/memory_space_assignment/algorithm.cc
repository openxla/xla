    ...
    for (int retry_number = 0; retry_number < options_.max_retries;
      ...
      if (result_is(result, AllocationResult::kFailSyncDataMoveReplacement)) {
        ...
        if (!inefficient_sites.empty()) {
          // Retrying after adding default memory requirements for the
          // inefficient sites only helps if at least one requirement is new:
          // if every flagged site already carries the requirement (with the
          // pending, about to be rolled back ones not counting), the retry
          // runs against exactly the same state, produces the same
          // allocations, and flags the same sites again, looping forever.
          // That arises when the alternate memory participants that make the
          // group look inefficient are colored in alternate memory (for
          // example view sources), which exempts them from the default
          // memory requirement. In that case accept the successful
          // allocation instead of retrying.
          absl::flat_hash_set<std::pair<const HloValue*, int64_t>>
              pending_requirement_keys;
          for (const auto& [pending_value, pending_assignment] :
               pending_required_assignments_) {
            pending_requirement_keys.insert(
                {pending_value, pending_assignment.time});
          }
          auto adds_new_requirement = [&](const HloPositionOrUse& site) {
            const HloValue* value = nullptr;
            int64_t time = 0;
            if (std::holds_alternative<HloPosition>(site)) {
              const HloPosition& position = std::get<HloPosition>(site);
              value = &alias_analysis_.dataflow_analysis().GetUniqueValueAt(
                  position.instruction, position.index);
              time = hlo_live_range_.instruction_schedule().at(
            ...
            return !RequiredMemoryAssignmentAt(value, time).has_value() ||
                   pending_requirement_keys.contains({value, time});
          };
          if (absl::c_any_of(inefficient_sites, adds_new_requirement)) {
            UncommitPendingWork(absl::MakeSpan(proposal.allocation_values));
            for (const HloPositionOrUse& site : inefficient_sites) {
              // To avoid a livelock situation, we commit the required
              // assignments right away. Otherwise, reallocation can find
              // alternate memory allocations at other sites, which can also
              // be inefficient.
              std::visit(
...
bool MsaAlgorithm::AddRequiredAssignment(
  ...
  VLOG(3) << "Adding required assignment for value : " << value->ToShortString()
          << " required assignment: " << required_assignment.ToString();
...
bool MsaAlgorithm::AddRequiredAssignment(
  ...
  return AddRequiredAssignment(value, instruction, memory_space,
                               instruction_time, required_assignment_source,
...
bool MsaAlgorithm::AddRequiredAssignment(
    ...
    RequiredMemoryAssignment::Source required_assignment_source,
    AliasedOffset* offset, bool add_to_pending) {
  return AddRequiredAssignment(position.instruction, position.index,
                               memory_space, required_assignment_source, offset,
                               add_to_pending);
}
...
bool MsaAlgorithm::AddRequiredAssignment(
  ...
  return AddRequiredAssignment(value, use.instruction, memory_space,
                               instruction_time, required_assignment_source,
                               offset, add_to_pending);
}
