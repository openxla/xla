...
absl::Status MsaAlgorithm::ProcessColoredBuffers() {
  for (const auto& buffer_coloring : options_.buffer_colorings) {
    ...
    interval_tree_.Add(time_of_coloring, time_of_coloring, chunk_candidate);
...
AllocationRequest MsaAlgorithm::CreateAllocationRequest(
  ...
  SetAlternateMemoryColoringRequirements(request);
  SetDefaultMemoryColoringRequirements(request);
  return request;
}
...
void MsaAlgorithm::SetAlternateMemoryColoringRequirements(
    AllocationRequest& request) {
  const HloPosition& defining_position =
      request.allocation_value->defining_position();
  int64_t definition_time =
...
void MsaAlgorithm::SetDefaultMemoryColoringRequirements(
    AllocationRequest& request) {
  const HloPosition& defining_position =
      request.allocation_value->defining_position();
  int64_t definition_time =
