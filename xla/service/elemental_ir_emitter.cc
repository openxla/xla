...
absl::StatusOr<llvm::Value*> ElementalIrEmitter::EmitComplexUnaryOp(
  ...
  switch (op->opcode()) {
    ...
    case HloOpcode::kExpm1: {
      ...
      TF_ASSIGN_OR_RETURN(
          auto expm1_a, EmitExpm1(component_type, a));
      auto exp_a = FAdd(expm1_a, one);
      TF_ASSIGN_OR_RETURN(
          auto sin_b, EmitSin(component_type, b));
...
absl::StatusOr<llvm::Value*> ElementalIrEmitter::EmitCosm1(
  ...
  return Select(FCmpOGT(xx, llvm::ConstantFP::get(type, 0.61685)), for_large_x, for_small_x);
}
