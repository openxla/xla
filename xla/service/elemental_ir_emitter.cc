...
absl::StatusOr<llvm::Value*> EmitF16ToF8e(llvm::Value* f16_value,
  ...
  int from_e5_convert_mask;
  int from_e5_convert_shift;
  if constexpr (f8_exponent_bits == 4) {
    // Bits of the F16 representation of the smallest F8E4M3 normal value. 2^-6
    min_normal_value = 0x2400;
    ...
    // Bits of the F16 representation of the smallest F8E3M4 normal value. 2^-2
    min_normal_value = 0x3400;
    // Right shift by two E5 exponent's leftmost bit to convert from E5 to E3.
    // For example, 10011 becomes 00111, another example, 00111 becomes 00011
    // (x & 0b1000'1111'1111'1111) | ((x & 0b0100'0000'0000'0000) >> 2)
    from_e5_convert_mask = 0x8FFF;
    from_e5_convert_shift = 2;
  }
  ...
  // Right shift E5 exponent's leftmost bit
  f16_reduced =
      b->CreateOr(b->CreateAnd(f16_reduced, i16_const(from_e5_convert_mask)),
                  b->CreateLShr(b->CreateAnd(f16_reduced, i16_const(0x4000)),
                                i16_const(from_e5_convert_shift)));
