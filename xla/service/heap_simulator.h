...
class GlobalDecreasingSizeBestFitHeap : public HeapAlgorithm<BufferType> {
 public:
  ...
  // is also free at slice times t0 and t1.
  //
  //    ^
  ...
  class SlicedAllocationFinder {
   public:
    // A function that returns true iff we are permitted to allocate a slice
    // starting at a specified offset.
    using IsAllocationOffsetAllowedFn = absl::AnyInvocable<bool(int64_t) const>;
   ...
   private:
    ...
    IsAllocationOffsetAllowedFn is_allocation_offset_allowed_fn_;
