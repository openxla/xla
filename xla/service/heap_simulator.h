...
class GlobalDecreasingSizeBestFitHeap : public HeapAlgorithm<BufferType> {
 public:
  ...
  // indicating that the BufferInterval should be allocated in slices. (If
  // NumSlices() is 1, the allocation will not be sliced.) This class is solely
  // used as an input to FindChunkCandidates().
  //
  ...
  class SlicedBufferInterval {
   public:
    ...
    // Updates the number of slices, slice sizes, and the min requirements at
    // each slice time. An empty slice_sizes_sorted_by_offset is treated the
    // same as setting the number of slices to 1. Every time Slice() is called
    // with a set of sizes > 1, UpdateSliceStartTimes needs to be called to
    // update the requirements at each slice time.
    //
    // REQUIRES:
    // - sum(slice_sizes_sorted_by_offset) == full_buffer_interval_.size
    void Slice(const std::vector<int64_t>& slice_sizes_sorted_by_offset);

    // REQUIRES:
    // - The SlicedBufferInterval was constructed using CreateMutableInterval.
    // - start_times.size() == NumSlices()
    // - start_times should be set such that it is permissible to start any
    //   slice at any start time.
    ...
    size_t NumSlices() const { return slice_sizes_sorted_by_offset_.size(); }
    const std::vector<int64_t>& SliceSizesSortedByOffset() const;

    // Returns a BufferInterval with the requirements to call MakeFreeChunks at
    // the specified slice time. The requirements are:
   ...
   private:
    explicit SlicedBufferInterval(
        const BufferInterval& const_full_buffer_interval,
        BufferInterval* mutable_full_buffer_interval = nullptr);

    const BufferInterval& const_full_buffer_interval_;
    BufferInterval* mutable_full_buffer_interval_;
    int64_t min_slice_size_;
