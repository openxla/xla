...
void EnsureControlDependency(Decomposed cp) {
  ...
  EXPECT_TRUE(absl::c_contains(cp.send->control_predecessors(), cp.recv))
      << "Send should depend on recv when decoposed";
  EXPECT_TRUE(absl::c_contains(cp.recv_done->control_predecessors(), cp.send))
...
TEST_F(DecomposerTest, OneSendRecvWithNonConflictingAllReduce) {
  ...
  // collective
  // stream attribute.
  EXPECT_EQ(ar->frontend_attributes().map().find(kCollectiveStreamAttrName),
            ar->frontend_attributes().map().end());
}
