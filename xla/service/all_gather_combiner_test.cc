...
ENTRY entry {
  ...
  AllGatherCombiner combine(1024 * 1024, kMaxCombineCount,
                            /*combine_major_most_layout_dim=*/false);
...
TEST_F(AllGatherCombinerTest, CombineAllGatherAlongMajorMostLayoutDim) {
  const char* const hlo_string = R"(
HloModule Module
...
ENTRY entry {
  ...
  param2 = f32[2,2]{0,1} parameter(2)
  param3 = f32[2,2]{0,1} parameter(3)
  param4 = f32[2,2]{1,0} parameter(4)
  allgather0 = f32[8,2]{1,0} all-gather(param0), replica_groups={},
      dimensions={0}
  allgather1 = f32[8,2]{1,0} all-gather(param1), replica_groups={},
      dimensions={0}
  allgather2 = f32[2,8]{0,1} all-gather(param2), replica_groups={},
      dimensions={1}
  allgather3 = f32[2,8]{0,1} all-gather(param3), replica_groups={},
      dimensions={1}
  allgather4 = f32[8,2]{1,0} all-gather(param4), replica_groups={},
      dimensions={0}
  ROOT tuple = (f32[8,2]{1,0}, f32[8,2]{1,0}, f32[2,8]{0,1}, f32[2,8]{0,1},
      f32[8,2]{1,0}) tuple(allgather0, allgather1, allgather2, allgather3,
      allgather4)
}
)";
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
  ...
  AllGatherCombiner combine(1024 * 1024, kMaxCombineCount,
                            /*combine_major_most_layout_dim=*/true);
  ASSERT_EQ(AllGatherCount(*module), 5);
  TF_ASSERT_OK_AND_ASSIGN(bool changed, combine.Run(module.get()));
  EXPECT_TRUE(changed);
  ...
  EXPECT_THAT(module->entry_computation()->root_instruction(),
              op::Tuple(op::GetTupleElement(combined_all_gather, 0),
                        ...
                        op::GetTupleElement(combined_all_gather, 3),
  ...
  ASSERT_EQ(HloAllGatherInstruction::kMajorMostLayoutDimension,
            all_gathers.front()->all_gather_dimension());
}

TEST_F(AllGatherCombinerTest, DontCombineAllGatherAlongMinorMostLayoutDim) {
  const char* const hlo_string = R"(
HloModule Module

ENTRY entry {
  param0 = f32[2,2]{0,1} parameter(0)
  param1 = f32[2,2]{0,1} parameter(1)
  param2 = f32[2,2]{1,0} parameter(2)
  param3 = f32[2,2]{1,0} parameter(3)
  param4 = f32[2,2]{0,1} parameter(4)
  allgather0 = f32[8,2]{0,1} all-gather(param0), replica_groups={},
      dimensions={0}
  allgather1 = f32[8,2]{0,1} all-gather(param1), replica_groups={},
      dimensions={0}
  allgather2 = f32[2,8]{1,0} all-gather(param2), replica_groups={},
      dimensions={1}
  allgather3 = f32[2,8]{1,0} all-gather(param3), replica_groups={},
      dimensions={1}
  allgather4 = f32[8,2]{0,1} all-gather(param4), replica_groups={},
      dimensions={0}
  ROOT tuple = (f32[8,2]{0,1}, f32[8,2]{0,1}, f32[2,8]{1,0}, f32[2,8]{1,0},
      f32[8,2]{0,1}) tuple(allgather0, allgather1, allgather2, allgather3,
      allgather4)
}
)";
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
