...
ENTRY %comp {
  ...
  broadcast.1 = f32[512] broadcast(constant)
  broadcast.2 = f32[1024] broadcast(constant)
  all-reduce-start.1 = (f32[256], f32[512]) all-reduce-start(broadcast, broadcast.1), to_apply=add
  foo = f64[] constant(42)
  bar = add(p0, foo)
  baz = f64[] constant(42)
  qux = add(bar, p0)
  quux = add(qux, p0)
  all-reduce-done.2 = (f32[256], f32[512]) all-reduce-done(all-reduce-start.1)
...
ENTRY %comp {
  ...
  broadcast.2 = f32[1024] broadcast(constant)
  all-reduce-start.1 = (f32[256], f32[512]) all-reduce-start(broadcast, broadcast.1), to_apply=add
  all-reduce-done.3 = (f32[256], f32[512]) all-reduce-done(all-reduce-start.1)
...
ENTRY %comp {
  ...
  bar = add(p0, constant)
  baz = add(p0, constant)
  all-reduce-start.1 = (f32[256], f32[512]) all-reduce-start(broadcast, broadcast.1), to_apply=add, control-predecessors={bar}
  all-reduce-done.2 = (f32[256], f32[512]) all-reduce-done(all-reduce-start.1), control-predecessors={baz}
  get-tuple-element = f32[256] get-tuple-element(all-reduce-done.2), index=0
  get-tuple-element.1 = f32[512] get-tuple-element(all-reduce-done.2), index=1
  ROOT tuple = (f32[256], f32[512]) tuple(get-tuple-element, get-tuple-element.1)
})";
