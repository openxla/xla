...
TEST_F(HloVerifierTestLayoutSensitive,
  ...
  ENTRY main {
    p = f32[32,32]{0,1} parameter(0)
    async-start = ((f32[32,32]{0,1}), (f32[32,32]{0,1}), f32[32,32]{0,1}) async-start(p),
        calls=async_computation
    async-update = ((f32[32,32]{0,1}), (f32[32,32]{0,1}), f32[32,32]{1,0}) async-update(async-start),
...
TEST_F(HloVerifierTest, VerifyAsyncUpdateAliasConfigValid) {
  ...
  ENTRY main {
    ...
    async-start = ((f32[32,32]), (f32[32,32]), s32[]) async-start(p),
        calls=async_computation
    async-update = ((f32[32,32]), (f32[32,32]), s32[]) async-update(async-start),
        output_to_operand_aliasing={{2}: (0, {2})}
...
TEST_F(HloVerifierTest, VerifyAsyncUpdateAliasConfigInvalidOperandNumber) {
  ...
  EXPECT_THAT(
      status.message(),
      HasSubstr("Invaid operand number in async-update aliasing config."));
}
