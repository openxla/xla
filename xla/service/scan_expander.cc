...
std::vector<HloInstruction*> SliceInputs(
  ...
  for (auto* input : current_inputs) {
    ...
    start_indices.reserve(input_shape.dimensions().size());
    for (int64_t d = 0; d < input_shape.dimensions().size(); ++d) {
      if (d == scan_dim) {
        start_indices.push_back(access_index);
      } else {
        start_indices.push_back(builder->AddInstruction(
    ...
    reshaped_dims.reserve(input_shape.dimensions().size() - 1);
    for (int64_t d = 0; d < input_shape.dimensions().size(); ++d) {
      if (d != scan_dim) {
        reshaped_dims.push_back(input_shape.dimensions(d));
      }
    }
    sliced_inputs.push_back(
        builder->AddInstruction(HloInstruction::CreateReshape(
            ShapeUtil::MakeShape(input_shape.element_type(), reshaped_dims),
            slice)));
  }
...
std::vector<HloInstruction*> UpdateOutputArrays(
  ...
  for (int64_t i = 0; i < current_outputs.size(); ++i) {
    ...
    std::vector<HloInstruction*> start_indices;
    start_indices.reserve(output_array_shape.dimensions().size());
    for (int64_t d = 0; d < output_array_shape.dimensions().size(); ++d) {
      if (d == scan_dim) {
        start_indices.push_back(access_index);
      } else {
        start_indices.push_back(builder->AddInstruction(
