...
static absl::StatusOr<bool> TryRemoveDeadWhileParamsImpl(
  ...
  // those candidate indices, numbered by candidate_bit; every other index
  // keeps -1 there.
  ...
  // body's local ids first, then the condition's. The slots map the
  // instructions to a dense index range, so the per instruction state below
  // lives in flat arrays instead of hash maps keyed by instruction. Local ids
  // stay stable during the analysis: nothing mutates the graph before
  ...
  // One post order pass per computation: operands come before their users, so
  // an instruction's dependencies are the union of its operands' by the time
  // it is visited. A get-tuple-element of the parameter seeds its own bit. A
  // side effecting instruction and the condition root are sinks: the inputs
  // they depend on must stay.
    ...
    comp->ForEachInstructionPostOrder([&](HloInstruction* inst) {
      ...
      if (inst->opcode() == HloOpcode::kGetTupleElement &&
        ...
        const HloInstruction* output =
            while_body_root->operand(inst->tuple_index());
        if (output != inst) {
          sets.Merge(slot_of(output), slot);
        }
      } else {
        for (const HloInstruction* operand : inst->operands()) {
          if (is_tracked(operand)) {
            sets.Merge(slot_of(operand), slot);
            input_deps.Or(slot, slot_of(operand));
          }
        }
      }
      if (facts.HasSideEffect(inst) || inst == while_cond->root_instruction()) {
        input_deps.Or(side_effecting_row, slot);
      }
    });
  }
  // Case 1: find inputs that can be removed because they don't affect others.
  for (int64_t i = 0; i < tuple_size; ++i) {
    const HloInstruction* output = while_body_root->operand(i);
    if (candidate_bit[i] >= 0) {
      // Ignore self-updates.
      input_deps.OrIgnoringBit(affecting_others_row, slot_of(output),
                               candidate_bit[i]);
    } else {
  ...
  // Case 2: a connected group of indices can be removed when none of its
  // indices blocks it. An index blocks its group when it is not a candidate,
  // that is used after the loop and not passed through, or when a side effect
  // or the condition depends on it. The group of an index is the set of its
  // output; the get-tuple-elements of the index were merged into that set.
  auto set_of = [&](int64_t index) {
    return sets.Find(slot_of(while_body_root->operand(index)));
  };
  InlinedBitSet<> blocked_sets(num_slots);
  for (int64_t i = 0; i < tuple_size; ++i) {
    if (candidate_bit[i] < 0 ||
        input_deps.Test(side_effecting_row, candidate_bit[i])) {
      blocked_sets.Set(set_of(i));
    }
  }
  for (int64_t i = 0; i < tuple_size; ++i) {
    if (!blocked_sets.Test(set_of(i))) {
