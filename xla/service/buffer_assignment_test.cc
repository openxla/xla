...
TEST_F(BufferAssignmentTest, MemoryUsageReportGroupedByMemorySpace) {
  // Test that MemoryUsageReport and MemoryUsageReportToProto correctly group
  ...
  MemoryUsageReportProto proto_report = buffers->MemoryUsageReportToProto();
  ...
  // Verify allocation entries have hlo_name populated.
  for (const auto& entry : memory_space.allocation_entries()) {
    EXPECT_FALSE(entry.hlo_name().empty());
  }
}
