...
std::optional<MovableAllReduceContext> MatchDynamicUpdateSliceContext(
  ...
  const int64_t kAllReduceMaxSizeBytes = 1048576;
  ...
  // Process dependency chain (allow one user only, the loop output).
  const HloInstruction* hlo = all_reduce;
  while (hlo->user_count() == 1) {
    const HloInstruction* prev = hlo;
    hlo = hlo->users()[0];

    switch (hlo->opcode()) {
      case HloOpcode::kReshape:
        if (!context.update_slice_offsets.empty()) {
          VLOG(5) << "Reshape after DUS is not supported.";
          return std::nullopt;
        }
      ...
      case HloOpcode::kDynamicUpdateSlice: {
        auto dus = Cast<HloDynamicUpdateSliceInstruction>(hlo);
        if (dus->update() != prev) {
          VLOG(5) << "DUS update must contain the all-reduce result.";
          return std::nullopt;
        }
        int size = hlo->shape().dimensions_size();
      ...
      case HloOpcode::kTuple:
        if (hlo->IsRoot()) {
          ...
          if (!Match(prev, m::DynamicUpdateSlice(&dus)) ||
              !Match(dus->operand(0),
                     m::GetTupleElement(&gte, m::Parameter(0))) ||
              !dus_unique_and_valid()) {
            VLOG(5) << "DUS must originate from the loop input and be stored "
                       "in the loop output at the same tuple index.";
            return std::nullopt;
          }
          if (gte->user_count() != 1 ||
              hlo_query::CountGteInstructionsWithIndex(
        ...
        [[fallthrough]];
      default:
        VLOG(5) << "Unsupported instruction: " << hlo->ToString();
        return std::nullopt;
    }
  }
  VLOG(5) << "Multiple users: " << hlo->ToString();
  return std::nullopt;
}
...
MovableAllReduceContext IsAllReduceMovable(
  ...
  if (auto update_slice_context =
          MatchDynamicUpdateSliceContext(all_reduce, while_instructions);
