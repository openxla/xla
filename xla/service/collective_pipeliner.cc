...
absl::Status UpdateSendRecvValidation(
  ...
  VLOG(3) << "Collective permute with _xla_send_recv_validation: " << instruction->ToString();
  TF_ASSIGN_OR_RETURN(
  ...
  VLOG(3) << "Updated collective_permute with _xla_send_recv_validation: " << instruction->ToString();
  return absl::OkStatus();
}
...
absl::Status TransformLoopForward(
  ...
  for (auto* instr : while_body->MakeInstructionPostOrder()) {
    ...
    TF_RETURN_IF_ERROR(UpdateSendRecvValidation(
        cloned_instr, true,
        CollectivePipeliner::PipeliningDirection::kForward, loop_analysis));
