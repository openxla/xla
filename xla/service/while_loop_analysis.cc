...
optional<int64_t> MatchTrivialLoopTripCount(const HloInstruction* while_op,
  ...
  // Check that `i` goes as `i += k` in the while body.
  auto* while_body = while_op->while_body();
  ...
  const HloInstruction* trip_count_increase_step_instr;
  ...
  if (!Match(while_body_indvar_update,
    ...
    if (trip_count_increase_step_instr == nullptr ||
        !trip_count_increase_step_instr->IsConstant() ||
        !ShapeUtil::IsEffectiveScalar(
            trip_count_increase_step_instr->shape())) {
      VLOG(2)
          << "Pattern-match failed: induction variable does not go as i += k: "
  ...
  if (Match(while_cond_root,
    ...
    if (trips) {
      int64_t remainder = std::remainder(*trips, trip_count_step);
      int64_t div = std::floor(*trips / trip_count_step);
      if (remainder == 0) {
  ...
  if (Match(while_cond_root,
    ...
    trips = CheckedAdd((int64_t)std::floor(*trips / trip_count_step), 1);
    if (!trips) {
      VLOG(2) << "Pattern-match failed: Trip count exceeds INT64_MAX";
      return nullopt;
    }
