...
TEST_F(CallGraphTest, SingleComputationNearestCommonAncestors) {
  ...
  EXPECT_EQ(call_graph->NearestCommonAncestors(
...
TEST_F(CallGraphTest, MultipleComputationNearestCommonAncestors) {
  ...
  HloComputation* cond_computation =
      module->AddEmbeddedComputation(MakeConditionComputation());
  HloInstruction* cond_root = cond_computation->root_instruction();
  HloComputation* c_computation =
      module->AddEmbeddedComputation(MakeScalarComputation());
  HloInstruction* c_root = c_computation->root_instruction();
  HloComputation* b_computation = module->AddEmbeddedComputation(
      MakeMappingComputation(c_computation, /*callsites=*/1));
  HloInstruction* b_map = b_computation->root_instruction();
  ...
  EXPECT_EQ(call_graph->NearestCommonAncestors(
                std::vector<const HloInstruction*>({a_call, a_call})),
            absl::flat_hash_set<const HloInstruction*>({a_call}));

  EXPECT_EQ(
      call_graph->NearestCommonAncestors(std::vector<const HloInstruction*>(
          {b_map, c_computation->root_instruction()})),
      absl::flat_hash_set<const HloInstruction*>({b_map}));

  EXPECT_EQ(call_graph->NearestCommonAncestors(
                std::vector<const HloInstruction*>({b_map, entry_while})),
            absl::flat_hash_set<const HloInstruction*>({entry_while}));

  EXPECT_EQ(call_graph->NearestCommonAncestors(
                std::vector<const HloInstruction*>({b_map, a_call})),
            absl::flat_hash_set<const HloInstruction*>({a_while}));

  EXPECT_EQ(call_graph->NearestCommonAncestors(
                std::vector<const HloInstruction*>({a_while, a_call})),
            absl::flat_hash_set<const HloInstruction*>({a_while}));

  EXPECT_EQ(call_graph->NearestCommonAncestors(
                std::vector<const HloInstruction*>({a_while, b_map})),
            absl::flat_hash_set<const HloInstruction*>({a_while}));

  EXPECT_EQ(call_graph->NearestCommonAncestors(
                std::vector<const HloInstruction*>({c_root, cond_root})),
            absl::flat_hash_set<const HloInstruction*>({a_while}));
}
