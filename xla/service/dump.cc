...
class HloModuleDump {
 public:
  HloModuleDump(HloModuleDump&&) = default;
  HloModuleDump& operator=(HloModuleDump&&) = default;
  ~HloModuleDump() = default;

  static std::optional<HloModuleDump> Create(
      const HloModule& module, const BufferAssignment* buffer_assn,
      absl::string_view prefix, absl::string_view suffix,
      const DumpOptions& opts, const DebugOptions& debug_options) {
    if (!opts.should_dump_module(module.name())) {
      return std::nullopt;
    }

    HloModuleDump dump(opts);
    dump.module = &module;
    dump.buffer_assn = buffer_assn;
    dump.prefix = std::string(prefix);
    dump.suffix = std::string(suffix);
    dump.debug_options = debug_options;

    dump.filenames_ = GetDumpFilenames(module, buffer_assn, prefix, suffix,
                                       opts, debug_options);
  ...
  static std::optional<HloModuleDump> Create(
      std::unique_ptr<HloModule> cloned_module, absl::string_view prefix,
      absl::string_view suffix, const DumpOptions& opts,
      const DebugOptions& debug_options) {
    if (!opts.should_dump_module(cloned_module->name())) {
      return std::nullopt;
    }

    HloModuleDump dump(opts);
    dump.filenames_ = GetDumpFilenames(*cloned_module, /*buffer_assn=*/nullptr,
                                       prefix, suffix, opts, debug_options);
    if (opts.dump_max_hlo_modules > 0) {
      absl::MutexLock lock(&mu);
      dumped_module_ids.insert(cloned_module->unique_id());
    }
    dump.cloned_module = std::move(cloned_module);
    dump.prefix = std::string(prefix);
    dump.suffix = std::string(suffix);
    dump.debug_options = debug_options;
  ...
  // Sync dump data
  const HloModule* module = nullptr;
  const BufferAssignment* buffer_assn = nullptr;

  // Async dump data
  std::unique_ptr<HloModule> cloned_module;
 ...
 private:
  HloModuleDump(DumpOptions opts) : opts(std::move(opts)) {}
  std::vector<std::string> filenames_;
};

std::vector<std::string> HloModuleDump::Write() && {
  std::vector<std::string> written_paths;

  const HloModule* module = cloned_module ? cloned_module.get() : this->module;

  if (module == nullptr) {
    LOG(ERROR) << "No HloModule available for dumping.";
    return {};
  }
  ...
  auto init_module_proto = [&]() {
    ...
    module_proto = buffer_assn ? MakeHloProto(*module, *buffer_assn)
                               : MakeHloProto(*module);
...
static std::vector<std::string> DumpHloModuleImpl(
  ...
  auto dump = HloModuleDump::Create(module, buffer_assn, prefix, suffix, opts,
                                    debug_options);
  if (!dump.has_value()) {
    return {};
  }
  std::vector<std::string> dumped_file_paths = std::move(*dump).Write();
...
static std::optional<HloModuleDump> PrepareDumpHloModuleBetweenPassesIfEnabled(
  ...
  return HloModuleDump::Create(module, /*buffer_assn=*/nullptr, timestamp,
...
std::vector<std::string> DumpHloModuleBetweenPassesIfEnabled(
  ...
  if (debug_options.xla_async_hlo_dump()) {
    ...
    auto dump = HloModuleDump::Create(std::move(clone), timestamp,
    ...
    auto shared_dump = std::make_shared<HloModuleDump>(std::move(*dump));
    GetDumpThreadPool(debug_options)->Schedule([shared_dump]() {
      std::move(*shared_dump).Write();
      DecrementPendingAsyncDumps();
    });
    return filenames;
  } else {
    auto dump = PrepareDumpHloModuleBetweenPassesIfEnabled(
        pipeline_name, before_pass_name, after_pass_name, module, step_number);
    if (!dump.has_value()) {
      return {};
    }
    return std::move(*dump).Write();
  }
}
