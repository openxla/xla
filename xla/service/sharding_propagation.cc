...
bool RefineManualAutoShardingFromAuto(
  ...
  auto man_tiling = manual_sharding->tile_assignment().shared_array_clone();
  ...
  for (HloComputation* computation : module->computations(execution_threads)) {
    ...
    for (auto it = instructions.rbegin(); it != instructions.rend(); ++it) {
      ...
      TF_RETURN_IF_ERROR(sharding_op_util::ParseAttributes(
          Cast<HloCustomCallInstruction>(instruction)->opaque(), &unspec_dims));
...
std::optional<HloSharding> ShardingPropagation::GetShardingFromUser(
  ...
  switch (user.opcode()) {
    ...
    case HloOpcode::kConcatenate: {
      ...
      auto new_tile_assignment = std::make_shared<const Array<int64_t>>(
