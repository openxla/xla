 ...
 public:
  ...
  class ExecutionScope {
   ...
   private:
    ...
    // `persistent_alloc_indices` (allocations at stable addresses): the module's
    // persistent set when this scope drives VMM VA-remapping, otherwise nullopt
    // so the command buffer treats every allocation as dynamic (legacy default).
    std::optional<absl::Span<const BufferAllocation::Index>>
