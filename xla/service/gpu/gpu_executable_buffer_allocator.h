...
// hardware.
//
// NOTE (merge with upstream main-mirror): upstream re-architected this allocator
// so command-buffer temp/output allocations are placed in-place inside the VA
// reservation (see Remapping/MemoryReservationAlias below), which subsumes the
// per-step temp/output remap that the ROCm skip-remap path targeted. These
// helpers remain wired to the `xla_gpu_experimental_command_buffer_vmm_*` flags
// and unit tests; the skip-remap / copy-into-shadow *execution behavior* is not
// yet re-ported onto upstream's map/unmap-alias lifecycle. See
// VMM_LLAMA_OPT_WORKLOG.md.
