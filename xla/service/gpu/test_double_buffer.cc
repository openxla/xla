...
StatusOr<bool> TestDoubleBuffer::Run(
    HloModule *module,
  ...
  for (HloInstruction* while_instr : while_instrs) {
    ...
    if(!config.has_known_trip_count()) {
      VLOG(2) << while_instr->ToString() << " doesn't have exact trip count, skipping double buffering for now";
      changed |= changed;
      continue;
    }
    int64_t exact_trip_count = config.known_trip_count().n();
    if(exact_trip_count % 2) {
      VLOG(2) << "SKipping loops with odd trip count.";
      continue;
    }
    VLOG(2) << "Processing while loop " << while_instr->ToString() << " with trip count: " << exact_trip_count;
    ...
    std::unordered_map<HloInstruction*, std::vector<LoopInputConsumerInfo>> loop_input_consumers;
    ...
    for (auto user : input_parameter->users()) {
      ...
      for(auto consumer : user->users()) {
        ...
        if(consumer != while_body->root_instruction()) {
          ...
          loop_input_consumers[const_cast<HloInstruction*>(consumer)].push_back(input_consumer_info);
        }
      }
    }
    ...
    absl::c_copy(while_body->instructions(), std::back_inserter(all_old_instructions));
    ...
    for(auto old_instr : all_old_instructions) {
      // This is to track mappings of old->new channel id for async collecitves wrapped in the form of HloAsyncInstruction,
      // the start and done need to have the same unique channel id.
      if(old_instr == input_parameter || old_instr == while_body->root_instruction() || old_instr->IsUserOf(input_parameter)) {
        continue;
      }
      VLOG(2) << "Cloning instruction " << old_instr->ToString();
      HloInstruction* new_instr = while_body->AddInstruction(old_instr->Clone(/*suffix=*/"double_buffer_clone"));
      // If an elementwise instruction with constant operand is present, we won't inject control dependency at the end
      // to allow more constant folding opportunities.
      if(old_instr->IsElementwiseBinary() && old_instr->HasConstantOperand()) {
        skip_control_dep_injection.insert(old_instr);
      }
      if(HloAsyncInstruction::ClassOf(new_instr) && hlo_query::IsCollectiveCommunicationOp(DynCast<HloAsyncInstruction>(new_instr)->async_wrapped_instruction()->opcode())) {
          ...
          if(old_to_new_channel_id_map.find(old_channel_id) == old_to_new_channel_id_map.end()) {
            ...
            VLOG(2) << "Found existing channel id for old channel id " << old_channel_id;
      ...
      } else if (hlo_query::IsCollectiveCommunicationOp(new_instr->opcode()) || hlo_query::IsAsyncCollectiveStartOp(new_instr->opcode())){
    ...
    for(auto old_instr : all_old_instructions) {
      if(old_to_new_map.find(old_instr) != old_to_new_map.end()) {
        ...
        VLOG(2) << "Processing control predecessors for " << new_instr->ToString();
        if(old_instr->control_predecessors().size() > 0) {
          ...
          for(auto new_pred : new_control_pred) {
            ...
            VLOG(2) << "Adding " <<  new_pred->ToString()  << " to control dependency of " << new_instr->ToString();
          }
        }
      }
    }
    for(auto instr : all_new_instructions) {
      for(int64_t op_index = 0; op_index < instr->operand_count(); op_index++) {
        VLOG(2) << "Processing operand " << op_index << " of " << instr->ToString();
        ...
        VLOG(2) << "Changing operand " <<  op_index  << " of instruction " << instr->ToString() << " from "
        << operand->ToString() << " to " <<  old_to_new_map[operand]->ToString();
        TF_RETURN_IF_ERROR(instr->ReplaceOperandWith(op_index, old_to_new_map[operand]));
      }
    }
    ...
    for(auto& instr_index_pair : loop_input_consumers) {
      VLOG(2) << "Processing boundary instruction: " << instr_index_pair.first->ToString();
      ...
      for(auto consumer_info : instr_index_pair.second) {
        ...
        VLOG(2) << "Changing boundary operand " <<  op_index  << " of instruction " << instr->ToString()
        << " from " << instr->operand(op_index)->ToString() << " to " << new_operand->ToString();
        if(instr->operand(op_index) != new_operand) {
          TF_RETURN_IF_ERROR(instr->ReplaceOperandWith(op_index, new_operand));
        }
      }
      if(skip_control_dep_injection.find(instr_index_pair.first) == skip_control_dep_injection.end() && !IsCollective(instr)) {
    ...
    TF_RETURN_IF_ERROR(while_body->ReplaceWithNewInstruction(while_body->root_instruction(), HloInstruction::CreateTuple(new_loop_roots)));
  ...
  VLOG(2) << "TestDoubleBuffer output: " << module->ToString();
