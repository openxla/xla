...
class MatMulEmitterHelper {
 public:
  ...
  absl::StatusOr<Value> EmitTensorPointer(
    ...
    VLOG(1) << "EmitTensorPointer: " << hlo->ToString();
    ...
    VLOG(1) << "bases size" << bases.size();
    VLOG(1) << "analysis: " << analysis_.ToString();

    VLOG(1) << "dims_lhs_contracting_idx_" << dims_.lhs_contracting_dim_idx;
    VLOG(1) << "dims_rhs_contracting_idx_" << dims_.rhs_contracting_dim_idx;
    ...
    auto add_dim = [&](const DimProperties& properties) {
      ...
      VLOG(1) << "properties.index: " << properties.index;
      Value pid_offset =
      ...
      if (properties.index == concat_dim_idx) {
        ...
        VLOG(1) << "majormost_dim_idx: " << majormost_idx;
        ...
        // I have set the guarantee that I am only going to use the
        // first (majormost) offset and all others must be 0 (because they would
        // lead to noncontiguous strides in the tensor.)
        ...
        for (int i = 0; i < hlo->shape().dimensions().size() - 1; ++i) {
          VLOG(1) << hlo->shape().dimensions_minor(i);
          majormost_idx_stride *= hlo->shape().dimensions_minor(i);
        }
        VLOG(1) << specs.back()->at(0).ToString();
        majormost_idx_stride =
            majormost_idx_stride / specs.back()->at(0).stride;
        VLOG(1) << "offset_size: " << majormost_idx_stride;
        Value offest_size_value = Cst32(majormost_idx_stride);
        Value tensor_dim_offset =
            b_.create<ma::MulIOp>(majormost_offset, offest_size_value);
...
SmallVector<Value> GetArguments(mlir::triton::FuncOp fn,
  ...
  if (input.opcode() == HloOpcode::kParameter) {
    ...
    // As defined in gemm_rewriter_triton, all inputs of concatenate and
    // dynamic slice are parameters.
