...
// ROCm-only gate for the per-slot skip-remap optimization. Other platforms keep
// their existing behavior unchanged. The flag
// (xla_gpu_experimental_command_buffer_vmm_skip_remap) lets ROCm fall back to the
// legacy path for A/B measurement; it has no effect on other platforms.
...
// Size threshold (bytes) for the ROCm copy-into-shadow path. 0 (default)
// disables the path entirely, so other platforms and the default ROCm behavior
// are unchanged. A negative flag value is clamped to 0.
