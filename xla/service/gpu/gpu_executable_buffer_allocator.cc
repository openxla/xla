...
// (xla_gpu_experimental_command_buffer_vmm_skip_remap) lets ROCm fall back to the legacy
...
GpuExecutableBufferAllocator::ExecutionScope::GetPersistentAllocIndices() const {
  // Only when this scope drives VMM VA-remapping are the module's command-buffer
  // allocations at stable (reservation-backed or shadow) addresses; report them
  // as persistent so the command buffer is traced once and never updated. When
  // inactive, return nullopt so the command buffer treats every allocation as
  // dynamic (the pre-upstream default behavior for this path).
...
GpuExecutableBufferAllocator::ExecutionScope::ExecuteWithVaRemapping(
  ...
  RETURN_IF_ERROR(EnsureVaReservation(owning_buffer_allocations, executor,
                                      vmm_allocator, granularity, copy_threshold,
                                      device_ordinal));
  ...
  RETURN_IF_ERROR(execute(remapped_buffer_allocations, persistent_alloc_indices));
...
GpuExecutableBufferAllocator::ExecutionScope::GetOrCreateSmallShadow(
  ...
  if (it == va_ranges_->small_shadow.end()) {
    ...
    if (i >= static_cast<BufferAllocation::Index>(owner_->allocations_.size())) {
      return Internal(
...
absl::Status GpuExecutableBufferAllocator::ExecutionScope::EstablishMapping(
  ...
  if (can_skip_remap) {
    ...
    // any prior GPU use of the VA range, drop the old mapping, then map the full
    // contiguous range in a single MapTo call.
