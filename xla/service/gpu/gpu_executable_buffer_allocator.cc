...
absl::StatusOr<uint64_t> RoundUpToGranularity(uint64_t size,
                                              uint64_t granularity) {
  if (granularity == 0) {
    return size;
  }
  uint64_t remainder = size % granularity;
  if (remainder == 0) {
    return size;
  }
  uint64_t padding = granularity - remainder;
  if (size > std::numeric_limits<uint64_t>::max() - padding) {
    return ResourceExhausted(
        "Buffer size %u overflows while rounding to VMM granularity %u", size,
        granularity);
  }
  return size + padding;
}
...
absl::Status GpuExecutableBufferAllocator::ExecutionScope::PrepareReservation(
  ...
  for (BufferAllocation::Index idx : remapping_->reservation_alloc_indices) {
    ...
    if (remapping_->total_size >
        std::numeric_limits<uint64_t>::max() - mapping_size) {
      return ResourceExhausted(
          "Command buffer VA reservation size overflows for module %s",
          owner_->module_name_);
    }
...
GpuExecutableBufferAllocator::ExecutionScope::BuildExecutionBufferAllocations(
  ...
  // DeviceAddressVmmAllocator currently supports only one reservation alias
  // per allocator address. Reject duplicate parameter buffers before creating
  // any aliases so activation cannot fail after partially mutating mappings.
  std::vector<std::pair<BufferAllocation::Index, se::DeviceAddressBase>>
      parameter_sources;
  for (BufferAllocation::Index index : candidates) {
    const BufferAllocation& allocation = *owner_->allocations_[index];
    if (!allocation.is_entry_computation_parameter() ||
        copy_protected_alloc_indices_.contains(index)) {
      continue;
    }
    se::DeviceAddressBase source =
        owning_buffer_allocations.GetDeviceAddress(index);
    for (const auto& [other_index, other_source] : parameter_sources) {
      if (source.IsSameAs(other_source)) {
        return Unimplemented(
            "SKIP_PROFILED does not support parameter allocations %d and %d "
            "using the same input buffer in module %s",
            other_index, index, owner_->module_name_);
      }
    }
    parameter_sources.emplace_back(index, source);
  }
...
GpuExecutableBufferAllocator::ExecutionScope::ExecuteWithBufferAllocations(
  ...
  if (remapping_ == nullptr) {
    return Internal("Missing SKIP_PROFILED state for module %s",
                    owner_->module_name_);
  }
...
GpuExecutableBufferAllocator::CreateExecutionScope(
  ...
  if (vmm_allocator == nullptr) {
    if (update_mode_ == DebugOptions::SKIP_PROFILED) {
      return FailedPrecondition(
          "SKIP_PROFILED command buffer update mode requires a "
          "DeviceAddressVmmAllocator for module %s",
          module_name_);
    }
    return scope_without_remapping();
  }
