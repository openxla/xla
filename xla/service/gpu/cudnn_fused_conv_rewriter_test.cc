...
TEST_F(CudnnFusedConvRewriterTest, TestBiasMultiCallMultiThread) {
  // max(0, conv(x, w) + bias);
  std::string code = R"(
    HloModule Test
    ...
    ENTRY Test {
      zero = f16[] constant(0)
      zeros = f16[1,3,3,64] broadcast(zero), dimensions={}

      input = f16[1,3,3,64] parameter(0)
      filter = f16[3,3,64,64] parameter(1)
      bias = f16[64] parameter(2)

      conv = f16[1,3,3,64] convolution(input, filter), window={size=3x3 pad=1_1x1_1}, dim_labels=b01f_01io->b01f, feature_group_count=1
      broadcasted_bias = f16[1,3,3,64] broadcast(bias), dimensions={3}
      add1 = f16[1,3,3,64] add(conv, broadcasted_bias)
      ROOT relu = f16[1,3,3,64] maximum(zeros, add1)
    })";

    // this seems very wasteful, but HloModule's are apparently not reusable
    std::unique_ptr<HloModule> modules[100];
    for(int i=0; i<100; i++) {
      TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(code));
      DebugOptions debug_opts = module->config().debug_options();
      debug_opts.set_xla_gpu_use_runtime_fusion(true);
      module->mutable_config().set_debug_options(debug_opts);
    ...
    std::atomic<int> counter(-1), counter2(-1), counter3(-1);

    // Threads 1-3 start all together
    // Thread 1 and 2 keep in sync for 5 iterations
    // Thread 4 starts when thread 1 reaches iteration 5
    auto executor = [&](int id) {
      for(int iter=0; iter<25; iter++) {
        if(id==0) {
          counter++;
          while(counter2<0 || counter3<0);
          if(iter<5)
            while(counter2 < iter);
        }
        else if(id==1) {
        ...
        EXPECT_TRUE(RunAndCompare(std::move(modules[id*25+iter]),
          ErrorSpec{0.01}));
        fflush(stdout);
      }
    };
