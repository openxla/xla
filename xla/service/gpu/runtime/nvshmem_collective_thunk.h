...
template <typename OpT>
void NnvshmemCollectiveConfig::SetCollectiveOpKindAndID(OpT op) {
  if (op.getChannelId()) {
...
class NvshmemCollectiveThunk : public Thunk {
 public:
  NvshmemCollectiveDoneThunk(Kind kind, ThunkInfo thunk_info, bool is_sync);
...
class NvshmemCollectiveDoneThunk : public Thunk {
 ...
 private:
  std::shared_ptr<NvshmemCollectiveThunk::AsyncEvents> async_events_;
