...
template <typename Pattern>
auto OptionalMaximumWithZero(HloInstruction **optional_maximum,
                             Pattern pattern) {
  return m::AnyOf<HloInstruction>(
      m::Maximum(optional_maximum, pattern, m::Broadcast(m::ConstantScalar(0))),
      std::move(pattern));
}
    ...
    if (Match(instr,
              m::Convert(
                  m::Clamp(
                      ...
                      .WithOneUser()))) {
      return maximum != nullptr
                 ? ReluConvertDF8(instr, existing_gemm, d_scale, clamp_lower,
                                  clamp_upper, maximum)
                 : F8ConvertD(
