...
/*static*/ StatusOr<std::unique_ptr<ExperimentalRuntimeExecutable>>
ExperimentalRuntimeExecutable::Create(
  ...
  // Compile IR in Experimental input dialect(s) into IREE VM flatbuffer.
  auto compiler = CreateExperimentalCompiler();
  ...
  auto device = std::make_unique<ExperimentalDevice>();
  ...
  return std::unique_ptr<ExperimentalRuntimeExecutable>(
      new ExperimentalRuntimeExecutable(
          std::move(device), std::move(bytecode),
          std::move(program->buffer_sizes), std::move(program->debug_options),
          asm_text, binary, context, instance, std::move(modules), function));
}

ExperimentalRuntimeExecutable::ExperimentalRuntimeExecutable(
    ...
    absl::Span<const uint8_t> binary, iree::vm::ref<iree_vm_context_t> context,
    iree::vm::ref<iree_vm_instance_t> instance,
  ...
  VLOG(1) << "Created Experimental executable: function name = "
...
Status ExperimentalRuntimeExecutable::Execute(
