...
#ifndef XLA_SERVICE_GPU_EXPERIMENTAL_EXECUTABLE_H_
#define XLA_SERVICE_GPU_EXPERIMENTAL_EXECUTABLE_H_
...
//===----------------------------------------------------------------------===//
// TODO(ezhulenev): We currently do not build with Experimental runtime in open
// source because we do not have bazel dependency from XLA to IREE.
#if XLA_DISABLE_EXPERIMENTAL_RUNTIME
//===----------------------------------------------------------------------===//

namespace xla::gpu {
struct ExperimentalRuntimeProgram {
  ExperimentalRuntimeProgram(std::unique_ptr<mlir::MLIRContext>,
                             mlir::OwningOpRef<mlir::ModuleOp>, std::string,
                             std::vector<int64_t>, DebugOptions) {}
};

struct ExperimentalRuntimeExecutable {
  static StatusOr<std::unique_ptr<ExperimentalRuntimeExecutable>> Create(
      std::unique_ptr<ExperimentalRuntimeProgram>, std::string_view,
      const std::vector<uint8_t>&) {
    return absl::UnimplementedError(
        "Experimental runtime is not supported in OSS build");
  }
...
//===----------------------------------------------------------------------===//
#else  // !XLA_DISABLE_EXPERIMENTAL_RUNTIME
//===----------------------------------------------------------------------===//
...
#include "xla/service/gpu/experimental/compiler.h"
...
// Forward declare.
struct ExperimentalDevice;

// Xla Gpu program lowered to the Experimental dialects (IREE input dialects).
// Experimental runtime executable jit-compiles this program to an executable
...
// before we pass IR to Experimental/IREE compiler (see
// ExperimentalRuntimeExecutable below), because in the XLA compilation pipeline
// backend compiler runs last.
//
...
// the responsibility of Experimental/IREE compiler.
struct ExperimentalRuntimeProgram {
  ExperimentalRuntimeProgram(std::unique_ptr<mlir::MLIRContext> ctx,
...
class ExperimentalRuntimeExecutable {
 public:
  using Bytecode = ExperimentalCompiler::Bytecode;

  // Creates ExperimentalRuntimeExecutable from the Experimental program.
  static StatusOr<std::unique_ptr<ExperimentalRuntimeExecutable>> Create(
      std::unique_ptr<ExperimentalRuntimeProgram> program,
      std::string_view asm_text, const std::vector<uint8_t>& binary);

  ~ExperimentalRuntimeExecutable();
 ...
 private:
  ExperimentalRuntimeExecutable(
      std::unique_ptr<ExperimentalDevice> device,
      std::unique_ptr<Bytecode> bytecode, std::vector<int64_t> buffer_sizes,
  ...
  std::unique_ptr<ExperimentalDevice> device_;
...
#endif  // !XLA_DISABLE_EXPERIMENTAL_RUNTIME
#endif  // XLA_SERVICE_GPU_EXPERIMENTAL_EXECUTABLE_H_
