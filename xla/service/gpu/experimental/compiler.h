...
#ifndef XLA_SERVICE_GPU_EXPERIMENTAL_COMPILER_H_
#define XLA_SERVICE_GPU_EXPERIMENTAL_COMPILER_H_
...
// Returns a new instance of the Experimental compiler loading it from a
...
// Updates Experimental input module with device kernels compiled by XLA.
Status BindXlaDeviceKernels(mlir::ModuleOp, std::string_view asm_text,
                            const std::vector<uint8_t>& binary);

// Wrapper around IREE compiler + bundled Experimental compiler plugins to
// orchestrate compilation from Experimental input dialects for IREE VM
// flatbuffer.
//
...
class ExperimentalCompiler {
 public:
  ...
  ExperimentalCompiler(iree_compiler_session_t* session,
 ...
 private:
  void SetError(iree_compiler_error_t* error) {
    LOG(ERROR) << "Experimental compiler error: "
...
#endif  // XLA_SERVICE_GPU_EXPERIMENTAL_COMPILER_H_
