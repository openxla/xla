...
#include "xla/service/gpu/experimental/google/compiler.h"
#else
#include "xla/service/gpu/experimental/default/compiler.h"
#endif

namespace xla::gpu {

//===-----------------------------------------------------------------------===/
// ExperimentalCompiler::Bytecode
//===-----------------------------------------------------------------------===/

ExperimentalCompiler::Bytecode::Bytecode(iree_compiler_output_t* output,
                                         void* data, size_t length)
    : output_(output), data_(data), length_(length) {}

ExperimentalCompiler::Bytecode::~Bytecode() {
...
ExperimentalCompiler::ExperimentalCompiler(iree_compiler_session_t* session,
                                           iree_compiler_invocation_t* inv)
    : session_(session), inv_(inv) {}

ExperimentalCompiler::~ExperimentalCompiler() {
  if (error_) {
...
bool ExperimentalCompiler::ParseSourceBuffer(std::string_view buffer) {
...
bool ExperimentalCompiler::SetFlag(const char* flag) {
...
std::unique_ptr<ExperimentalCompiler::Bytecode>
ExperimentalCompiler::CompileStandardPipeline() {
...
//===-----------------------------------------------------------------------===/
// Loading ExperimentalCompiler from a library
//===-----------------------------------------------------------------------===/
...
std::unique_ptr<ExperimentalCompiler> CreateExperimentalCompiler() {
  ...
  return std::make_unique<ExperimentalCompiler>(session, inv);
}
