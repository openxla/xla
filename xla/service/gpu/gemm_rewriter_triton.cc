...
std::variant<FusionDecision, DimOrderMap> HandleElementwise(
...
std::variant<FusionDecision, DimOrderMap> HandleBitcast(
    const HloInstruction* hlo, const DimOrderMap& dim_orders,
...
std::variant<FusionDecision, DimOrderMap> HandleCopyOrTransposeOrBroadcast(
    const HloInstruction* hlo, const DimOrderMap& dim_orders,
    const TransformDirection direction) {
  const HloInstruction* src =
      (direction == TransformDirection::kOutputToInput) ? hlo : hlo->operand(0);
...
std::variant<FusionDecision, DimOrderMap> HandleInstruction(
    const HloInstruction* hlo, const DimOrderMap& dim_orders,
...
std::variant<FusionDecision, DimOrderMap> CanFuse(
...
bool MergeDimOrderMapUpdates(DimOrderMap& target, const DimOrderMap& updates) {
  for (const auto& [key, value] : updates) {
    ...
    if (!inserted && !it->second.IsPhysicallyEquivalent(value)) {
      return false;
    }
  }
  return true;
...
void TryToFuseWithInputsRecursively(
  ...
  auto try_fuse_one = [&](HloInstruction& hlo) {
    auto result = CanFuse(hlo, /*as_input=*/true, dim_orders,
...
StatusOr<FusionDecision> FuseDot(HloInstruction& dot,
  ...
  while (output_changed) {
    ...
    auto result = CanFuse(*user, /*as_input=*/false, out_dim_orders,
                          old_to_new_mapping, gpu_version);
    if (!std::holds_alternative<DimOrderMap>(result)) {
      continue;
    }
