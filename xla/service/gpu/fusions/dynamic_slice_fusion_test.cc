...
TEST_F(DynamicSliceFusionTest, AsyncCollective) {
  ...
  uint64_t* result_offset_0 = std::get_if<uint64_t>(&result_offset->at(0));
  uint64_t* result_offset_1 = std::get_if<uint64_t>(&result_offset->at(1));
  uint64_t* result_offset_2 = std::get_if<uint64_t>(&result_offset->at(2));
  ASSERT_NE(result_offset_0 && result_offset_1 && result_offset_2, false);
  EXPECT_EQ(*result_offset_0, 1ul);
  EXPECT_EQ(*result_offset_1, 0ul);
  EXPECT_EQ(*result_offset_2, 0ul);
