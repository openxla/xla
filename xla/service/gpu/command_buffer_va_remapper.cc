...
#include "xla/service/gpu/command_buffer_va_remapper.h"
...
void CommandBufferVaRemapper::CaptureAllocations(
  ...
  //   NEVER_UPDATE - collect all allocations from all command buffer commands
  //   CAPTURE_CMD_NEVER_UPDATE - collect only allocations from traced commands,
  //     including collective commands recorded by CollectiveThunk.
  if (update_mode != DebugOptions::NEVER_UPDATE &&
      update_mode != DebugOptions::CAPTURE_CMD_NEVER_UPDATE) {
    return;
  }

  CHECK_OK(thunk_executor.thunks().WalkNested([&](const Thunk* t) {
    ...
    return cbt->WalkCommands([&](const Command* cmd) -> absl::Status {
      ...
      for (const BufferUse& use : cmd->buffer_uses()) {
        ...
        allocation_indexes_.insert(index);
      }
      return absl::OkStatus();
    });
  }));

  VLOG(3) << "VA remapping: collected " << allocation_indexes_.size()
          << " allocation indexes for module " << module_name;
}

bool CommandBufferVaRemapper::ShouldUse(
    DebugOptions::CommandBufferUpdateMode update_mode,
    se::DeviceAddressAllocator* memory_allocator) const {
  return !allocation_indexes_.empty() &&
         update_mode != DebugOptions::ALWAYS_UPDATE &&
...
// NOLINTEND
// clang-format on
absl::Status CommandBufferVaRemapper::Execute(
    const BufferAllocations& buffer_allocations,
    const ServiceExecutableRunOptions* run_options,
    se::StreamExecutor* executor, absl::string_view module_name,
    ExecuteFn execute) {
  ...
  XLA_VLOG_DEVICE(3, executor->device_ordinal())
      << "VA remapping: module " << module_name
      << " va_range_idx=" << command_buffer_va_range_idx
      << " num_allocations=" << allocation_indexes_.size();

  // Get the DeviceAddressVmmAllocator to look up physical allocations.
  // vmm_allocator is guaranteed non-null here because ShouldUse already checked
  // for it.
  ...
  if (va_ranges->va_reservation == nullptr) {
    ...
    for (BufferAllocation::Index i : allocation_indexes_) {
    ...
    XLA_VLOG_DEVICE(3, executor->device_ordinal()) << absl::StreamFormat(
        ...
        module_name, va_ranges->va_reservation->address().opaque(),
  ...
  // Iterate through allocation_indexes_ in order (btree_set provides
  ...
  for (BufferAllocation::Index idx : allocation_indexes_) {
...

  {
    ...
    // Batch-map all command buffer allocations into the reserved VA range in a
    // single call. This maps the contiguous range formed by the descriptors and
    // enables device access before returning.
