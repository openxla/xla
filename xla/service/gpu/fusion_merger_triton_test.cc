...
ENTRY main {
  ...
  ROOT triton_softmax = f32[125,127]{1,0} fusion(auxiliary_fusion), kind=kCustom, calls=triton_softmax_computation, backend_config={"operation_queue_id":"0","wait_on_operation_queues":[], "fusion_backend_config": {"kind":"__triton_softmax"}}
})";
...
ENTRY main {
  ...
  triton_softmax = f32[125,127]{1,0} fusion(param_0), kind=kCustom, calls=triton_softmax_computation, backend_config={"operation_queue_id":"0","wait_on_operation_queues":[], "fusion_backend_config": {"kind":"__triton_softmax"}}
...
ENTRY main {
  param_0 = f32[125]{0} parameter(0)
  auxiliary_fusion = f32[125,127]{1,0} fusion(param_0), kind=kLoop, calls=auxiliary_computation
  triton_softmax = f32[125,127]{1,0} fusion(auxiliary_fusion), kind=kCustom, calls=triton_softmax_computation, backend_config={"operation_queue_id":"0","wait_on_operation_queues":[], "fusion_backend_config": {"kind":"__triton_softmax"}}
...
ENTRY main {
  ...
  ROOT triton_softmax = f32[10,125,127]{2,1,0} fusion(auxiliary_fusion), kind=kCustom, calls=triton_softmax_computation, backend_config={"operation_queue_id":"0","wait_on_operation_queues":[], "fusion_backend_config": {"kind":"__triton_softmax"}}
}
)";
  auto module = ParseAndReturnVerifiedModule(kHloText).value();
  FusionMergerTriton fusion_merger;
...
ENTRY main {
  ...
  triton_softmax = f32[125,127]{1,0} fusion(producer_fusion), kind=kCustom, calls=triton_softmax_computation, backend_config={"operation_queue_id":"0","wait_on_operation_queues":[], "fusion_backend_config": {"kind":"__triton_softmax"}}
  ROOT consumer_fusion = f32[125,127]{1,0} fusion(param_1, triton_softmax), kind=kLoop, calls=consumer_computation
})";
  auto module = ParseAndReturnVerifiedModule(kHloText).value();
  FusionMergerTriton fusion_merger{};
