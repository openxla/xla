...
void UpadteBufferImpl(se::gpu::GpuStreamHandle stream, void** buffers,
                      int offset) {
  // Expect that the input and output buffers are the same.
  if (buffers[0] != buffers[1]) {
    return;
...
void UpdateBuffer1(se::gpu::GpuStreamHandle stream, void** buffers,
                   const char* /*opaque*/, size_t /*opaque_len*/) {
  return UpadteBufferImpl(stream, buffers, /*offset=*/0);
}
// Adds 1 to the last 2 elements of the input buffer.
void UpdateBuffer2(se::gpu::GpuStreamHandle stream, void** buffers,
                   const char* /*opaque*/, size_t /*opaque_len*/) {
  return UpadteBufferImpl(stream, buffers, /*offset=*/2);
}

XLA_REGISTER_CUSTOM_CALL_TARGET(UpdateBuffer1, PLATFORM);
XLA_REGISTER_CUSTOM_CALL_TARGET(UpdateBuffer2, PLATFORM);
...
TEST_F(CustomCallHloTest, CallConcurrentUpdateTwoBuffers) {
  ...
  async_comp1 {
   ...
   ROOT va1 = b(s32[4]) custom-call(pa1), custom_call_target="UpdateBuffer1", output_to_operand_aliasing={{}: (0, {})}, api_version=API_VERSION_STATUS_RETURNING
  }

  async_comp2 {
   pa2 = b(s32[4]) parameter(0)
   ROOT va2 = b(s32[4]) custom-call(pa2), custom_call_target="UpdateBuffer2", output_to_operand_aliasing={{}: (0, {})}, api_version=API_VERSION_STATUS_RETURNING
  }
  ...
  ENTRY test_computation {
    ...
    b1_1 = b(s32[4]) call(b1_0), to_apply=async_comp1, frontend_attributes={_xla_stream_annotation="1", inlineable="false"}
    b2_1 = b(s32[4]) call(b2_0), to_apply=async_comp2, frontend_attributes={_xla_stream_annotation="2", inlineable="false"}
...
// TODO: Enable this test once the GPU pipeline support stream notation for
//       async-start and async-done.
TEST_F(CustomCallHloTest, DISABLED_CustomCallConcurrentUpdateTwoBuffers) {
  ...
  ENTRY test_computation {
    ...
    b1_1_start = ((b(s32[4])), b(s32[4])) async-start(b1_0), calls=async_comp1, frontend_attributes={_xla_stream_annotation="1"}
    b1_1 = b(s32[4]) async-done(b1_1_start), frontend_attributes={_xla_stream_annotation="1"},
    ...
    b2_1_start = ((b(s32[4])), b(s32[4])) async-start(b2_0), calls=async_comp2, frontend_attributes={_xla_stream_annotation="2"}
    b2_1 = b(s32[4]) async-done(b2_1_start), frontend_attributes={_xla_stream_annotation="2"},
      backend_config={"operation_queue_id":"0","wait_on_operation_queues":[],"force_earliest_schedule":false,"reification_cost":[],"device_type":"DEVICE_TYPE_INVALID"}
