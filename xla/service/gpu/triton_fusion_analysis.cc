...
int64_t GetContractingDimSize(const HloInstruction& dot, int operand_number) {
  ...
  for (int64_t dim : contracting_dims) {
    contracting_dim_size *=
        dot.operand(operand_number)->shape().dimensions(dim);
  }
...
/*static*/ absl::StatusOr<FusionContext> FusionContext::FromDotOperand(
  ...
  int64_t contracting_size = GetContractingDimSize(dot, operand_number);
...
/*static*/ FusionContext FusionContext::FromDotOutput(
  ...
  int64_t contracting_size = GetContractingDimSize(dot, /*operand_number=*/0);
