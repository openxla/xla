...
Status IrEmitterUnnested::EmitUnnestedReduction(
  ...
  if (!reduction_codegen_info->IsRaceFree()) {
    auto fusion_roots = fusion_analysis.fusion_roots();
  ...
  auto instr_index_groups = reduction_codegen_info->GetIndexGroups();
  auto reduce_operand_shape = reduction_codegen_info->GetReduceOperandShape();
