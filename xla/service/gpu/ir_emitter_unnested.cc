...
Status IrEmitterUnnested::EmitCollectivePermute(
  ...
  TF_RET_CHECK(
      ...
      Shape::Equal().IgnoreMemorySpaceInLayout()(instr->shape().tuple_shapes(0),
                                                 instr->shape().tuple_shapes(1)));
  TF_ASSIGN_OR_RETURN(BufferAllocation::Slice result_slice,
                      GetAllocationSliceForHlo(instr, {1}));
  ...
  const int64_t dst_memory_space = instr->shape().tuple_shapes(1).layout().memory_space();
