...
__global__ void l2_prefetch(
  ...
  for (int b = 0; b < kMaxNumPrefetchBuffers; ++b) {
    const int num_iterations =
        (buffer_sizes[b] + total_size_per_iteration - 1) /
    ...
    const char* ptr = pointers[b] + blockIdx.x * total_size_per_block +
                      threadIdx.x * total_size_per_thread;

    for (int i = 0; i < num_iterations; ++i) {
      // #if (__CUDA_ARCH__ >= 800)
      //       asm("prefetch.global.L2::evict_last [%0];" ::"l"(ptr));
      // #else
