...
TEST_F(CutlassFusionTest, RowMajorGemmKernel) {
  ...
  ENTRY e {
    ...
    ROOT _ = f32[100,10]{1,0} fusion(arg0, arg1), kind=kCustom, calls=cutlass_gemm,
      backend_config={"operation_queue_id":"0","wait_on_operation_queues":[],"fusion_backend_config":{kind: "__custom_fusion", custom_fusion_config: {"name":"cutlass_gemm"}}}
  })";
...
TEST_F(CutlassFusionTest, RowMajorGemmWithUpcastKernel) {
  ...
  ENTRY e {
    ...
    ROOT _ = bf16[16,8]{1,0} fusion(p0, p1), kind=kCustom, calls=cutlass_gemm_with_upcast,
      backend_config={"operation_queue_id":"0","wait_on_operation_queues":[],"fusion_backend_config":{kind: "__custom_fusion", custom_fusion_config: {"name":"cutlass_gemm_with_upcast"}}}
  })";
...
TEST_F(CutlassFusionTest, RowMajorGemmWithDynamicUpdateSliceKernel) {
  ...
  ENTRY e {
    ...
    r.0 = (bf16[2,8,8]{2,1,0}, u8[1024]{0}) fusion(p1, p0, p2, p3), kind=kCustom,
      calls=%cutlass_gemm,
      backend_config={"operation_queue_id":"0","wait_on_operation_queues":[],"fusion_backend_config":{"kind":"__custom_fusion","custom_fusion_config":{"name":"cutlass_gemm_with_dynamic_update_slice"}}}
