...
TEST_F(GpuSchedulePostprocessingTest, SynchronousOpsNotChanged) {
  ...
  ENTRY entry {
    ...
    all-gather-start = (f32[1], f32[2]) all-gather-start(pf32), dimensions={0}, backend_config={"operation_queue_id":"0","wait_on_operation_queues":[],"collective_backend_config":{"is_sync":true,"no_parallel_custom_call":false}}
...
TEST_F(GpuSchedulePostprocessingTest, AsynchronousOpsChanged) {
  ...
  ENTRY entry {
    ...
    all-gather-start = (f32[1], f32[2]) all-gather-start(pf32.2), dimensions={0}, backend_config={"operation_queue_id":"0","wait_on_operation_queues":[],"collective_backend_config":{"is_sync":false}}
    ROOT all-gather-done = f32[2] all-gather-done(all-gather-start)
  }
)";
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
...
TEST_F(GpuSchedulePostprocessingTest, AsynchronousOpsWithParallelCustomcall) {
  ...
  ENTRY entry {
    pf32 = f32[1] parameter(0)
    all-gather-start = (f32[1], f32[2]) all-gather-start(pf32), dimensions={0}, backend_config={"operation_queue_id":"0","wait_on_operation_queues":[],"collective_backend_config":{"is_sync":false}}
    pf32.2 = f32[1] custom-call(pf32), custom_call_target="my_custom_call"
