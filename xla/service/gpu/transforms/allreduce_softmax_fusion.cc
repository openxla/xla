...
bool CanFuseAllReduceWithTritonSoftmax(const HloInstruction* all_reduce,
  ...
  if (triton_softmax->operand_count() != 1 || 
    ...
    VLOG(2) << "No direct dependency from all-reduce to triton softmax";
    return false;
  }
  
  // Check shape compatibility
  if (!ShapeUtil::Equal(all_reduce->shape(), triton_softmax->shape())) {
    VLOG(2) << "Shape mismatch between all-reduce and triton softmax";
    return false;
  }
  
  // Ensure all-reduce has only one user (the triton softmax)
  if (all_reduce->user_count() != 1) {
    VLOG(2) << "All-reduce has multiple users, cannot fuse";
    return false;
  }
  
  // Safety check: verify tensor size is reasonable for fusion
  const Shape& shape = all_reduce->shape();
  if (shape.IsArray()) {
    int64_t total_elements = ShapeUtil::ElementsIn(shape);
    VLOG(2) << "Tensor has " << total_elements << " elements";
    
    // Skip fusion for very large tensors to avoid compilation issues
    if (total_elements > 10000) {
      VLOG(2) << "Tensor too large for safe fusion: " << total_elements << " elements";
      return false;
    }
  }
  
  VLOG(2) << "Fusion is feasible";
  return true;
}
...
absl::StatusOr<GpuBackendConfig> CreateSafeBackendConfig(
  ...
  if (fused_config.has_fusion_backend_config() && 
    ...
    if (output_shape.IsArray()) {
      ...
      VLOG(2) << "Adjusting backend config for tensor with " << total_elements << " elements";
      
      // Apply conservative settings for small tensors
      if (total_elements <= 64) {
        VLOG(2) << "Applying small tensor optimizations";
        
        // Set conservative tile sizes for small tensors
        if (block_config->output_tiles_size() > 0) {
          auto* first_tile = block_config->mutable_output_tiles(0);
          if (first_tile->sizes_size() > 0) {
            // Use power-of-2 tile sizes
            int64_t tile_size = 1;
            while (tile_size < total_elements && tile_size < 32) {
              tile_size *= 2;
            }
            tile_size = std::min(tile_size, total_elements);
            
            for (int i = 0; i < first_tile->sizes_size(); ++i) {
              first_tile->set_sizes(i, tile_size);
            }
            VLOG(2) << "Set tile size to " << tile_size;
          }
        }
        
        // Use conservative warp count for small tensors  
        if (total_elements <= 32) {
          block_config->set_num_warps(1);
          VLOG(2) << "Set num_warps to 1 for small tensor";
        }
      }
    }
  }
...
absl::StatusOr<bool> FuseAllReduceWithTritonSoftmax(
  ...
  VLOG(1) << "Attempting to fuse all-reduce with triton softmax";
  VLOG(2) << "AllReduce: " << all_reduce->ToString();
  VLOG(2) << "TritonSoftmax: " << triton_softmax->ToString();
  
  HloComputation* computation = all_reduce->parent();
  
  // Get the triton softmax's fused computation
  HloComputation* original_softmax_computation = triton_softmax->fused_instructions_computation();
  if (!original_softmax_computation) {
    return absl::InvalidArgumentError("No fused computation found in triton softmax");
  }
  ...
  if (original_softmax_computation->instruction_count() > 50) {
    VLOG(1) << "Skipping fusion: computation too complex (" 
            << original_softmax_computation->instruction_count() << " instructions)";
    return false;
  }
  
  // Create a new fused computation that combines all-reduce and softmax
  HloComputation::Builder builder("fused_allreduce_softmax");
  ...
  absl::flat_hash_map<const HloInstruction*, HloInstruction*> cloned_instructions;
  
  // Map the original parameter to our cloned all-reduce
  for (HloInstruction* original_param : original_softmax_computation->parameter_instructions()) {
    cloned_instructions[original_param] = cloned_all_reduce;
  }
  
  // Clone other instructions in topological order with safety checks
  for (HloInstruction* original_instr : original_softmax_computation->MakeInstructionPostOrder()) {
    ...
    for (HloInstruction* operand : original_instr->operands()) {
      ...
      if (it == cloned_instructions.end()) {
        return absl::InternalError(absl::StrCat("Operand not found during cloning: ", operand->name()));
      }
      new_operands.push_back(it->second);
    }
    
    // Clone the instruction
    HloInstruction* cloned_instr = builder.AddInstruction(
        original_instr->CloneWithNewOperands(original_instr->shape(), new_operands));
    cloned_instructions[original_instr] = cloned_instr;
  }
  
  // Set the root to be the cloned version of the original root
  HloInstruction* original_root = original_softmax_computation->root_instruction();
  ...
  HloComputation* new_computation = computation->parent()->AddEmbeddedComputation(
      std::move(fused_computation));
  
  // Create the fused fusion instruction
  std::vector<HloInstruction*> fusion_operands = {const_cast<HloInstruction*>(all_reduce->operand(0))};
  HloInstruction* fused_fusion = computation->AddInstruction(
      HloInstruction::CreateFusion(
          triton_softmax->shape(),
          HloInstruction::FusionKind::kCustom,
          fusion_operands,
          new_computation));
  
  // Set up safe backend config for the fused operation
  TF_ASSIGN_OR_RETURN(auto safe_config, CreateSafeBackendConfig(triton_softmax));
  ...
  VLOG(1) << "Successfully fused all-reduce with triton softmax";
  return true;
}
...
absl::StatusOr<bool> AllReduceSoftmaxFusion::Run(
  ...
  VLOG(1) << "Running AllReduceSoftmaxFusion pass on module: " << module->name();
  ...
  for (HloComputation* computation : module->computations(execution_threads)) {
    ...
    for (HloInstruction* instruction : computation->instructions()) {
      if (instruction->opcode() == HloOpcode::kAllReduce) {
        ...
        VLOG(2) << "Found all-reduce: " << instruction->name();
      }
    }
    ...
    for (HloInstruction* all_reduce : all_reduces) {
      ...
      for (HloInstruction* user : all_reduce->users()) {
        if (IsTritonSoftmaxFusion(user) && 
          ...
          fusion_attempts++;
          VLOG(1) << "Found fusion opportunity #" << fusion_attempts 
          ...
          if (fused) {
            fusion_count++;
            changed = true;
            VLOG(1) << "Successfully fused AllReduce with TritonSoftmax (#" << fusion_count << ")";
  ...
  VLOG(1) << "AllReduceSoftmaxFusion completed. Fusion attempts: " << fusion_attempts 
          << ", Successful fusions: " << fusion_count;
