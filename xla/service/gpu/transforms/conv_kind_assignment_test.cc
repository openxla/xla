...
class ConvKindAssignmentTest : public HloHardwareIndependentTestBase {
 ...
 protected:
  bool RunPass(HloModule* module) {
    return ConvKindAssignment(kDefaultCC).Run(module).value();
  }
...
TEST_F(ConvKindAssignmentTest, BackwardFilterConvolveWithPaddedActivations) {
...
TEST_F(ConvKindAssignmentTest, BackwardFilterConvolveWithPaddedGradients) {
  auto builder = HloComputation::Builder(TestName());
  HloInstruction* activations =
      builder.AddInstruction(HloInstruction::CreateParameter(
...
TEST_F(ConvKindAssignmentTest, BackwardInputConvolve1x1Filter) {
  auto builder = HloComputation::Builder(TestName());
...
TEST_F(ConvKindAssignmentTest, BackwardInputConvolveUnevenPaddingOnGradients) {
  auto builder = HloComputation::Builder(TestName());
  HloInstruction* output =
      builder.AddInstruction(HloInstruction::CreateParameter(
...
TEST_F(ConvKindAssignmentTest, BackwardInputConvolveLowPaddingTooLarge) {
  auto builder = HloComputation::Builder(TestName());
  HloInstruction* output =
      builder.AddInstruction(HloInstruction::CreateParameter(
          0, ShapeUtil::MakeShape(F32, {20, 4, 4, 320}), "output"));
...
TEST_F(ConvKindAssignmentTest, BackwardInputConvolveConstantFilter) {
