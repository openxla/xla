...
TEST_F(L2PrefetchSchedulerTest,
       DoesNotSchedulePrefetchWithUnannotatedAllReduce) {
...
entry {
  ...
  n = bf16[460,1024] fusion(x, ard), kind=kLoop, calls=f
})"));
  EXPECT_THAT(L2PrefetchScheduler{TestGpuDeviceInfo::RTXA6000DeviceInfo()}.Run(
                  module.get()),
              IsOkAndHolds(false));
}
...
entry {
  ...
  n = bf16[460,1024] fusion(x, bc), kind=kLoop, calls=f
      ...
      L2PrefetchScheduler{TestGpuDeviceInfo::RTXA6000DeviceInfo()}, R"(
// CHECK: bf16[2244]{0} slice
// CHECK: all-reduce-start
// CHECK-NEXT: l2_prefetch
...
entry {
  ...
  p = (s8[10000]) fusion(b), kind=kCustom, calls=%l2_prefetch,
