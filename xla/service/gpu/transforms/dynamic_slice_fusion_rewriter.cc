...
// Returns true if the value `offset` is a function of the loop induction
// variable.
bool IsOffsetFunctionOfLoopIndvar(const HloInstruction& offset,
                                  CallGraph& call_graph) {
  auto callers = call_graph.GetComputationCallers(offset.parent());
  if (callers.size() != 1) {
    VLOG(1) << "Computation has multiple callers: "
  ...
  if (while_op->opcode() != HloOpcode::kWhile) {
    VLOG(1) << "Computation caller is not while, it is "
            << while_op->ToString();
    return false;
  }
  HloComputation* while_body = while_op->while_body();
  auto tuple_idx = GetLoopInductionVarTupleIdx(while_op);
  if (tuple_idx == std::nullopt) {
    VLOG(1) << "Induction variable tuple index is nullopt";
    return false;
  }
  if (while_body->num_parameters() != 1) {
    VLOG(1) << "Expected only one parameter for while body: "
            << while_op->ToString();
    return false;
  }
  if (!while_body->parameter_instruction(0)->shape().IsTuple()) {
    VLOG(1) << "Expected while body param to be a tuple, found "
            << while_op->ToString();
    return false;
  }
  auto indvar = hlo_query::GetUniqueGteInstruction(
      while_body->parameter_instruction(0), *tuple_idx);
  if (!indvar) {
    VLOG(1) << "Unable to find unique GTE for while induction variable idx: "
            << *tuple_idx << ", while op: " << while_op->ToString();
    return false;
  }
  auto update = while_body->root_instruction()->operand(*tuple_idx);
  auto update_module = ExtractModule(update);
  auto offset_module = ExtractModule(&offset);
  auto is_dependent_on_indvar_only = [tuple_idx](HloModule* module) {
    auto entry = module->entry_computation();
    if (entry->num_parameters() != 1) {
      VLOG(1) << "Entry computation has more than one parameter : "
              << module->ToString();
      return false;
    }
    auto param = entry->parameter_instruction(0);
    // Check that all users of this param are gte
    if (!absl::c_all_of(
            param->users(), [tuple_idx](const HloInstruction* user) {
              return user->opcode() == HloOpcode::kGetTupleElement &&
                     user->tuple_index() == *tuple_idx;
            })) {
      VLOG(1)
          << "Not all users of the param are gte operations with tuple idx: "
          << *tuple_idx << ", module: " << module->ToString();
      return false;
    }

    // if any instruction is partition-id or replica-id then this is not valid.
    for (auto computation : module->computations()) {
      for (auto instr : computation->instructions()) {
        if (instr->opcode() == HloOpcode::kPartitionId ||
            instr->opcode() == HloOpcode::kReplicaId) {
          VLOG(1) << "Offset depends on partition id or replica id: "
                  << module->ToString();
          return false;
        }
      }
  ...
  if (is_dependent_on_indvar_only(update_module.get()) &&
      is_dependent_on_indvar_only(offset_module.get())) {
    return true;
  }
  return false;
}
...
// constant or a function of loop induction variable.
bool HasConstantOrLoopIterationOffsets(const HloDynamicIndexInstruction& instr,
  ...
  return llvm::all_of(
      instr.index_operands(), [&call_graph](const HloInstruction* offset) {
        return IsOffsetFunctionOfLoopIndvar(*offset, call_graph) ||
