...
absl::StatusOr<bool> L2PrefetchScheduler::Run(
  ...
  for (HloComputation* computation : module->MakeNonfusionComputations()) {
    ...
    for (HloInstruction* done : computation->instructions()) {
      ...
      int num_blocks;
      if (!absl::SimpleAtoi(done->get_frontend_attribute(
                                    se::gpu::PrefetchKernel::kNumBlocksAttr)
                                .value_or(""),
                            &num_blocks)) {
        num_blocks = stream_executor::gpu::PrefetchKernel::kDefaultBlocks;
      }
      if (num_blocks <= 0) {
        continue;
      }
      ...
      auto stop_schedule_traversal = [&]() {
        if (to_prefetch.size() >=
            stream_executor::gpu::PrefetchKernel::kMaxNumBuffers) {
          return true;
        }
      ...
      custom_call->set_frontend_attribute(
          se::gpu::PrefetchKernel::kNumBlocksAttr, std::to_string(num_blocks));
