...
TEST_F(GpuHloScheduleTest, LHSSendRecvAllReduce) {
  ...
  while_body {
    ...
    all-reduce-start = f32[1, 1024, 1024] all-reduce-start(f32[1, 1024, 1024] p),
      replica_groups={{0,1}}, to_apply=add,  backend_config={"operation_queue_id":"0","wait_on_operation_queues":[],"collective_backend_config":{"is_sync":false}}
...
TEST_F(GpuHloSchedulePostProcessTest, PostProcessAsyncCollectives) {
  ...
  ENTRY ar {
    ...
    // other.
    ag-start = (f32[32], f32[64]) all-gather-start(p1), dimensions={0}, backend_config="{\"operation_queue_id\":\"0\",\"wait_on_operation_queues\":[],\"collective_backend_config\":{\"is_sync\":true}}"
