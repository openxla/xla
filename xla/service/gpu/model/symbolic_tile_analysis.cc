...
llvm::SmallVector<int64_t> GetNumberOfTilesPerDimension(
    TiledHloInstruction* tiled_hlo_instr) {
  ...
  for (auto [dim_size, tile_size] :
    ...
    // We need to use padded tile size here, as that is what the triton fusion
    // emitter will actually use.
    result.push_back(CeilOfRatio(
        dim_size, static_cast<int64_t>(llvm::PowerOf2Ceil(tile_size))));
  }
  return result;
}
...
IndexingMap LinearizeTileOffsets(
    ...
    const llvm::SmallVector<int64_t>& num_output_tiles_per_dim,
    mlir::MLIRContext* mlir_context) {
  int64_t num_tiles = absl::c_accumulate(
      num_output_tiles_per_dim, static_cast<int64_t>(1), std::multiplies<>());
...
// Returns true when the propagated tile offsets for `root` are the same as the
// tile offsets we can compute by tiling `root` using the propagated tile size
// as tiling parameters.
bool PropagatedTileOffsetsAreEqualToTileOffsetsForRoot(
    TiledHloInstruction* root,
    const llvm::SmallVector<int64_t>& num_output_tiles_per_dim,
    mlir::MLIRContext* mlir_context) {
  ...
  llvm::SmallVector<int64_t> num_output_tiles_per_dim_for_root =
      GetNumberOfTilesPerDimension(root);
  if (Product(num_output_tiles_per_dim) !=
      Product(num_output_tiles_per_dim_for_root)) {
    return false;
  }
  // Compute the tile offset indexing directly for `root`.
  auto root_indexing_map =
      CreateIdentityMap(root->hlo()->shape(), mlir_context);
  // We need to use the padded tile sizes here, as that is what the fusion
  // emitter will do.
  std::vector<int64_t> padded_tile_sizes;
  padded_tile_sizes.reserve(root->tile_sizes().size());
  for (int64_t tile_size : root->tile_sizes()) {
    padded_tile_sizes.push_back(llvm::PowerOf2Ceil(tile_size));
  }
  auto maybe_output_tiling_info = ComputeOutputTilingInfo(
      root_indexing_map, padded_tile_sizes, mlir_context);
  if (!maybe_output_tiling_info.ok()) {
    return false;
  }
  ...
  // maps to be able to compare them. We need to have the same tile offsets
  // to avoid problems with buffer sharing.
  auto maybe_tile_offset_indexing = root->tile_offsets_indexing();
  if (!maybe_tile_offset_indexing.ok()) {
    return false;
  }
  auto linearized_propagated_tile_offsets_indexing =
      LinearizeTileOffsets(maybe_tile_offset_indexing.value(),
                           num_output_tiles_per_dim, mlir_context);
  auto linearized_direct_tile_offsets_indexing = LinearizeTileOffsets(
      maybe_output_tiling_info->output_tile_offset_indexing,
      num_output_tiles_per_dim_for_root, mlir_context);
  return linearized_propagated_tile_offsets_indexing ==
         linearized_direct_tile_offsets_indexing;
}
...
absl::StatusOr<std::vector<const TiledHloInstruction*>> InitializeTiledRoots(
  ...
  for (const auto& tiled_hlo_instr : llvm::drop_end(tiled_hlo_instructions)) {
    ...
    // In some cases we allow to share input buffers of fusion parameters with
    // output buffers. To avoid race conditions where we overwrite an input
    // value that is still required to compute another output value, we need to
    // make sure that we process the same set of tiles as the real root. Just
    // using the propagated tile offsets does not ensure this, as all tile sizes
    // are padded to a power of 2, and the padding might work out differently
    // for the two different shapes. For example consider the shapes [3,12] and
    // [36] with tiling parameters {1, 16} and {16}, respectively. With the [36]
    // shape, we would only pad the last tile, while for the shape [3,12] we
    // would pad all tiles. By comparing the propagated tile offsets with a tile
    // offset computed directly for this root, we ensure that there will be no
    // difference in which of the output tiles is padded. In our example above,
    // the propagated tile offsets would be [0, 12, 24], while the directly
    // computed tile offsets would be [0, 16, 32].
    if (!PropagatedTileOffsetsAreEqualToTileOffsetsForRoot(
            /*root=*/tiled_hlo_instr.get(), num_output_tiles_per_dim,
            mlir_context)) {
      continue;
    }
