...
// instruction uniquely (i.e. without duplicate tiles).
bool TilingCoversWholeShapeUniquely(TiledHloInstruction* tiled_hlo_instr) {
  ...
  for (int64_t i = 0; i < output_shape.rank(); ++i) {
    ...
    // that we restrict the stride to 0 or 1, it is enough to check the range of
    // the tile offsets whether the largest tile offset plus the tile cover the
