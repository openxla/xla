...
absl::StatusOr<EstimateRunTimeData> EstimateRunTimeForDotOpWithBlockParameters(
  ...
  if (num_stages <= 1 || k_loop_iterations <= 1) {
    ...
    total_main_loop_time = hbm_timing.total_time() +
                           compute_and_flops.compute_time +
                           k_loop_iterations * detail::kLoopLatencyTax;
  } else {
    // Pipelined execution: Calculate the compute and memory per loop iteration.
    const absl::Duration iter_compute_time =
    ...
    const absl::Duration iter_mem_time =
        iter_raw_mem_time + detail::kLoopLatencyTax;
    ...
    const absl::Duration prologue_time =
        detail::kLoopLatencyTax + prologue_loops * iter_raw_mem_time;
  ...
  estimates.exec_time = std::max({total_main_loop_time, l2_time});
