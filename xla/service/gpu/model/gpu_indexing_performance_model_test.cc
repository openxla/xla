...
ENTRY e {
  ...
  EXPECT_NEAR(absl::ToDoubleNanoseconds(runtime_data.exec_time), 11660, 100);
}
...
ENTRY e {
  ...
  // For now just check that we do not crash both with default and custom
  // parameters and also that the custom parameters are actually used.
  const auto result_default =
      indexing_cost_model_.EstimateRunTimeForTriton(root, nullptr);
  ASSERT_OK(result_default.status());
  ...
  const auto result_with_custom =
      indexing_cost_model_.EstimateRunTimeForTriton(root, &custom_params);
  ASSERT_OK(result_with_custom.status());
...
ENTRY e {
  ...
  auto dot_instr = Cast<HloDotInstruction>(root->fused_expression_root());

  ASSERT_OK_AND_ASSIGN(
      auto expected_compute_and_flops,
  ...
  ASSERT_GT(runtime.exec_time, expected_compute_and_flops.compute_time * 1.2);
}
...
ENTRY e {
  ...
  absl::Duration expected_time = expected_compute_and_flops.compute_time +
  ...
  ASSERT_GE(runtime.exec_time, lower_bound);
  ASSERT_LE(runtime.exec_time, upper_bound);
}
