...
// Fallback parsing utility used when the backend config cannot be parsed as a
// structured GpuBackendConfig proto. This commonly happens for custom calls
// generated by frontends like JAX, which pass their configuration as opaque
// strings (often containing JSON-encoded attributes).
//
// Since these strings are parsed from HLO representation, quotes within the
// nested JSON might be escaped in various ways depending on the serialization
// path. The regex pattern handles the following escaping schemes for quotes
// surrounding keys (and optionally values):
// - flops                     : unquoted key
// - "flops"                   : quoted key
// - \"flops\"                 : standard escaped quote
// - \22flops\22               : octal-like escape (hex 22 = 34 = ")
// - \x22flops\x22 or \X22...  : hex-like escape
// - \u0022flops\u0022 or \U...: unicode escape
//
// The regex uses independent optional groups for the backslash (?:\\)? and
// the quote/escape representation (?:\"|22|...) to match any of these formats
// prefixing or suffixing the key name.
static constexpr absl::string_view kQuoteEscapePattern =
    "(?:\\\\)?(?:\\\"|22|[xX]22|[uU]0022|[uU]00000022)?";

std::string CostRegexPattern(absl::string_view key) {
  return absl::StrCat(key, kQuoteEscapePattern, "\\s*[=:]\\s*",
                      kQuoteEscapePattern, "(-?[0-9]+)");
}

struct CustomCallCostEstimate {
  std::optional<int64_t> flops;
  std::optional<int64_t> bytes_accessed;
};

// Extracts the custom call cost estimate (flops and bytes_accessed) from the
// raw backend config string.
std::optional<CustomCallCostEstimate> ExtractCustomCallCostEstimate(
    absl::string_view backend_config_str) {
  if (!absl::StrContains(backend_config_str, "cost_estimate_json")) {
    return std::nullopt;
  }

  CustomCallCostEstimate cost_estimate;
  bool found = false;
  int64_t flops;
  static const absl::NoDestructor<std::string> flops_pattern(
      CostRegexPattern("flops"));
  static const LazyRE2 flops_re = {flops_pattern->c_str()};
  if (RE2::PartialMatch(backend_config_str, *flops_re, &flops) && flops >= 0) {
    cost_estimate.flops = flops;
    found = true;
  }

  int64_t bytes_accessed;
  static const absl::NoDestructor<std::string> bytes_accessed_pattern(
      CostRegexPattern("bytes_accessed"));
  static const LazyRE2 bytes_accessed_re = {bytes_accessed_pattern->c_str()};
  if (RE2::PartialMatch(backend_config_str, *bytes_accessed_re,
                        &bytes_accessed) &&
      bytes_accessed >= 0) {
    cost_estimate.bytes_accessed = bytes_accessed;
    found = true;
  }
...
void GpuHloCostAnalysis::ExtractAndApplyCustomCallCostEstimate(
    const HloInstruction* custom_call) {
  ...
  // Note: We avoid using MLIR attribute deserialization here because
  // HloCostAnalysis operates at the XLA HLO level (where backend_config is
  // just an opaque string). Pulling in heavy MLIR parser dependencies into this
  // HLO pass would violate the modularity boundary between MLIR and XLA.
  // Furthermore, frontends like JAX do not directly populate the
  // GpuBackendConfig protobuf, making the regex-based extraction the most
  // lightweight and robust approach.
  std::optional<CustomCallCostEstimate> cost_estimate =
      ExtractCustomCallCostEstimate(custom_call->raw_backend_config_string());

  if (cost_estimate.has_value()) {
    if (cost_estimate->flops.has_value() && *cost_estimate->flops >= 0) {
      current_properties_[kFlopsKey] = *cost_estimate->flops;
    }
    if (cost_estimate->bytes_accessed.has_value() &&
        *cost_estimate->bytes_accessed >= 0) {
      current_properties_[kBytesAccessedKey] = *cost_estimate->bytes_accessed;
    }
  }
}
