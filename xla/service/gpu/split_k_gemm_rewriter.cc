...
absl::StatusOr<HloInstruction*> MakeSplitKOperand(
  ...
  if (need_padding) {
    ...
    int64_t padding = contracting_size.has_value()
                          ? *contracting_size - k
  ...
  while (!to_process.empty()) {
    ...
    if (current == dot) {
      if (dot_cast != nullptr) {
        ...
        std::optional<int64_t> contracting_size;
        HloInstruction* lhs_scale = dot->mutable_operand(1);
        if (analysis.lhs_block_size().has_value()) {
          TF_ASSIGN_OR_RETURN(
              lhs_scale,
              MakeSplitKOperand(*dot, analysis, config, lhs_contracting_idx,
                                TritonFusionAnalysis::Scope::LHS_SCALE, 1));
          contracting_size =
              lhs_scale->shape().dimensions(lhs_contracting_idx + 1) *
              analysis.lhs_block_size().value() * config.split_k;
        }
        HloInstruction* rhs_scale = dot->mutable_operand(3);
        if (analysis.rhs_block_size().has_value()) {
          if (contracting_size.has_value()) {
            int64_t rhs_block_size = analysis.rhs_block_size().value();
            if (*contracting_size % (rhs_block_size * config.split_k) != 0) {
            ...
            TF_ASSIGN_OR_RETURN(
                ...
                MakeSplitKOperand(*dot, analysis, config, rhs_contracting_idx,
                                  ...
                                  *contracting_size / rhs_block_size));
          } else {
            TF_ASSIGN_OR_RETURN(
                rhs_scale,
                MakeSplitKOperand(*dot, analysis, config, rhs_contracting_idx,
                                  TritonFusionAnalysis::Scope::RHS_SCALE, 3));
            contracting_size =
                rhs_scale->shape().dimensions(rhs_contracting_idx + 1) *
                analysis.rhs_block_size().value() * config.split_k;
          }
        }
        auto is_padded = [](const HloInstruction* op) {
          return op->operand_count() > 0 &&
