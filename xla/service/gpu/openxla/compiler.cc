...
#include "xla/service/gpu/openxla/google/compiler.h"
#else
#include "xla/service/gpu/openxla/default/compiler.h"
#endif

namespace xla::gpu {

//===-----------------------------------------------------------------------===/
// OpenXlaCompiler::Bytecode
//===-----------------------------------------------------------------------===/

OpenXlaCompiler::Bytecode::Bytecode(iree_compiler_output_t* output, void* data,
                                    size_t length)
    : output_(output), data_(data), length_(length) {}

OpenXlaCompiler::Bytecode::~Bytecode() { ireeCompilerOutputDestroy(output_); }

//===-----------------------------------------------------------------------===/
// OpenXlaCompiler
//===-----------------------------------------------------------------------===/

OpenXlaCompiler::OpenXlaCompiler(iree_compiler_session_t* session,
                                 iree_compiler_invocation_t* inv)
    : session_(session), inv_(inv) {}

OpenXlaCompiler::~OpenXlaCompiler() {
  if (error_) {
...
bool OpenXlaCompiler::ParseSourceBuffer(std::string_view buffer) {
...
bool OpenXlaCompiler::SetFlag(const char* flag) {
...
std::unique_ptr<OpenXlaCompiler::Bytecode>
OpenXlaCompiler::CompileStandardPipeline() {
...
//===-----------------------------------------------------------------------===/
// Loading OpenXlaCompiler from a library
//===-----------------------------------------------------------------------===/
...
std::unique_ptr<OpenXlaCompiler> CreateOpenXlaCompiler() {
  ...
  return std::make_unique<OpenXlaCompiler>(session, inv);
}
