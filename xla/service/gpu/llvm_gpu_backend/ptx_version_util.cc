...
DetermineHighestSupportedPtxVersionFromCudaVersion(
  ...
  if (cuda_version > stream_executor::SemanticVersion{13, 0, 0}) {
    // Return maximum known PTX version.
    return kMaxPtxVersion;
  }

  unsigned major_version = cuda_version.major() - 4;
  ...
  if (cuda_version >= stream_executor::SemanticVersion{12, 6, 0} &&
    ...
    minor_version = cuda_version.minor() - 1;
  } else {
    minor_version = cuda_version.minor();
  }
  return {major_version, minor_version, 0};
}
}  // namespace xla::gpu::nvptx
