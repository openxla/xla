...
class GpuCompilerTest
 ...
 public:
  absl::Status Schedule(HloModule* module) {
    auto compiler = Compiler::GetForPlatform(GpuPlatform()->id()).value();
    const se::DeviceDescription& gpu_device_info =
        GpuExecutor()->GetDeviceDescription();
    GpuCompiler* gpu_compiler =
        tensorflow::down_cast<GpuCompiler*>(compiler.get());
  ...
  se::CudaComputeCapability get_cuda_cc() const {
    return GpuExecutor()->GetDeviceDescription().cuda_compute_capability();
  }
};
 ...
 public:
  void SetUp() override {
    ...
    stream_executor::SemanticVersion dnn_version =
        GpuExecutor()->GetDeviceDescription().dnn_version();
...
ENTRY main {
  ...
  auto gpu_cc = GpuExecutor()->GetDeviceDescription().gpu_compute_capability();
  bool is_cuda = gpu_cc.IsCuda();
  auto cuda_cc = get_cuda_cc();
  auto rocm_cc =
      GpuExecutor()->GetDeviceDescription().rocm_compute_capability();
...
class AotCompilationTest : public HloHardwareIndependentTestBase,
                           public ::testing::WithParamInterface<bool> {
 protected:
  AotCompilationTest()
      : ::testing::WithParamInterface<bool>(),
        stream_exec_(GpuPlatform()->ExecutorForDevice(0).value()),
        compiler_(Compiler::GetForPlatform(GpuPlatform()->id()).value()),
        aot_options_(
...
class KernelCacheTest : public HloPjRtTestBase {
 public:
  void SetUp() override {
    ...
    ASSERT_OK_AND_ASSIGN(
        ...
        dynamic_cast<GpuCompiler*>(compiler_.get())
            ->CanUseLinkModules(config, GpuExecutor()->GetDeviceDescription()));
...
ENTRY e {
  ...
  AotCompilationOptions aot_options(compiler_->PlatformId());
  aot_options.set_executor(GpuExecutor());
  ...
  auto test = [this, &aot_options](absl::string_view hlo, int input,
    ...
    ASSERT_OK_AND_ASSIGN(
        std::vector<std::unique_ptr<CompiledModule>> aot_results,
        compiler_->CompileAheadOfTime(std::move(module), aot_options));
...
ENTRY main {
  ...
  std::unique_ptr<Compiler> compiler =
      Compiler::GetForPlatform(GpuPlatform()->id()).value();
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<Executable> executable,
                       compiler->RunBackend(std::move(module), GpuExecutor(),
                                            /*device_allocator=*/nullptr));
...
TEST_F(GpuCompilerPassTest,
  ...
  bool is_rocm =
      GpuExecutor()->GetDeviceDescription().gpu_compute_capability().IsRocm();
...
ENTRY %main {
  ...
  TF_ASSERT_OK(
      tsl::WriteTextProto(tsl::Env::Default(), target_file,
                          Compiler::GpuTargetConfig(GpuExecutor()).ToProto()));
...
ENTRY main {
  ...
  ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<Executable> executable,
      compiler->RunBackend(std::move(hlo_module), GpuExecutor(),
                           /*device_allocator=*/nullptr));
  std::unique_ptr<GpuExecutable> gpu_exec(
      static_cast<GpuExecutable*>(executable.release()));
...
ENTRY main {
  ...
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> compiled_module,
                       compiler->RunHloPasses(module->Clone(), executor,
                                              /*device_allocator=*/nullptr));

  ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<Executable> executable,
      compiler->RunBackend(std::move(compiled_module), executor,
                           /*device_allocator=*/nullptr));
