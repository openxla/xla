...
class FlashAttentionBMMScaleBiasSoftmaxBMM : public MultiHeadedAttentionTest {
 protected:
  const std::string  // NOLINT
  GetModuleFMHA_Flash_Attention_BMM1_Bias_Softmax_BMM2_HloString_BF16() {  // NOLINT
    const std::string hlo_text = R"(
  ...
  template <typename T>
  void TestImpl_FMHA_Flash_Attention_BMM1_Bias_Softmax_BMM2() {
    stream_executor::CudaComputeCapability cc = GetCudaComputeCapability();
    se::dnn::VersionInfo real_cudnn_version = GetCudnnVersion();
    if (!(cc.IsAtLeast(se::CudaComputeCapability::AMPERE) && cc.minor == 0 &&
          real_cudnn_version >= se::dnn::VersionInfo(8, 9, 0))) {
    ...
    if (std::is_same<T, Eigen::half>::value) {
      ...
      hlo_string =
          GetModuleFMHA_Flash_Attention_BMM1_Bias_Softmax_BMM2_HloString_BF16();
    }
...
XLA_TEST_F(FlashAttentionBMMScaleBiasSoftmaxBMM,
           FMHA_Flash_Attention_BMM1_Bias_Softmax_BMM2_BF16) {
  TestImpl_FMHA_Flash_Attention_BMM1_Bias_Softmax_BMM2<bfloat16>();
}
