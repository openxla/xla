 ...
 protected:
  ...
  GetModuleFlash_Attention_Bwd_BMM1_NoMask_Generation_Softmax_BMM2_HloString_BF16() {  // NOLINT
    ...
    ENTRY main.106 {
      ...
      convert.42 = f8e4m3fn[4,16,4,16]{3,2,1,0} convert(call.41), metadata={op_name="jit(foo)/jit(main)/convert_element_type[new_dtype=float8_e4m3fn weak_type=False]" source_file="/opt/flax/flax/linen/fp8_ops.py" source_line=130}
      convert.43 = bf16[4,16,4,16]{3,2,1,0} convert(convert.42), metadata={op_name="jit(foo)/jit(main)/convert_element_type[new_dtype=bfloat16 weak_type=False]" source_file="/opt/flax/flax/linen/fp8_ops.py" source_line=134}
      Arg_3.4 = bf16[4,16,4,16]{3,2,1,0} parameter(3), metadata={op_name="fwd_output"}
      divide.72 = bf16[4,16,4,16]{3,2,1,0} divide(Arg_3.4, broadcast.7), metadata={op_name="jit(foo)/jit(main)/div" source_file="/opt/flax/flax/linen/fp8_ops.py" source_line=128}
      call.71 = bf16[4,16,4,16]{3,2,1,0} call(divide.72, constant.5, constant.4), to_apply=clip.33
      convert.72 = f8e4m3fn[4,16,4,16]{3,2,1,0} convert(call.71), metadata={op_name="jit(foo)/jit(main)/convert_element_type[new_dtype=float8_e4m3fn weak_type=False]" source_file="/opt/flax/flax/linen/fp8_ops.py" source_line=130}
      convert.73 = bf16[4,16,4,16]{3,2,1,0} convert(convert.72), metadata={op_name="jit(foo)/jit(main)/convert_element_type[new_dtype=bfloat16 weak_type=False]" source_file="/opt/flax/flax/linen/fp8_ops.py" source_line=134}
      Arg_4.5 = bf16[4,16,4,16]{3,2,1,0} parameter(4), metadata={op_name="dO"}
      divide.82 = bf16[4,16,4,16]{3,2,1,0} divide(Arg_4.5, broadcast.7), metadata={op_name="jit(foo)/jit(main)/div" source_file="/opt/flax/flax/linen/fp8_ops.py" source_line=128}
      call.81 = bf16[4,16,4,16]{3,2,1,0} call(divide.82, constant.5, constant.4), to_apply=clip.33
      convert.82 = f8e4m3fn[4,16,4,16]{3,2,1,0} convert(call.81), metadata={op_name="jit(foo)/jit(main)/convert_element_type[new_dtype=float8_e4m3fn weak_type=False]" source_file="/opt/flax/flax/linen/fp8_ops.py" source_line=130}
      convert.83 = bf16[4,16,4,16]{3,2,1,0} convert(convert.82), metadata={op_name="jit(foo)/jit(main)/convert_element_type[new_dtype=bfloat16 weak_type=False]" source_file="/opt/flax/flax/linen/fp8_ops.py" source_line=134}
      bt.0 = f32[4,4,16]{2,1,0} broadcast(constant.99), dimensions={}
      custom-call.7 = (bf16[4,4,16,16]{3,1,2,0}, bf16[4,4,16,16]{3,1,2,0}, bf16[4,4,16,16]{3,1,2,0}, u8[0]{0}) custom-call(convert.19, convert.31, convert.43, bt.0, convert.83, /*index=5*/convert.73), custom_call_target="__cudnn$fmhaSoftmaxBackward", operand_layout_constraints={bf16[4,16,4,16]{3,2,1,0}, bf16[4,16,4,16]{3,2,1,0}, bf16[4,16,4,16]{3,2,1,0}, f32[4,4,16]{2,1,0}, bf16[4,16,4,16]{3,2,1,0}, bf16[4,16,4,16]{3,2,1,0}}, api_version=API_VERSION_STATUS_RETURNING, metadata={op_name="jit(<unnamed wrapped function>)/jit(main)/dot_product_attention_bwd[scale=1.0 seed=42 dropout_rate=0.0 variadic_args=(False, False, False) mask_type=MaskType.NO_MASK layout=0]" source_file="/opt/rosetta/test_num.py" source_line=357}, backend_config={"operation_queue_id": "0", "wait_on_operation_queues": [], "cudnn_fmha_backend_config": {"algorithm": {"algo_id": "0", "math_type": "TENSOR_OP_MATH", "tuning_knobs": {"17": "1", "24": "0"}, "is_cudnn_frontend": true, "workspace_size": "0"}, "fmha_scale": 1.0, "dropout_rate": 0.0, "intermediate_tensor_shape": {"element_type": "BF16", "dimensions": ["4", "4", "16", "16"], "tuple_shapes": [], "layout": {"dim_level_types": [], "dim_unique": [], "dim_ordered": [], "minor_to_major": ["3", "2", "1", "0"], "tiles": [], "element_size_in_bits": "0", "memory_space": "0", "index_primitive_type": "PRIMITIVE_TYPE_INVALID", "pointer_primitive_type": "PRIMITIVE_TYPE_INVALID", "dynamic_shape_metadata_prefix_bytes": "0"}, "is_dynamic_dimension": [false, false, false, false]}, "seed": 42, "is_flash_attention": true, "mask_type": "NO_MASK", "bmm1_grad_gemm1_dot_dimension_numbers": {"lhs_contracting_dimensions": ["2"], "rhs_contracting_dimensions": ["1"], "lhs_batch_dimensions": ["0", "1"], "rhs_batch_dimensions": ["0", "2"]}, "bmm1_grad_gemm2_dot_dimension_numbers": {"lhs_contracting_dimensions": ["3"], "rhs_contracting_dimensions": ["1"], "lhs_batch_dimensions": ["0", "1"], "rhs_batch_dimensions": ["0", "2"]}, "bmm2_grad_gemm1_dot_dimension_numbers": {"lhs_contracting_dimensions": ["2"], "rhs_contracting_dimensions": ["1"], "lhs_batch_dimensions": ["0", "1"], "rhs_batch_dimensions": ["0", "2"]}, "bmm2_grad_gemm2_dot_dimension_numbers": {"lhs_contracting_dimensions": ["3"], "rhs_contracting_dimensions": ["3"], "lhs_batch_dimensions": ["0", "2"], "rhs_batch_dimensions": ["0", "2"]}}}
  ...
  GetModuleFlash_Attention_Bwd_BMM1_NoMask_Generation_Softmax_BMM2_HloString_FP8() {  // NOLINT
    ...
    ENTRY main.106 {
      ...
      convert.83 = bf16[4,16,4,16]{3,2,1,0} convert(convert.82), metadata={op_name="jit(foo)/jit(main)/convert_element_type[new_dtype=bfloat16 weak_type=False]" source_file="/opt/flax/flax/linen/fp8_ops.py" source_line=134}
      bt.0 = f32[4,4,16]{2,1,0} broadcast(constant.99), dimensions={}
      custom-call.9 = (f8e4m3fn[4,4,16,16]{3,1,2,0}, f8e4m3fn[4,4,16,16]{3,1,2,0}, f8e4m3fn[4,4,16,16]{3,1,2,0}, f32[1,1,1,1]{3,2,1,0}, f32[1,1,1,1]{3,2,1,0}, /*index=5*/f32[1,1,1,1]{3,2,1,0}, f32[1,1,1,1]{3,2,1,0}, u8[0]{0}) custom-call(convert.18, convert.30, convert.42, convert.72, convert.82, /*index=5*/bt.0, broadcast.99, broadcast.99, broadcast.99, broadcast.99, /*index=10*/broadcast.99, broadcast.99, broadcast.99, broadcast.99, broadcast.99, /*index=15*/broadcast.99, broadcast.99, broadcast.99), custom_call_target="__cudnn$fmhaSoftmaxBackward$f8", operand_layout_constraints={f8e4m3fn[4,16,4,16]{3,2,1,0}, f8e4m3fn[4,16,4,16]{3,2,1,0}, f8e4m3fn[4,16,4,16]{3,2,1,0}, f8e4m3fn[4,16,4,16]{3,2,1,0}, f8e4m3fn[4,16,4,16]{3,2,1,0}, f32[4,4,16]{2,1,0}, f32[1,1,1,1]{3,2,1,0}, f32[1,1,1,1]{3,2,1,0}, f32[1,1,1,1]{3,2,1,0}, f32[1,1,1,1]{3,2,1,0}, f32[1,1,1,1]{3,2,1,0}, f32[1,1,1,1]{3,2,1,0}, f32[1,1,1,1]{3,2,1,0}, f32[1,1,1,1]{3,2,1,0}, f32[1,1,1,1]{3,2,1,0}, f32[1,1,1,1]{3,2,1,0}, f32[1,1,1,1]{3,2,1,0}, f32[1,1,1,1]{3,2,1,0}}, api_version=API_VERSION_STATUS_RETURNING, metadata={op_name="jit(<unnamed wrapped function>)/jit(main)/dot_product_attention_fp8_bwd[scale=1.0 use_causal_mask=False layout=0]" source_file="/opt/rosetta/test_num.py" source_line=369}, backend_config={"operation_queue_id": "0", "wait_on_operation_queues": [], "cudnn_fmha_backend_config": {"algorithm": {"algo_id": "0", "math_type": "TENSOR_OP_MATH", "tuning_knobs": {"17": "1", "24": "0"}, "is_cudnn_frontend": true, "workspace_size": "0"}, "fmha_scale": 1.0, "intermediate_tensor_shape": {"element_type": "BF16", "dimensions": ["4", "4", "16", "16"], "tuple_shapes": [], "layout": {"dim_level_types": [], "dim_unique": [], "dim_ordered": [], "minor_to_major": ["3", "2", "1", "0"], "tiles": [], "element_size_in_bits": "0", "memory_space": "0", "index_primitive_type": "PRIMITIVE_TYPE_INVALID", "pointer_primitive_type": "PRIMITIVE_TYPE_INVALID", "dynamic_shape_metadata_prefix_bytes": "0"}, "is_dynamic_dimension": [false, false, false, false]}, "is_flash_attention": true, "mask_type": "NO_MASK", "bmm1_grad_gemm1_dot_dimension_numbers": {"lhs_contracting_dimensions": ["2"], "rhs_contracting_dimensions": ["1"], "lhs_batch_dimensions": ["0", "1"], "rhs_batch_dimensions": ["0", "2"]}, "bmm1_grad_gemm2_dot_dimension_numbers": {"lhs_contracting_dimensions": ["3"], "rhs_contracting_dimensions": ["1"], "lhs_batch_dimensions": ["0", "1"], "rhs_batch_dimensions": ["0", "2"]}, "bmm2_grad_gemm1_dot_dimension_numbers": {"lhs_contracting_dimensions": ["2"], "rhs_contracting_dimensions": ["1"], "lhs_batch_dimensions": ["0", "1"], "rhs_batch_dimensions": ["0", "2"]}, "bmm2_grad_gemm2_dot_dimension_numbers": {"lhs_contracting_dimensions": ["3"], "rhs_contracting_dimensions": ["3"], "lhs_batch_dimensions": ["0", "2"], "rhs_batch_dimensions": ["0", "2"]}}}
