...
ENTRY bf16gemm {
  ...
  if (CudaOrRocmCheck(se::CudaComputeCapability::AMPERE, Switch::True)) {
    ...
    MatchOptimizedHlo(hlo_text,
                      R"(
    ; CHECK: [[OUT:%[^ ]+]] = (bf16[3,4,2]{2,1,0}, s8[{{[0-9]+}}]{0}) custom-call(bf16[3,3,4]{2,1,0} [[A:%[^ ]+]], bf16[3,3,2]{2,1,0} [[B:%[^ ]+]]), custom_call_target="<<CUBLAS_CUSTOM_CALL_TARGET_PLACEHOLDER>>"
    )",
                      /*print_operand_shape=*/true);
  } else {
    MatchOptimizedHlo(hlo_text,
...
ENTRY test {
  ...
  EXPECT_THAT(module->entry_computation()->root_instruction(),
              GmockMatch(m::Bitcast(m::GetTupleElement(m::CustomCall(
                                        {"__cublas$lt$matmul"},
                                        m::Parameter(0).WithShape(F32, {2, 3}),
                                        m::Parameter(1).WithShape(F32, {3, 4})), 0))
                             .WithShape(F32, {2, 2, 2})));
}
...
ENTRY test {
  ...
  EXPECT_THAT(
      module->entry_computation()->root_instruction(),
      GmockMatch(m::Bitcast(m::GetTupleElement(m::CustomCall({"__cublas$lt$matmul"},
                                          m::Parameter(0), m::Parameter(1),
                                          m::Parameter(2).WithShape(F32, {2})), 0)
                                .WithShape(F32, {2, 4}))
                     .WithShape(F32, {4, 2})));
}
...
ENTRY test {
  ...
  EXPECT_THAT(
      module->entry_computation()->root_instruction(),
      GmockMatch(
          m::Bitcast(m::GetTupleElement(m::CustomCall(
                         {"__cublas$lt$matmul"},
                         m::Parameter(0).WithShape(BF16, {8, 16}),
                         m::Parameter(1).WithShape(BF16, {16, 8}),
                         m::Bitcast(m::Parameter(2)).WithShape(BF16, {8, 8})), 0))
...
ENTRY test {
  ...
  EXPECT_THAT(
      module->entry_computation()->root_instruction(),
      GmockMatch(m::Tuple(
          m::GetTupleElement(m::CustomCall(m::Parameter(0), m::Parameter(1), m::Parameter()), 0),
          m::GetTupleElement(m::CustomCall(m::Parameter(0), m::Parameter(1), m::Constant()), 0),
          m::GetTupleElement(m::CustomCall(m::Parameter(0), m::Parameter(1), m::Constant()), 0),
          m::GetTupleElement(m::CustomCall(m::Parameter(0), m::Parameter(1), m::Constant()), 0))));
}
...
ENTRY test {
  ...
  for (const auto& type_combination : type_combinations) {
    ...
    TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> optimized_module,
                            GetOptimizedModule(hlo_text));
    EXPECT_THAT(optimized_module->entry_computation()->root_instruction(),
                GmockMatch(m::GetTupleElement(m::CustomCall(m::Parameter(0), m::Parameter(1),
                                         m::Parameter(2)), 0)));
  }
}
...
TEST_P(ParameterizedFp8GemmRewriteTest, ScaledABUnscaledDBitcastF8) {
  ...
  EXPECT_THAT(
      module->entry_computation()->root_instruction(),
      GmockMatch(m::GetTupleElement(
          m::CustomCall({"__cublas$lt$matmul$f8"}), 0).WithShape(F32, {16, 16})));
}
...
; CHECK-PTX-NEXT:    [[OUT:%[^ ]+]] = (<<F8E4M3>>[16,16]{1,0}, s8[{{[0-9]+}}]{0}) custom-call([[P0]], [[P1_TRANSPOSE]], [[C1]], [[C1]], [[C1]], /*index=5*/[[P2_INV]]),
; CHECK-GCN-NEXT:    [[OUT:%[^ ]+]] = f32[16,16]{1,0} custom-call([[P0]], [[P1_TRANSPOSE]], [[C1]], [[C1]], [[C1]], /*index=5*/[[C1]]),
; CHECK:           custom_call_target="__cublas$lt$matmul$f8",
; CHECK:           backend_config={
; CHECK-DAG:         "alpha_real":1
...
TEST_P(ParameterizedFp8GemmRewriteTest, Rank3ScaledABUnscaledDVectorBiasF8) {
  ...
  EXPECT_THAT(module->entry_computation()->root_instruction(),
              GmockMatch(m::Bitcast(m::GetTupleElement(m::CustomCall({"__cublas$lt$matmul$f8"}), 0)
                                        .WithShape(F16, {64, 32}))
...
TEST_P(ParameterizedFp8GemmRewriteTest,
  ...
  EXPECT_THAT(
      module->entry_computation()->root_instruction(),
      GmockMatch(m::Bitcast(m::Slice(m::GetTupleElement(m::CustomCall({"__cublas$lt$matmul$f8"}), 0)
                                         .WithShape(F16, {64, 32}))
                                .WithShape(F16, {60, 31}))
