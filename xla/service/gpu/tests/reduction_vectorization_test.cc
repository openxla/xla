...
TEST_F(ReductionVectorizationNoOptTest, ColumnReductionVectorization) {
  const char* hlo_text = R"(
HloModule ColumnReductionVectorization
...
%fused_computation {
  ...
  %param_1 = f32[2048,16384] parameter(1)
  %broadcast0 = f32[2048,16384] broadcast(%param_0), dimensions={0}
  %multiply0 = f32[2048,16384] multiply(%param_1, %broadcast0)
  %constant0 = f32[] constant(0)
  ROOT %reduce = f32[16384] reduce(%multiply0, %constant0), dimensions={0}, to_apply=%add_f32
}

ENTRY %cluster {
  %param0 = f32[2048] parameter(0)
  %param1 =  f32[2048,16384] parameter(1)
  ROOT %fusion = f32[16384] fusion(%param0, %param1), kind=kInput, calls=%fused_computation
}
)";
...
TEST_F(ReductionVectorizationNoOptTest,
       ColumnReductionVectorizationWithAtomic) {
  const char* hlo_text = R"(
HloModule ColumnReductionVectorizationWithAtomic
...
ENTRY %cluster {
  ...
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> optimized_module,
                          ParseAndReturnVerifiedModule(hlo_text));
  // Check if exists atomic operation.
  std::string expected = R"(
CHECK: ld.global.nc.v2.f32
CHECK: ld.global.nc.v2.f32
CHECK: ld.global.nc.v2.f32
