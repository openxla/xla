...
StatusOr<AutotuneResult> DoGemmAutotuneNoCache(
  ...
  if (IsCublasLtMatmul(*gemm)) {
    ...
    TF_ASSIGN_OR_RETURN(bool has_vector_bias, xla::gpu::gpublas_lt::EpilogueAddsVectorBias(
                                                  gemm_config.epilogue()));

    TF_ASSIGN_OR_RETURN(
        bool has_aux_output,
        xla::gpu::gpublas_lt::EpilogueHasAuxiliaryOutput(gemm_config.epilogue()));
    ...
    TF_ASSIGN_OR_RETURN(auto plan,
                        se::gpu::BlasLt::GetMatmulPlan(stream, config, epilogue));
