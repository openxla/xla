...
absl::Status ExecuteThunks(
  ...
  if (!collective_cliques.empty()) {
    TF_RETURN_IF_ERROR(RendezvousAfterInitialization(run_options, debug_options));
  }
...
absl::Status RendezvousAfterInitialization(
    const ServiceExecutableRunOptions* run_options, const DebugOptions* debug_options) {
  ...
  RendezvousSingle(rendezvous_name, rendezvous_key, num_local_participants,
                   absl::Seconds(debug_options ? debug_options->xla_gpu_executable_warn_stuck_timeout() : 10),
                   absl::Seconds(debug_options ? debug_options->xla_gpu_executable_terminate_timeout() : 30));
