...
absl::Status ExecuteThunksImpl(
  ...
  size_t num_partitipants_requiring_barrier =
      collective_clique_requests
          .NumParticipantsRequiringBarrierAfterModuleExecution(
              collective_params.global_device_id);
  if (num_partitipants_requiring_barrier > 0) {
    XLA_VLOG_DEVICE(1, run_options->device_ordinal())
        << "Barrier after executable required by "
        << num_partitipants_requiring_barrier << " participants";
    RETURN_IF_ERROR(BarrierAfterExecutable(*run_options, debug_options,
                                           *main_stream,
                                           num_partitipants_requiring_barrier));
  }
