...
StatusOr<GpuCompiler::BackendCompileResult> GpuCompiler::CompileToTargetBinary(
  ...
  StatusOr<std::vector<uint8_t>> maybe_backend_result =
      LinkModules(stream_exec, std::move(submodule_compile_results),
                  module_config.debug_options());
...
GpuCompiler::CompileToBackendResult(
    ...
    se::StreamExecutor* executor, se::DeviceMemoryAllocator* allocator,
    const se::DeviceDescription& gpu_device_info) {
  const int64_t scheduler_mem_limit =
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      CompileToTargetBinary(module->config(),
                            compile_module_results.llvm_module.get(),
...
StatusOr<std::unique_ptr<Executable>> GpuCompiler::RunBackend(
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      CompileToBackendResult(module.get(), &llvm_context, stream_exec,
                             options.device_allocator, gpu_device_info));

  RecordXlaDeviceBinarySize(res.backend_result.binary.size());
