...
absl::Status RunDynamicSliceFusionPasses(
    ...
    const se::DeviceDescription& device_description, int64_t pointer_size, const int combine_threshold_count) {
  const DebugOptions& opts = hlo_module->config().debug_options();
...
absl::Status GpuCompiler::OptimizeHloModule(
  ...
  TF_RETURN_IF_ERROR(
      RunPostFusionPasses(hlo_module, device_description, pointer_size_, combine_threshold_count));
