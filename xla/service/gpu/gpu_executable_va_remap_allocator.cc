...
class GpuExecutableVaRemapAllocator::VaRemapExecutionScope
 ...
 public:
  ...
  se::DeviceAddressBase ResolveOutputBuffer(
      ...
      se::DeviceAddressBase current) const override;
 ...
 private:
  ...
  absl::Status OnCopyProtectedOutput(
...
void GpuExecutableVaRemapAllocator::VaRemapExecutionScope::RecordStepAlias(
  ...
  external_address_by_index_[index] = external_address;
}
...
se::DeviceAddressBase
GpuExecutableVaRemapAllocator::VaRemapExecutionScope::ResolveOutputBuffer(
    BufferAllocation::Index index, se::DeviceAddressBase current) const {
  auto it = external_address_by_index_.find(index);
  if (it == external_address_by_index_.end()) {
    return current;
  }
  return it->second;
}
...
absl::StatusOr<se::DeviceAddressBase>
GpuExecutableVaRemapAllocator::VaRemapExecutionScope::AllocateEscapingBuffer(
    int device_ordinal, const BufferAllocation& allocation) {
  ASSIGN_OR_RETURN(uint64_t va_offset,
                   remapping_->GetReservationOffset(allocation.index()));
  ASSIGN_OR_RETURN(uint64_t mapping_size,
...
absl::Status
GpuExecutableVaRemapAllocator::VaRemapExecutionScope::OnCopyProtectedOutput(
  ...
  if (remapping_->phase == Remapping::ProfilePhase::kProfiling) {
    ...
    return Internal(
        ...
        allocation.index(), owner_->module_name());
  }
  return absl::OkStatus();
}
