...
// Returns whether GpuExecutable runs with Experimental Runtime.
...
class GpuExecutable : public Executable {
 public:
  ...
  using OwnedExperimentalRuntimeProgram =
      std::unique_ptr<ExperimentalRuntimeProgram>;
  ...
  struct Params {
    ...
    // (native function) or Experimental runtime executable (IREE VM function)
    // depending on which is supplied.
    std::variant<OwnedThunkSequence, OwnedGpuRuntimeProgram,
                 OwnedExperimentalRuntimeProgram>
        executable;
 ...
 private:
  ...
  // IrEmitter (null if Xla/Experimental runtime is enabled).
  ...
  // Experimental executable that encapsulates all the state for running XLA:GPU
  // executables compiled to IREE VM modules (including VM module itself).
  std::unique_ptr<ExperimentalRuntimeExecutable> experimental_executable_;
