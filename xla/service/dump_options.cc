...
const RE2* GetCachedRE2(const std::string& pattern) {
  static absl::Mutex mu(absl::kConstInit);
  static auto& cache ABSL_GUARDED_BY(mu) =
      *new absl::flat_hash_map<std::string, std::unique_ptr<RE2>>();
...
DumpOptions DumpOptions::Build(const DebugOptions& opts,
  ...
  if (dump_options.dump_hlo_to_subfolder && !dump_options.dump_to.empty() &&
    ...
    static const std::string& pid_hostname_dir = *new std::string(
