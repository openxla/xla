...
absl::StatusOr<bool> BufferAssigner::AssignSpecialHloBuffer(
  ...
  // they get no allocation of their own. The backend injects their color;
  // buffer assignment stays backend-agnostic by matching on it. A mixed color
  // buffer must not silently fall through to normal allocation here (a view
  // that got an allocation would break the aliasing design); the caller
  // already fails on color() before invoking this, so propagate the error.
