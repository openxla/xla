...
HloSharding GetAllToAllSharding(const HloSharding& source_sharding,
  ...
  if (source_sharding.UseNamedShardingLeaf()) {
    NamedSharding new_named_sharding = source_sharding.named_sharding();
    std::vector<NamedSharding::DimensionSharding> new_dim_shardings(
        new_named_sharding.dim_shardings().begin(),
        new_named_sharding.dim_shardings().end());
    for (int64_t i = 0; i < source_dims.size(); ++i) {
      ...
      int64_t target_size = new_named_sharding.dimension(target_dim);
      CHECK_EQ(new_named_sharding.dimension(source_dim) % target_size, 0);
      ...
      std::optional<NamedSharding::DimensionSharding> keep =
          to_move.Slice(new_named_sharding.mesh(), target_size);
      CHECK(keep.has_value());

      src_sharding = *keep;
      tgt_sharding.Append(to_move, new_named_sharding.mesh());
    }
    return HloSharding(NamedSharding(
        new_named_sharding.mesh(), new_dim_shardings,
        new_named_sharding.replicated_axes(),
        new_named_sharding.unreduced_axes(), new_named_sharding.manual_axes(),
        new_named_sharding.metadata()));
  }
