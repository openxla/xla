...
PartitionedHlo MakePartitionedHloMaybeMX(
    PartitionedHlo p, Shape base_shape,
...
PartitionedHloMX MakePartitionedHloMaybeMX(
    PartitionedHloMX p, std::pair<Shape, Shape> base_shapes,
    PartitionedHlo::PartitioningState state) {
...
PartitionedHlo ReplicatePartiallySharded(
    PartitionedHlo partitioned, absl::Span<const int64_t> batch_dims,
    GroupedSharding grouped, SpmdBuilder* b,
    PartitionedHlo::PartitioningState state) {
...
PartitionedHloMX ReplicatePartiallySharded(
    PartitionedHloMX partitioned, absl::Span<const int64_t> batch_dims,
    GroupedSharding grouped, SpmdBuilder* b,
    PartitionedHlo::PartitioningState state) {
  return PartitionedHloMX(
...
std::pair<Shape, Shape> GetPerGroupBaseShape(
    ...
    const PartitionedHloMX::MXShapes& original_base_shapes) {
...
class CreateShardedDotFunctor final
 ...
 public:
  CreateShardedDotFunctor(HloDotInstruction* dot,
                          const std::vector<HloInstruction*>& resharded_meta,
                          const std::vector<SparsityDescriptor>& sparsity)
      : dot_(dot), resharded_meta_(resharded_meta), sparsity_(sparsity) {}
  ...
  absl::StatusOr<HloInstruction*> CreateSharded(
    ...
    return b->AddInstruction(HloInstruction::CreateDot(
        ...
        dot_->precision_config(), sparsity_, resharded_meta_));
  }

 private:
  HloDotInstruction* dot_;
  const std::vector<HloInstruction*>& resharded_meta_;
  const std::vector<SparsityDescriptor>& sparsity_;
};
...
absl::Status SpmdPartitioningVisitor::HandleDot(HloInstruction* hlo) {
  ...
  CreateShardedDotFunctor create_sharded_dot_functor(dot, resharded_meta,
                                                     sparsity);
...
absl::StatusOr<HloInstruction*> PartitionDotGroupOnContractingImpl(
  ...
  std::function<absl::StatusOr<HloInstruction*>(const PartitionedHloMaybeMX&,
    ...
    // call_custom_create_sharded_ to false.
    TF_ASSIGN_OR_RETURN(
        auto inner_dot,
        create_sharded_dot(l, r, b, conv_window,
                           /*call_custom_create_sharded_=*/false));
