...
ENTRY entry {
  ...
  EXPECT_THAT(root, AllOf(op::Shape("f32[4,24,32]"),
                          op::AllReduce(op::DynamicUpdateSlice(
                              _, op::Dot(lhs, rhs), _, _, _))));
}
...
ENTRY entry {
  ...
  const auto lhs = AllOf(op::Shape("f32[6,8,100]"));
...
TEST_P(SpmdPartitioningTest, DotBatchDimPushAllGatherFromLhsToOutput) {
  absl::string_view hlo_string = R"(
HloModule module
...
ENTRY entry {
  ...
  const auto lhs = AllOf(op::Shape("f32[2,24,100]"), op::Parameter(0));
  const auto rhs = AllOf(op::Shape("f32[2,32,100]"),
                         op::DynamicSlice(op::Parameter(1), _, _, _));
  auto dot = AllOf(op::Shape("f32[2,24,32]"), op::Dot(lhs, rhs));
  const auto root = module->entry_computation()->root_instruction();
  EXPECT_THAT(root,
              AllOf(op::AllReduce(op::DynamicUpdateSlice(_, dot, _, _, _)),
                    op::Shape("f32[4,24,32]")));
}

TEST_P(SpmdPartitioningTest, DotBatchDimPushAllGatherFromRhsToOutput) {
  absl::string_view hlo_string = R"(
HloModule module
...
ENTRY entry {
  ...
  const auto lhs = AllOf(op::Shape("f32[2,24,100]"),
                         op::DynamicSlice(op::Parameter(0), _, _, _));
  const auto rhs = AllOf(op::Shape("f32[2,32,100]"), op::Parameter(1));
  auto dot = AllOf(op::Shape("f32[2,24,32]"), op::Dot(lhs, rhs));
  const auto root = module->entry_computation()->root_instruction();
  EXPECT_THAT(root,
              AllOf(op::AllReduce(op::DynamicUpdateSlice(_, dot, _, _, _)),
...
TEST_P(SpmdPartitioningTest, DotNonContractingDimPushAllGatherFromLhsToOutput) {
  absl::string_view hlo_string = R"(
HloModule module
...
ENTRY entry {
  ...
  const auto lhs = AllOf(op::Shape("f32[4,12,100]"), op::Parameter(0));
  const auto rhs = AllOf(op::Shape("f32[4,32,100]"), op::Parameter(1));
  auto dot = AllOf(op::Shape("f32[4,12,32]"), op::Dot(lhs, rhs));
  const auto root = module->entry_computation()->root_instruction();
  EXPECT_THAT(root,
              AllOf(op::AllReduce(op::DynamicUpdateSlice(_, dot, _, _, _)),
                    op::Shape("f32[4,24,32]")));
...
TEST_P(SpmdPartitioningTest, DotNonContractingDimPushAllGatherFromRhsToOutput) {
  absl::string_view hlo_string = R"(
HloModule module
...
ENTRY entry {
  ...
  const auto lhs = AllOf(op::Shape("f32[4,24,100]"), op::Parameter(0));
  const auto rhs = AllOf(op::Shape("f32[4,16,100]"), op::Parameter(1));
  auto dot = AllOf(op::Shape("f32[4,24,16]"), op::Dot(lhs, rhs));
  const auto root = module->entry_computation()->root_instruction();
  EXPECT_THAT(root,
              AllOf(op::AllReduce(op::DynamicUpdateSlice(_, dot, _, _, _)),
                    op::Shape("f32[4,24,32]")));
