...
ENTRY entry {
  ...
  auto root = module->entry_computation()->root_instruction();
  EXPECT_THAT(root, op::Copy(op::Reshape(op::Transpose(op::AllToAll()))));
}

TEST_F(SpmdPartitioningV3Test, TransposeAllToAllNamedSharding) {
  const char* hlo_text = R"(
HloModule module

ENTRY entry {
  p0 = f32[128,128] parameter(0), sharding={mesh[x=2,y=2,z=2] [{x},{y,z}]}
  ROOT p1 = f32[128,128] transpose(p0), dimensions={1,0}, sharding={mesh[x=2,y=2,z=2] [{y},{x,z}]}
}
)";
  TF_ASSERT_OK_AND_ASSIGN(auto module, PartitionComputation(hlo_text, 8));
  auto root = module->entry_computation()->root_instruction();
  EXPECT_THAT(root, op::Transpose(op::Reshape(op::Transpose(op::AllToAll()))));
}
