...
ENTRY entry {
  ...
  ROOT %transpose = f32[38,4,16,38] transpose(%param0.copy),
    dimensions={1,3,0,2}, sharding={devices=[2,1,4,1]<=[2,2,2]T(2,0,1)}
})";
...
ENTRY entry {
  ...
  %p2 = bf16[1,2048,768]{2,1,0} parameter(2), sharding={devices=[1,4,16]<=[4,4,4]T(2,0,1)}
  %reshape.1074 = bf16[4,512,768]{2,1,0} reshape(bf16[1,2048,768]{2,1,0} %p2), sharding={devices=[4,1,16]<=[4,4,4]T(2,0,1)}
...
ENTRY entry {
  ...
  %rhs = bf16[2,1536,512,1] parameter(1)
  %rhs.copy = bf16[2,1536,512,1] copy(rhs),
    sharding={devices=[2,1,2,1,2]<=[2,2,2]T(2,0,1) last_tile_dim_replicate}
  ROOT %convolution = bf16[8,1024,512,1] convolution(lhs.copy, rhs.copy),
    window={size=1x2}, dim_labels=0b1f_1io0->0bf1,
    sharding={devices=[4,1,2,1]<=[8]}
})";
...
ENTRY entry {
  %lhs = f32[16,801,1,1024] parameter(0)
  %lhs.copy = f32[16,801,1,1024] copy(%lhs), sharding={devices=[1,2,1,2]<=[4]}
  %rhs = f32[5,1,1,1024] parameter(1)
  %rhs.copy = f32[5,1,1,1024] copy(%rhs),
    sharding={devices=[1,1,1,2,2]0,2,1,3 last_tile_dim_replicate}
  ROOT %conv = f32[16,801,1,1024] convolution(%lhs.copy, %rhs.copy),
    dim_labels=b01f_01io->b01f,feature_group_count=1024,
    window={size=5x1 pad=2_2x0_0}, sharding={devices=[1,2,1,2]<=[4]}
})";
...
ENTRY entry {
  ...
  %lhs.copy = f32[16,801,1,1024] copy(%lhs), sharding={devices=[1,2,1,2]<=[4]}
  %rhs = f32[16,801,1,1024] parameter(1)
  %rhs.copy = f32[16,801,1,1024] copy(%rhs), sharding={devices=[1,2,1,2]<=[4]}
  ROOT %conv = f32[5,1,1,1024] convolution(%lhs.copy, %rhs.copy),
    dim_labels=f01b_i01o->01bf,batch_group_count=1024,
    window={size=801x1 pad=2_2x0_0},
...
ENTRY entry {
  ...
  %convolution.10 = bf16[16,36,256,16,4,288,1]{6,5,4,3,2,1,0}
    ...
    dim_labels=4f01b23_4i23o01->01b23f4, sharding={devices=[4,1,1,4,2,1,1]<=[2,4,4]T(2,0,1)}
...
ENTRY entry {
  %lhs = f32[32,32,24,4096] parameter(0), sharding={devices=[2,1,1,2]<=[4]}
  %rhs = f32[32,4096,1024] parameter(1), sharding={devices=[2,2,1]<=[4]}
...
ENTRY entry {
  ...
  %input.copy = f32[64,2,250112] copy(%input), sharding={devices=[16,1,2]<=[32]}
  ...
  ROOT %copy = (f32[64,2,10]) tuple(gather), sharding={{devices=[16,1,1,2]<=[32] last_tile_dim_replicate}}
})";
...
ENTRY entry {
  ...
  %p.copy = f32[4,15,4,16] copy(p),
    sharding={devices=[1,1,1,2,4]<=[2,2,2]T(2,0,1) last_tile_dim_replicate}
...
ENTRY entry {
  ...
  %a = f32[4,15,4,16] add(p.copy, p.copy),
    sharding={devices=[1,4,2,1]<=[8]}
  ROOT %c2 = f32[4,15,4,16] copy(a), sharding={devices=[1,1,1,2,4]<=[2,2,2]T(2,0,1) last_tile_dim_replicate}
})";
...
ENTRY entry {
  ...
  %a = f32[4,15,4,15] add(p.copy, p.copy),
    sharding={devices=[1,4,1,2]<=[8]}
  ROOT %c2 = f32[4,15,4,15] copy(a), sharding={devices=[1,1,1,8]<=[2,2,2]T(2,0,1)}
})";
...
ENTRY entry {
  ...
  %a = f32[4,15,4,16] add(p.copy, p.copy),
    sharding={devices=[1,4,1,2]<=[8]}
  ROOT %c2 = f32[4,15,4,16] copy(a), sharding={devices=[1,8,1,1]<=[2,2,2]T(2,0,1)}
})";
...
ENTRY %main.21 {
  ...
  transpose.3172 = f32[19,19]{0,1} transpose(add.3171), dimensions={1,0}, sharding={devices=[2,4]<=[2,2,2]T(2,0,1)}
...
ENTRY %main.21 {
  ...
  transpose.3172 = f32[5,5]{0,1} transpose(add.3171), dimensions={1,0}, sharding={devices=[2,4]<=[2,2,2]T(2,0,1)}
...
ENTRY %main.21 {
  ...
  dot.11597 = f32[4,4]{1,0} dot(multiply.3535, reshape.4221), lhs_contracting_dims={1}, rhs_contracting_dims={0}, sharding={devices=[2,1,8]<=[2,2,4]T(1,2,0) last_tile_dim_replicate}
  ROOT copy.1 = f32[4,4]{1,0} copy(dot.11597), sharding={devices=[2,1,8]<=[2,2,4]T(1,2,0) last_tile_dim_replicate}
}
)";
