...
struct DotConvDimsMapping {
  ...
  std::vector<int64_t> GetLhsBatchDims() const {
    std::vector<int64_t> lhs_batch_dims;
    lhs_batch_dims.reserve(batch_dims.size());
    for (const auto& dim : batch_dims) {
      lhs_batch_dims.push_back(dim.lhs);
    }
    return lhs_batch_dims;
  }

  std::vector<int64_t> GetRhsBatchDims() const {
    std::vector<int64_t> rhs_batch_dims;
    rhs_batch_dims.reserve(batch_dims.size());
    for (const auto& dim : batch_dims) {
      rhs_batch_dims.push_back(dim.rhs);
    }
    return rhs_batch_dims;
  }

  std::vector<int64_t> GetOutputBatchDims() const {
    std::vector<int64_t> output_batch_dims;
    output_batch_dims.reserve(batch_dims.size());
    for (const auto& dim : batch_dims) {
      output_batch_dims.push_back(dim.output);
    }
    return output_batch_dims;
  }

  std::vector<int64_t> GetLhsContractingDims() const {
    std::vector<int64_t> lhs_contracting_dims;
    lhs_contracting_dims.reserve(contracting_dims.size());
    for (const auto& dim : contracting_dims) {
      lhs_contracting_dims.push_back(dim.lhs);
    }
    return lhs_contracting_dims;
  }

  std::vector<int64_t> GetRhsContractingDims() const {
    std::vector<int64_t> rhs_contracting_dims;
    rhs_contracting_dims.reserve(contracting_dims.size());
    for (const auto& dim : contracting_dims) {
      rhs_contracting_dims.push_back(dim.rhs);
    }
    return rhs_contracting_dims;
  }

  std::vector<int64_t> GetLhsNonContractingDims() const {
    std::vector<int64_t> lhs_nc_dims;
    lhs_nc_dims.reserve(lhs_non_contracting_dims.size());
    for (const auto& dim : lhs_non_contracting_dims) {
      lhs_nc_dims.push_back(dim.lhs);
    }
    return lhs_nc_dims;
  }

  std::vector<int64_t> GetRhsNonContractingDims() const {
    std::vector<int64_t> rhs_nc_dims;
    rhs_nc_dims.reserve(rhs_non_contracting_dims.size());
    for (const auto& dim : rhs_non_contracting_dims) {
      rhs_nc_dims.push_back(dim.rhs);
    }
    return rhs_nc_dims;
  }

  std::vector<int64_t> GetOutputLhsNonContractingDims() const {
    std::vector<int64_t> output_lhs_non_contracting_dims;
    output_lhs_non_contracting_dims.reserve(lhs_non_contracting_dims.size());
    for (const auto& dim : lhs_non_contracting_dims) {
      output_lhs_non_contracting_dims.push_back(dim.output);
    }
    return output_lhs_non_contracting_dims;
  }

  std::vector<int64_t> GetOutputRhsNonContractingDims() const {
    std::vector<int64_t> output_rhs_non_contracting_dims;
    output_rhs_non_contracting_dims.reserve(rhs_non_contracting_dims.size());
    for (const auto& dim : rhs_non_contracting_dims) {
      output_rhs_non_contracting_dims.push_back(dim.output);
    }
    return output_rhs_non_contracting_dims;
  }
};
