...
std::optional<GroupedSharding> AlignGroupsWithInternal(
  ...
  if (matching_groups && !grouped_sharding.sharding.IsTileMaximal()) {
    auto tiles = [&] {
      auto& src_iota = grouped_sharding.sharding.tile_assignment().iota();
      auto& ref_iota = reference.sharding.tile_assignment().iota();
      if (src_iota && ref_iota) {
        return HloSharding::TileAssignment(src_iota->dims(),
                                           ref_iota->reshape_dims(),
                                           ref_iota->transpose_perm());
      }
      auto array =
      ...
      return HloSharding::TileAssignment(std::move(array));
    }();
...
HloInstruction* PerGroupSliceFromReplicated(
  ...
  auto group_level_tile = [&] {
    ...
    return HloSharding::TileAssignment(group_level_tile_dims, group_dim_sizes,
                                       transpose_perm);
  }();
