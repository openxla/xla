...
// Convert the `oldSharding` into a `xla::HloSharding`.
HloSharding convertToHloSharding(
    TensorShardingAttr oldSharding,
    std::function<MeshAttr(TensorShardingAttr)> getMeshAttr,
    ArrayRef<AxisRefAttr> manualAxes = {}) {
  MeshAttr mesh = getMeshAttr(oldSharding);

  SmallVector<int64_t> tileAssignmentDims(oldSharding.getRank(), 1);
  ...
  for (auto [index, dimSharding] :
       llvm::enumerate(oldSharding.getDimShardings())) {
    for (AxisRefAttr axisRef : dimSharding.getAxes()) {
  ...
  SmallVector<AxisRefAttr> meshAxisRefs =
      getOrderedAxisRefs(oldSharding.getDimShardings(), mesh);
  ...
  for (int64_t argNum = 0; argNum < funcOp.getNumArguments(); ++argNum) {
    if (auto oldSharding = funcOp.getArgAttrOfType<TensorShardingAttr>(
      ...
      funcOp.setArgAttr(
          ...
          getStringAttr(convertToHloSharding(oldSharding, getMeshAttr)));
      funcOp.removeArgAttr(argNum, kShardingAttr);
    }
  }

  for (int64_t resNum = 0; resNum < funcOp.getNumResults(); ++resNum) {
    if (auto oldSharding = funcOp.getResultAttrOfType<TensorShardingAttr>(
...
StringAttr convertToHloShardingAttr(
  ...
  SmallVector<AxisRefAttr> manualAxisRefs;
  llvm::transform(manualAxes, std::back_inserter(manualAxisRefs),
                  [&](StringAttr manualAxis) {
                    return AxisRefAttr::get(op->getContext(), manualAxis);
                  });
  ...
  if (op->getNumResults() == 1) {
    ...
    return getStringAttr(
        convertToHloSharding(sdySharding, getMeshAttr, manualAxisRefs));
  }
