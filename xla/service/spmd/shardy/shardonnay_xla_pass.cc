...
#include "third_party/openxla/shardy/src/shardy/common/file_utils.h"
#include "third_party/openxla/shardy/src/shardy/dialect/sdy/transforms/propagation/passes.h"
#include "third_party/tensorflow/compiler/mlir/tensorflow/utils/error_util.h"
#include "third_party/tensorflow/compiler/mlir/utils/array_container_utils.h"
...
// in google internal xla/tests/fuzzing/hlo_fuzzer_utils.cc.
...
void removeFrontendAttributes(HloModule* hloModule,
  ...
  proto2::Map<std::string, std::string>* map = feAttrs.mutable_map();
...
absl::StatusOr<bool> ShardonnayXLA::Run(
  ...
  mlir::StatusScopedDiagnosticHandler diagnosticHandler(mlirContext.get());
  if (failed(pm.run(*mlirModule))) {
    return diagnosticHandler.ConsumeStatus();
  }

  // MLIR MHLO -> HLO
  HloProto hloProto;
  RETURN_IF_ERROR(ConvertMlirHloToHlo(*mlirModule, &hloProto, useTupleArgs,
                                      /*return_tuple=*/false));
  RETURN_IF_ERROR(
