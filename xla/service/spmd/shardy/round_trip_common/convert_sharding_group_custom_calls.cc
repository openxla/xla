...
std::optional<int64_t> getShardingGroupId(Operation* op) {
  if (hasFrontendAttr(op, kShardingGroupIdAttr)) {
    return parseStringAttr<IntegerAttr>(getFrontendAttrs(op),
                                        kShardingGroupIdAttr)
        .getInt();
  }
  return std::nullopt;
}
...
class ShardingGroupCustomCallPattern
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    if (op->getNumResults() > 1) {
      op.emitError() << "expected dangling CustomCallOp with no results or one "
                        "unused result.";
      return mlir::failure();
    }

    std::optional<int64_t> shardingGroupId = getShardingGroupId(op);
