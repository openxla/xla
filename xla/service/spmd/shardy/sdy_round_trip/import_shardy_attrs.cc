...
HloSharding parseShardingFromString(const StringAttr& sharding) {
  absl::StatusOr<xla::HloSharding> shardingCpp =
      xla::ParseSharding(sharding.str());
  CHECK(shardingCpp.ok());
  return *shardingCpp;
}
...
void convertShardyAttrs(FuncOp funcOp, IRRewriter& rewriter,
  ...
  funcOp.front().walk([&](Operation* op) {
    ...
    // No existing frontend attributes to import.
    ...
    // No existing shardings to import.
    if (enableHloShardingV3 &&
 ...
 public:
  ...
  void runOnOperation() final {
    ...
    if (enableHloShardingV3) {
      ...
      // TODO: This might cause issues if a mesh is specifically inlined we will
      // lift it, although I don't see any reason to keep inlined mesh
