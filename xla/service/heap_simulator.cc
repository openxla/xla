...
GlobalDecreasingSizeBestFitHeap<BufferType>::SlicedAllocationFinder::
    SlicedAllocationFinder(
        const std::vector<FreeChunks>& free_chunks_per_slice_time,
        std::vector<int64_t> sorted_slice_sizes, int64_t max_colocation_size,
        int64_t preferred_offset,
        IsAllocationOffsetAllowedFn is_allocation_offset_allowed_fn)
    : sorted_slice_sizes_(std::move(sorted_slice_sizes)),
      ...
      is_allocation_offset_allowed_fn_(
          std::move(is_allocation_offset_allowed_fn)) {
  ...
  // Build free_chunks_.
  for (int64_t free_chunk_slice_time = LatestSliceTime();
       free_chunk_slice_time >= EarliestSliceTime(); --free_chunk_slice_time) {
    // At the latest slice time, initialize FreeChunkRoots.
    if (free_chunk_slice_time == LatestSliceTime()) {
    ...
    // free_chunks_ has already been initialized with roots. We are now slicing
    // the space of each root according to when each subset of that root space
    // is available.
    //
...
GlobalDecreasingSizeBestFitHeap<BufferType>::SlicedAllocationFinder::Find()
  ...
  auto heap_next = [&]() -> const FreeChunkRoot* {
    ...
    std::pop_heap(root_heap.begin(), root_heap.end(), heap_cmp);
    const FreeChunkRoot* root = root_heap.back();
    root_heap.pop_back();
    return root;
  };
  std::make_heap(root_heap.begin(), root_heap.end(), heap_cmp);
  // Each call to heap_next() gives us the next smallest root.
  for (const FreeChunkRoot* root = heap_next(); root; root = heap_next()) {
...
Status GlobalDecreasingSizeBestFitHeap<BufferType>::SlicedAllocationFinder::
    DoesPermutationFitImpl(
        ...
        const FreeChunkRoot& root, int64_t offset) const {
  if (!is_allocation_offset_allowed_fn_(offset)) {
    return FailedPrecondition(
        "%s",
        absl::StrCat("Sliced allocation is not allowed starting at offset ",
                     offset, "."));
  }
...
class SliceTimePermuationIterator {
 public:
  explicit SliceTimePermuationIterator(int64_t latest_slice_time)
  ...
  void Next() {
    ...
    done_ = !std::next_permutation(permutation_.begin(), permutation_.end());
  }
...
GlobalDecreasingSizeBestFitHeap<BufferType>::SlicedAllocationFinder::FindInRoot(
  ...
  for (int64_t offset = first_offset; offset + max_colocation_size_ <= last_end;
       ++offset) {
    for (SliceTimePermuationIterator permutation_it(LatestSliceTime());
    ...
    if (root.pieces.size() == 1 && offset == root.chunk.offset) {
      break;
    }
  }
...
GlobalDecreasingSizeBestFitHeap<BufferType>::SlicedAllocationFinder::
  ...
  ChunksSortedBySliceTime chunks(permutation_of_slice_times.size(),
  ...
  DCHECK(std::all_of(chunks.begin(), chunks.end(),
                     [](const Chunk& chunk) { return chunk.offset >= 0; }));
...
GlobalDecreasingSizeBestFitHeap<BufferType>::MakeFreeChunks(
  ...
  auto subtract_used_chunks = [&](const std::vector<Chunk>& used_chunks) {
    for (const Chunk& used_chunk : used_chunks) {
      ...
      // Subtract from free chunk containing start of used range, removing if
      // it becomes too small for the buffer.
...
GlobalDecreasingSizeBestFitHeap<BufferType>::FindChunkCandidates(
  ...
  // Get all collocated buffers and gather all interferenced chunks.
  ...
  // TODO(b/275905276): when slicing, merge the free_chunks for each slice
  // time. The end result should be a list of free chunks in which
  // buffer_interval not only fits in each free chunk, but the slices of
  // buffer interval can be allocated according to their requirements. Try to
  // find a large enough free chunk containing the preferred offset.
  ...
  if (it == free_chunks.end() || (it->second < chunk.chunk_end())) {
    // Otherwise, find the smallest free chunk. In the case of a tie, prefer
    // the smallest offset. We ensure above that all of the free chunks are
    // large enough to store the buffer.
...
void GlobalDecreasingSizeBestFitHeap<BufferType>::CommitChunk(
  ...
  // returned is the maximum of all colocations, so use this value to update
  // the heap size.
  ...
  for (auto colocation : GetTransitiveColocations(buffer_interval)) {
    ...
    // Create a colocation chunk with the same offset but with the correct
    // size of the colocated interval in case the colocations are of different
    // sizes.
...
ConstrainedGlobalDecreasingSizeBestFitHeap::Finish() {
  ...
  // Use do-while here, because we need to create 1 heap in
  // `multi_heap_result` even if `sorted_buffer_intervals` is empty.
