...
void GlobalDecreasingSizeBestFitHeap<BufferType>::SlicedBufferInterval::Slice(
    const std::vector<int64_t>& slice_sizes_sorted_by_offset) {
  if (slice_sizes_sorted_by_offset.empty()) {
    min_slice_size_ = const_full_buffer_interval_.size;
    slice_sizes_sorted_by_offset_ = {const_full_buffer_interval_.size};
    make_free_chunks_intervals_ = {const_full_buffer_interval_};
    return;
  }

  min_slice_size_ = *std::min_element(slice_sizes_sorted_by_offset.begin(),
                                      slice_sizes_sorted_by_offset.end());
  slice_sizes_sorted_by_offset_ = slice_sizes_sorted_by_offset;
  ...
  for (int i = 0; i < num_slices; ++i) {
    ...
    make_free_chunks_intervals_.push_back(BufferInterval{
        const_full_buffer_interval_.buffer,
        /*size=*/
        (i == num_slices - 1 ? const_full_buffer_interval_.size
                             : min_slice_size_),
        /*start=*/0,
        /*end=*/const_full_buffer_interval_.end,
        /*colocations=*/
        (i == num_slices - 1 ? const_full_buffer_interval_.colocations
                             : empty_colocations),
        const_full_buffer_interval_.need_allocation});
  }

  CHECK_EQ(size_total, const_full_buffer_interval_.size);
}
...
GlobalDecreasingSizeBestFitHeap<
  ...
  return const_full_buffer_interval_;
}
...
GlobalDecreasingSizeBestFitHeap<BufferType>::SlicedBufferInterval::
  ...
  CHECK_LT(slice_time, NumSlices());
  return make_free_chunks_intervals_[slice_time];
}

template <typename BufferType>
GlobalDecreasingSizeBestFitHeap<BufferType>::SlicedBufferInterval::
    SlicedBufferInterval(const BufferInterval& const_full_buffer_interval,
                         BufferInterval* mutable_full_buffer_interval)
    : const_full_buffer_interval_(const_full_buffer_interval),
...
std::string GlobalDecreasingSizeBestFitHeap<
  ...
  return absl::StrCat(
      "{ full_buffer_interval: ", const_full_buffer_interval_.ToString(), ", ",
...
GlobalDecreasingSizeBestFitHeap<BufferType>::FindChunkCandidates(
  ...
  free_chunks_per_slice_time.reserve(sliced_buffer_interval.NumSlices());
  for (int slice_time = 0; slice_time < sliced_buffer_interval.NumSlices();
       ++slice_time) {
    int64_t slice_interval_max_colocation_size = -1;
    if (slice_time == sliced_buffer_interval.NumSlices() - 1) {
      // We don't need to account for colocation until the last slice time,
      // in which we've allocated all the slices.
      slice_interval_max_colocation_size = max_colocation_size;
    }
    free_chunks_per_slice_time.push_back(MakeFreeChunks(
        sliced_buffer_interval.IntervalForMakeFreeChunks(slice_time),
        slice_interval_max_colocation_size));
  }
  ...
  CHECK_EQ(chunks.size(), sliced_buffer_interval.NumSlices() + 1);
