...
TEST_F(HloCseTest, Iota) {
  ...
  ASSERT_OK_AND_ASSIGN(auto m, ParseAndReturnVerifiedModule(hlo_string));
  HloCSE cse_layout_insensitive(/*is_layout_sensitive=*/false);
  ASSERT_OK_AND_ASSIGN(bool changed_insensitive,
                       RunHloPass(&cse_layout_insensitive, m.get()));
  EXPECT_FALSE(changed_insensitive);
...
TEST_F(HloCseTest, IotaDifferentSharding) {
  const char* const hlo_string = R"(
    HloModule m

    ENTRY entry {
      p0 = s64[16,16] parameter(0)
      p1 = s64[16,16] parameter(1)
      i1 = s64[16,16] iota(), iota_dimension=0, sharding={replicated}
      i2 = s64[16,16] iota(), iota_dimension=0, sharding={maximal device=0}
      i3 = s64[16,16] iota(), iota_dimension=0, sharding={maximal device=0}
      add1 = s64[16,16] add(p0, i1)
      add2 = s64[16,16] add(p0, i2)
      add3 = s64[16,16] add(p1, i3)
      ROOT root = (s64[16,16], s64[16,16], s64[16,16]) tuple(add1, add2, add3)
    })";
  ...
  const HloInstruction* add1 = root->operand(0);
  const HloInstruction* add2 = root->operand(1);
  const HloInstruction* add3 = root->operand(2);
  EXPECT_NE(add1->operand(1), add2->operand(1));
  EXPECT_EQ(add2->operand(1), add3->operand(1));
}
