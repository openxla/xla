...
absl::Status Autotuner::Autotune(HloModule* module,
  ...
  VLOG(1) << "Finding configs for " << all_instruction_groups.size()
          << " unique instructions.";

  TF_ASSIGN_OR_RETURN(std::vector<Config> configs,
                      GetConfigsForAll(all_instruction_groups));

  for (int i = 0; i < all_instruction_groups.size(); i++) {
    auto& instructions = all_instruction_groups[i];
...
std::vector<Autotuner::InstructionGroup> Autotuner::GetAutotuningCandidates(
  ...
  std::vector<std::pair<tsl::Fprint128, InstructionGroup>> sorted_groups;
  for (auto& [fingerprint, nodes] : grouped_instructions) {
    sorted_groups.push_back({fingerprint, std::move(nodes)});
  }
  ...
  result.reserve(sorted_groups.size());
  for (auto& pair : sorted_groups) {
