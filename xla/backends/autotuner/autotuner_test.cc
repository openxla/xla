...
TEST_F(AutotunerTest, DumpLogsToFileUniquePath) {
  ASSERT_OK_AND_ASSIGN(
      tsl::testing::TemporaryDirectory temp_dir,
      tsl::testing::TemporaryDirectory::CreateForCurrentTestcase());
  config_.dump_logs_to =
      tsl::io::JoinPath(temp_dir.path(), "dump._unique_.log");
  ...
  // Find the written log file (since _unique_ was replaced).
  std::vector<std::string> files;
  EXPECT_THAT(tsl::Env::Default()->GetChildren(temp_dir.path(), &files),
              absl_testing::IsOk());
  ASSERT_EQ(files.size(), 1);
  EXPECT_THAT(files[0], MatchesRegex("dump\\.[0-9a-f]+\\.log"));
  std::string content;
  EXPECT_THAT(tsl::ReadFileToString(
                  tsl::Env::Default(),
                  tsl::io::JoinPath(temp_dir.path(), files[0]), &content),
              absl_testing::IsOk());
  AutotuningLogs actual_logs;
  EXPECT_TRUE(
      tsl::protobuf::TextFormat::ParseFromString(content, &actual_logs));
  EXPECT_EQ(actual_logs.logs_size(), 1);
}
