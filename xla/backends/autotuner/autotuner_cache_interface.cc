...
AutotuneCacheContext AutotuneCacheContext::Create(
  ...
  AutotuneCacheContext ctx;
  ctx.device = ComputeDeviceName(device_description);
  ctx.explicit_version = std::move(explicit_version);
  ctx.codegen_version = std::move(codegen_version);
  ctx.per_backend_versions = std::move(per_backend_versions);
  return ctx;
}
