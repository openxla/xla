...
class Thunk {
 public:
  ...
  static absl::StatusOr<std::unique_ptr<Thunk>> FromString(
...
class ThunkSequence : public std::vector<std::unique_ptr<Thunk>> {
 public:
  ...
  // TODO(basioli): This will probably require more arguments to actually be
  // able to deserialize.
  static absl::StatusOr<std::unique_ptr<ThunkSequence>> FromString(
      const std::string& serialized);
