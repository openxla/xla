...
static void Parallelize2D(  // NOLINT
  ...
  ParallelLoopRunner::Task2D task = [function, context](size_t offset_i,
                                                        size_t offset_j) {
    (*function)(context, offset_i, offset_j);
  };
  Cast(threadpool)->runner()->Parallelize2D(range_i, range_j, std::move(task));
}
...
extern "C" void pthreadpool_parallelize_2d(pthreadpool_t threadpool,
  ...
  xla::cpu::Parallelize2D(threadpool, function, context, range_i, range_j,
                          flags);
}
