...
struct ParallelLoopRunner::ParallelTask2D {
  ABSL_ATTRIBUTE_ALWAYS_INLINE void operator()(size_t task_index) const {
    size_t i = task_index / range_j;
    size_t j = task_index % range_j;
    task(i, j);
  }
...
void ParallelLoopRunner::Parallelize2D(size_t range_i, size_t range_j,
                                       Task2D task) {
  DCHECK(done_event_) << "Parallel loop runner is in moved-from state";
  DCHECK_GT(range_i, 0) << "Expected at least one `i` task";
  DCHECK_GT(range_j, 0) << "Expected at least one `j` task";

  size_t num_tasks = range_i * range_j;
  ...
  if (ABSL_PREDICT_TRUE(num_tasks == 1)) {
    ...
    // Schedule task when done event becomes available.
    ScheduleOne([task = std::move(task)] { task(0, 0); });
    return;
  }

  ScheduleAll(num_tasks, ParallelTask2D{range_i, range_j, std::move(task)});
}
