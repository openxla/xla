...
declare float @xla.tanh.f32(float)
...
loop.body:
  ...
  %tanh.val = call fast float @xla.tanh.f32(float %val)
; check that all calls have been inlined
; CHECK-NOT: {{.*}}call{{.*}}xla.tanh
; Check that we see inline float vectorized here automatically 
; CHECK: fm{{.*}}<{{(4|8|16)}} x float>{{.*}}0x3E
