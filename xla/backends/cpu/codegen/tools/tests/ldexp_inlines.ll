...
; Checks that when we emit xla.ldexp, it gets inlined if its vectorized.
...
; CHECK: fmul <{{[0-9]+}} x double> %wide.load8, splat (double 8.000000e+00)

; Check that the loop epilogue still calls original uninlined function.
; CHECK: scalar.ph:
; CHECK: tail call double @xla.ldexp.f64.i32
