...
TEST(IrCompilerTest, OverrideIrCompilerCompileOptions) {
  ...
  auto add_module_with_options =
    ...
    llvm_compilation_options[modules.back().get()] = options;
  ...
  ir_compiler->SetCompilationOptionsOverrides(llvm_compilation_options);
  ...
  // compilation options.
  EXPECT_NE(llvm_ir::DumpToString(modules[0].get()),
            llvm_ir::DumpToString(modules[1].get()));
}
