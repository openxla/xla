...
triton_computation {
  ...
  c1 = f32[] constant(1)
  slice = f32[1,1] slice(abs), slice={[0:1:1], [0:1:1]}
  reshape = f32[] reshape(slice)
  add = f32[] add(reshape, c1)
  bcast = f32[64,64] broadcast(add), dimensions={}
  result = add(abs, bcast)
  ROOT tuple = (f32[64,64], f32[64,64]) tuple(result, abs)
}

ENTRY main {
  param_0.1 = f32[64,64]{1,0} parameter(0)
  ROOT res = (f32[64,64]{1,0}, f32[64,64]{1,0}) fusion(param_0.1), kind=kCustom, calls=triton_computation, backend_config={"fusion_backend_config":{"kind":"__triton","block_level_fusion_config":{"output_tiles":[{"sizes":["2","4"]},{"sizes":["2","4"]}],"num_warps":"2"}}}
})";
  TF_EXPECT_OK(
      CreateTritonIrAndFileCheck(this, kHloText, "triton_computation", R"(
CHECK: %[[LOAD:.*]] = tt.load
CHECK: %[[ABS:.*]] = math.absf %[[LOAD]] : tensor<2x4xf32>
CHECK: %[[LOAD2:.*]] = tt.load
CHECK: math.absf %[[LOAD2]] : tensor<1x1xf32>
CHECK: %[[ADD:.*]] = arith.addf %[[ABS]], %{{.*}}
CHECK: tt.store %{{.*}}, %[[ADD]]
CHECK: tt.store %{{.*}}, %[[ABS]]
)"));
  EXPECT_TRUE(RunAndCompareNoHloPasses(
...
triton_computation {
  param_0 = f32[15,7,3]{2,1,0} parameter(0)
  abs = f32[15,7,3]{2,1,0} abs(param_0)
  transpose = f32[3,15,7]{2,1,0} transpose(abs), dimensions={2,0,1}
  ROOT res = (f32[3,15,7]{2,1,0}, f32[15,7,3]{2,1,0}) tuple(transpose, abs)
}

ENTRY main {
  param_0 = f32[15,7,3]{2,1,0} parameter(0)
  ROOT triton_fusion = (f32[3,15,7]{2,1,0}, f32[15,7,3]{2,1,0}) fusion(param_0),
    kind=kCustom, calls=triton_computation,
    backend_config={"fusion_backend_config":
      {"kind":"__triton",
      "block_level_fusion_config":{"output_tiles":[{"sizes":["1","8","4"]}, {"sizes":["4","8","1"]}],
                                   "num_warps":"1"}}}
})";
  TF_EXPECT_OK(
      CreateTritonIrAndFileCheck(this, kHloText, "triton_computation", R"(
