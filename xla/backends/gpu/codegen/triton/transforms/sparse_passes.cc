...
#include "third_party/triton/include/triton/Dialect/TritonGPU/IR/Types.h"
...
namespace {
int64_t getSwizzlingFromLayout(
    const triton::gpu::SwizzledSharedEncodingAttr &layout,
...
Value smemDescriptor(int a, int b, ConversionPatternRewriter &rewriter,
                     ...
                     int minor) {
  auto sharedLayout = cast<SwizzledSharedEncodingAttr>(tensorTy.getEncoding());
...
LogicalResult convertSparseWGMMA(SparseDotOp op, SparseDotOp::Adaptor adaptor,
  ...
  auto getSharedData = [&](Value arg, triton::gpu::MemDescType tensorTy) {
    auto sharedObj = getSharedMemoryObjectFromStruct(
        ...
        rewriter);
    auto sharedLayout =
        cast<SwizzledSharedEncodingAttr>(tensorTy.getEncoding());
