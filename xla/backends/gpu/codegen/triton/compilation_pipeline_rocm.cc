...
static void MakeLLIR(mlir::OpPassManager* pm,
                     const stream_executor::RocmComputeCapability& rocm_cc,
                     int num_stages) {
  pm->addPass(mlir::createTritonAMDGPUUpdateAsyncWaitCount({rocm_cc.gfx_version()}));
  pm->addPass(
  ...
  pm->addPass(
      mlir::triton::AMD::createTritonAMDGPUConvertWarpSpecializeToLLVMPass(rocm_cc.gfx_version()));
  pm->addPass(mlir::createCanonicalizerPass());
  pm->addPass(mlir::createCSEPass());
