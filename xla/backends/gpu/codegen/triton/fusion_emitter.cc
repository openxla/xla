...
ScalarOrTensor EmitParameterExtract(EmitterLocOpBuilder b,
  ...
  auto memref_type = mlir::cast<mlir::MemRefType>(arg.getType());
  if (tile_info.padded_tile_sizes().empty()) {
    auto ptr_type = triton::GetPointerType(memref_type);
    mlir::Value cast_to_ptr = b.create<mtx::MemrefToPtrOp>(ptr_type, arg);
    return ScalarOrTensor(ttir::LoadOp::create(
        b, cast_to_ptr, ttir::CacheModifier::NONE, ttir::EvictionPolicy::NORMAL,
        /*isVolatile=*/false));
  }

  return ScalarOrTensor(xla::xtile::ExtractTileOp::create(
      b,
      mlir::RankedTensorType::get(tile_info.padded_tile_sizes(),
                                  tile_info.storage_type()),
      arg, tile_info.offsets(), tile_info.padded_tile_sizes(),
...
absl::StatusOr<std::vector<ScalarOrTensor>> EmitTiledComputation(
    ...
    const TiledHloComputation& tiled_computation, ::xla::xtile::EntryFuncOp fn,
    Value pid,
    absl::flat_hash_map<const TiledHloInstruction*, ScalarOrTensor>& values);
...
absl::StatusOr<ScalarOrTensor> EmitDot(
    ...
    const se::DeviceDescription& device_info,
    const HloFusionInstruction* fusion,
    const TiledHloInstruction& tiled_hlo_dot, ::xla::xtile::EntryFuncOp fn,
    Value pid,
    absl::flat_hash_map<const TiledHloInstruction*, ScalarOrTensor>& values) {
...
absl::StatusOr<ScalarOrTensor> EmitConcatenate(
    ...
    const se::DeviceDescription& device_info,
    const HloFusionInstruction* fusion,
    const TiledHloInstruction& tiled_concatenate, ::xla::xtile::EntryFuncOp fn,
    Value pid,
    absl::flat_hash_map<const TiledHloInstruction*, ScalarOrTensor>& values) {
...
absl::StatusOr<ScalarOrTensor> EmitTiledHloInstruction(
    ...
    ::xla::xtile::EntryFuncOp fn, Value pid,
    absl::flat_hash_map<const TiledHloInstruction*, ScalarOrTensor>& values) {
...
absl::Status EmitGeneric(mlir::OpBuilder builder,
  ...
  for (auto [root, result, arg] :
    ...
    if (result.IsScalar()) {
      ...
      auto ptr_type = triton::GetPointerType(memref_type);
      mlir::Value cast_to_ptr = b.create<mtx::MemrefToPtrOp>(ptr_type, arg);
      ttir::StoreOp::create(b, cast_to_ptr, result.UnwrapScalar(),
                            /*mask=*/nullptr);
      continue;
    }
    ...
    ::xla::xtile::InsertTileOp::create(
        b, result.UnwrapTensor(), arg, tile_info.offsets(),
...
mlir::MemRefType GetMemRefType(const Shape& shape, mlir::Type element_type) {
  ...
  auto memory_space = mlir::NVVM::NVVMMemorySpaceAttr::get(
      context, mlir::NVVM::NVVMMemorySpace::Global);
...
absl::StatusOr<mlir::OwningOpRef<mlir::ModuleOp>> EmitXTileModule(
  ...
  auto fn = b.create<::xla::xtile::EntryFuncOp>(fn_name, fn_arg_types);
  ...
  b.create<::xla::xtile::EntryFuncReturnOp>();
