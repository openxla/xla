...
absl::StatusOr<TensorValue> EmitAllReduce(
  ...
  auto channel_handle = all_reduce.channel_id();
  auto use_global_device_ids = all_reduce.use_global_device_ids();
...
absl::StatusOr<TensorValue> EmitTiledHloInstruction(
  ...
  if (hlo->opcode() == HloOpcode::kAllReduceStart) {
    ...
    return EmitAllReduce(
        b, computation,
        *xla::Cast<HloAllReduceInstruction>(computation->root_instruction()),
        tiled_hlo, block_level_parameters, fn, pid, values);
  }
