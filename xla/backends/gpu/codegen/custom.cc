...
std::unique_ptr<HloModule> ExtractWhileUpdateModule(
  ...
  ModuleCallInliner module_inliner(while_op->while_body()->parent());
  ...
  VLOG(2) << "While loop induction var tuple index " << tuple_idx.value()
          << ", While update value: \n"
          << update->ToString();
...
std::unique_ptr<HloModule> ExtractWhileInitModule(
  ...
  ModuleCallInliner module_inliner(while_op->parent()->parent());
  auto while_op_mapped = module_inliner.get_mapped_instruction(while_op);
  CHECK(while_op_mapped != nullptr)
...
absl::Status CollectSliceInfo(
  ...
  VLOG(2) << "Collecting slice info for arg idx: " << arg_idx
          << " slice instr: " << slice_instrs[arg_idx]->ToString();
  ...
  std::optional<std::unique_ptr<ModuleCallInliner>> module_inliner =
      std::nullopt;
  std::optional<HloInstruction*> fusion_instr_mapped;
  std::optional<HloInstruction*> async_caller_mapped = std::nullopt;
  module_inliner =
      std::make_unique<ModuleCallInliner>(fusion_instr.parent()->parent());
  ...
  for (auto idx_op : arg_slice_instr->index_operands()) {
    ...
    VLOG(2) << "Index operand: \n"
            << idx_op->ToString() << "\n mapped to instruction :\n"
            << offset_value_mapped->ToString();
...
absl::StatusOr<FusionEmissionResult> DynamicSliceFusion::Emit(
  ...
  VLOG(1) << "Emit DynamicSliceFusion for fusion instruction: \n"
          << fusion.ToString();
  VLOG(1) << "Original module: \n" << fusion.GetModule()->ToString();
