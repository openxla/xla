...
std::unique_ptr<HloModule> ExtractSliceModule(const HloInstruction* slice_instr,
                                              int64_t indvar_idx) {
  const HloInstruction* offset_value = slice_instr->operand(indvar_idx);
...
absl::Status CollectSliceInfo(
  ...
  for (auto idx_op : arg_slice_instr->index_operands()) {
    ...
    if (offset_literal.ok()) {
      ...
      std::unique_ptr<HloModule> offset_module =
          ExtractSliceModule(arg_slice_instr, indvar_idx.value());
      if (offset_module != nullptr) {
    ...
    sliced_shapes[arg_idx] =
        DynCast<HloDynamicSliceInstruction>(arg_slice_instr)
  ...
  GetResultSliceForPartiallyUnnestedTuple(
      const BufferAssignment& buffer_assignment,
      ...
      switch (user->opcode()) {
        ...
        default:
          return absl::InternalError(absl::StrCat(
              "Unexpected opcode while processing the epilogue of a "
              "DynamicSliceFusion: ",
  ...
  absl::StatusOr<FusionEmissionResult> EmitGemm(
    ...
    if (absl::c_any_of(slice_instrs, IsDynamicSliceOrDynamicUpdateSlice)) {
      ...
      BufferAllocation::Slice slice_lhs_fake(
          fake_allocations[fake_arg_idx].get(), 0, lhs_byte_size);
      ...
      BufferAllocation::Slice slice_rhs_fake(
          fake_allocations[fake_arg_idx].get(), 0, rhs_byte_size);
      ...
      BufferAllocation::Slice slice_out_fake(
          fake_allocations[fake_arg_idx].get(), 0, out_fake_byte_size);
      ThunkSequence seq;
      ...
      thunk = std::make_unique<GemmThunk>(thunk_info, std::move(config),
                                          lhs_slice, rhs_slice, output,
                                          workspace, deterministic_ops);
    }
  ...
  absl::StatusOr<FusionEmissionResult> EmitCustomCall(
    ...
    bool is_ffi_custom_call = custom_call.api_version() ==
    ...
    if (absl::c_any_of(slice_instrs, IsDynamicSliceOrDynamicUpdateSlice)) {
      ...
      TF_ASSIGN_OR_RETURN(
          ...
          found_ffi_handler
              ...
              : legacy_thunk(std::move(fake_operands),
                             std::move(fake_results)));
  ...
  CollectSliceArgumentMetadataForCollectives(
    ...
    slice_data.can_compute_indvar_on_host =
        (slice_data.init_module != nullptr &&
    ...
    if (absl::c_all_of(slice_data.slice_instrs,
                       [&](HloInstruction* slice_instr) {
                         return slice_instr == nullptr;
                       })) {
  ...
  absl::StatusOr<FusionEmissionResult> EmitCollective(
    ...
    absl::Status implementable_status = NcclThunkType::CheckImplementable(
        instr, replica_count, partition_count);
    ...
    if (slice_data.isDynamic) {
      ...
      std::unique_ptr<Thunk> thunk = std::make_unique<DynamicSliceThunk>(
          ...
          std::move(slice_data.arguments),
          std::move(slice_data.fake_allocations),
          std::move(slice_data.offset_buffer_indices),
          std::move(slice_data.orig_shapes),
          std::move(slice_data.sliced_shapes),
