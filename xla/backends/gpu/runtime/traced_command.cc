...
// can never be updated, see `TracedCommand::ShouldInlineTracedCommand`.
...
bool TracedCommand::ShouldInlineTracedCommand(
  ...
  for (const BufferUse& buffer_use : buffer_uses()) {
...
TracedCommand::RecordTracedCommand(
  ...
  if (auto* create = std::get_if<RecordCreate>(&record_action);
      create != nullptr && ShouldInlineTracedCommand(execute_params)) {
    // Trace the command directly into the parent command buffer: the command
    // will never need an update, so the child node indirection and the traced
    // command buffer cache are unnecessary.
    ...
    // Fall through to the child node recording path on platforms that do not
  ...
  if (auto* update = std::get_if<RecordUpdate>(&record_action);
    ...
    if (!ShouldInlineTracedCommand(execute_params)) {
      return Internal(
