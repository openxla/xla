...
class UpdateFlagCmd : public Command {
 public:
  UpdateFlagCmd(bool requires_update_on_initialize,
                bool requires_update_on_execute, BufferUses uses = {})
...
TEST(CommandExecutorTest,
     PersistentNestedInitializationCommandSkipsRecordUpdate) {
  BufferAllocation allocation(/*index=*/0, /*size=*/4, /*color=*/0);
  Shape shape = ShapeUtil::MakeShape(U8, {4});
  BufferAllocation::Slice slice(&allocation, /*offset=*/0, /*size=*/4);
  RecordCounts counts;

  CommandSequence commands;
  commands.Emplace<RecordingCompositeCmd>(
      std::make_unique<UpdateFlagCmd>(
          /*requires_update_on_initialize=*/true,
          /*requires_update_on_execute=*/false,
          Command::BufferUses{BufferUse::Read(slice, shape)}),
      &counts);
  ASSERT_OK_AND_ASSIGN(
      CommandExecutor executor,
      CommandExecutor::Create(std::move(commands), kSerialize));

  BufferAllocations allocations({se::DeviceAddressBase{}},
                                /*device_ordinal=*/0,
                                /*memory_allocator=*/nullptr);
  ServiceExecutableRunOptions run_options;
  Thunk::ExecuteParams dynamic_params = Thunk::ExecuteParams::Create(
      run_options, allocations, /*stream=*/nullptr,
      /*command_buffer_trace_stream=*/nullptr,
      /*collective_params=*/nullptr, /*collective_cliques=*/nullptr,
      /*collective_memory=*/nullptr);
  std::vector<BufferAllocation::Index> persistent_indices = {0};
  Thunk::ExecuteParams persistent_params = Thunk::ExecuteParams::Create(
      run_options, allocations, /*stream=*/nullptr,
      /*command_buffer_trace_stream=*/nullptr,
      /*collective_params=*/nullptr, /*collective_cliques=*/nullptr,
      /*collective_memory=*/nullptr,
      ...
      absl::MakeConstSpan(persistent_indices));
  ...
  ASSERT_OK(executor
                .RecordCreate(dynamic_params, record_params, &command_buffer,
                              /*dependencies=*/{})
                .status());
  EXPECT_EQ(counts.creates, 1);

  command_buffer_state = se::CommandBuffer::State::kUpdate;
  ASSERT_OK(
      executor.RecordUpdate(dynamic_params, record_params, &command_buffer));
  EXPECT_EQ(counts.updates, 1);

  ASSERT_OK(
      executor.RecordUpdate(persistent_params, record_params, &command_buffer));
  EXPECT_EQ(counts.updates, 1);
}
