...
#define COMMAND_BUFFER_CMD_LIST(V)                       \
  ...
  V(kLaunchSetForConditionKernelCmd, "LaunchSetForConditionKernelCmd") \
  V(kUpdateSetForConditionKernelCmd, "UpdateSetForConditionKernelCmd") \
  V(kLaunchSetWhileConditionKernelCmd, "LaunchSetWhileConditionKernelCmd") \
  V(kUpdateSetWhileConditionKernelCmd, "UpdateSetWhileConditionKernelCmd") \
...
class CommandBufferCmd {
 public:
  ...
  // Returns the tail nodes of the command. A single command can create multiple
...
class IfCmd : public CommandBufferCmd {
 ...
 private:
  ...
  GraphConditionalHandle then_cond_handle_;
  GraphNodeHandle set_cond_handle_kernel_node_;
  GraphNodeHandle then_cond_node_;
...
class IfElseCmd : public CommandBufferCmd {
 ...
 private:
  ...
  GraphConditionalHandle then_cond_handle_;
  GraphConditionalHandle else_cond_handle_;
  GraphNodeHandle set_cond_handle_kernel_node_;

  GraphNodeHandle then_cond_node_;
  GraphNodeHandle else_cond_node_;
...
//===----------------------------------------------------------------------===//
// LaunchSetForWhileConditionKernelCmd
//===----------------------------------------------------------------------===//

class LaunchSetWhileConditionKernelCmd : public CommandBufferCmd {
 public:
  LaunchSetWhileConditionKernelCmd(GraphConditionalHandle cond_handle,
  ...
  std::unique_ptr<CommandBufferCmd> Clone() const override {
    return std::make_unique<LaunchSetWhileConditionKernelCmd>(cond_handle_,
                                                              pred_);
  }
...
class WhileCmd : public CommandBufferCmd {
 ...
 private:
  ...
  GraphConditionalHandle cond_handle_;
  GraphNodeHandle initialize_while_handle_node_;
...
class EmptyCmd : public CommandBufferCmd {
 public:
  // Creates a barrier that will synchronize with commands specified
  // by dependencies.
  explicit EmptyCmd(DependencySet dependencies);
