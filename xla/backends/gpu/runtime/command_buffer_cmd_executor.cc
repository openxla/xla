...
CmdOrThunk::CmdOrThunk(std::unique_ptr<CommandBufferCmd> cmd)
    : cmd_or_thunk_(std::move(cmd)) {}

CmdOrThunk::CmdOrThunk(Thunk* thunk) : cmd_or_thunk_(thunk) {
...
CommandBufferCmd* CmdOrThunk::cmd() const {
  if (auto* ptr =
          std::get_if<std::unique_ptr<CommandBufferCmd>>(&cmd_or_thunk_)) {
    return ptr->get();
  }
  return nullptr;
}

Thunk* CmdOrThunk::thunk() const {
  if (auto* ptr = std::get_if<Thunk*>(&cmd_or_thunk_)) {
    return *ptr;
  }
  return nullptr;
}

std::variant<const CommandBufferCmd*, const Thunk*> CmdOrThunk::cmd_or_thunk()
    const {
  if (auto* c = cmd()) {
    return c;
  }
...
absl::Status CmdOrThunk::Prepare(const Thunk::PrepareParams& params) {
  if (auto* c = cmd()) {
    return c->Prepare(params);
  }
  return thunk()->Prepare(params);
}

absl::Status CmdOrThunk::Initialize(const Thunk::InitializeParams& params) {
  if (auto* c = cmd()) {
    return c->Initialize(params);
  }
  return thunk()->Initialize(params);
}

absl::Status CmdOrThunk::Record(
    const Thunk::ExecuteParams& execute_params) const {
  if (auto* c = cmd()) {
    return c->Record(execute_params);
  }
  return thunk()->ExecuteOnStream(execute_params);
}

std::shared_ptr<CollectiveThunk::AsyncEvents> CmdOrThunk::async_events() const {
  if (auto* c = this->cmd()) {
    // Check if it's a CollectiveCmd
    if (auto* collective_cmd = dynamic_cast<const CollectiveCmd*>(c)) {
      return collective_cmd->async_events();
    }
    // Check if it's an AsyncDoneCmd
    if (auto* async_done_cmd = dynamic_cast<const AsyncDoneCmd*>(c)) {
      return async_done_cmd->async_events();
    }
  } else if (auto* t = this->thunk()) {
    // Check if it's a CollectiveThunk
    if (auto* collective_thunk = dynamic_cast<const CollectiveThunk*>(t)) {
      return collective_thunk->async_events();
    }
    // Check if it's a CollectiveDoneThunk
    if (auto* collective_done_thunk =
            dynamic_cast<const CollectiveDoneThunk*>(t)) {
      return collective_done_thunk->async_events();
    }
  }
  return nullptr;
}

bool CmdOrThunk::IsAsyncStart() const {
  if (auto* c = cmd()) {
    return c->IsAsyncStart();
  }
  return thunk()->IsAsyncStart();
}

bool CmdOrThunk::IsAsyncDone() const {
  if (auto* c = cmd()) {
    return c->IsAsyncDone();
  }
  return thunk()->IsAsyncDone();
}

bool CmdOrThunk::IsCollective() const {
  if (auto* c = cmd()) {
    return c->IsCollective();
  }
  return thunk()->IsCollective();
}

bool CmdOrThunk::command_buffer_requires_initialization() const {
  if (auto* c = cmd()) {
    return c->command_buffer_requires_initialization();
  }
  return thunk()->command_buffer_requires_initialization();
}

bool CmdOrThunk::command_buffer_support_loop_unroll() const {
  if (auto* c = cmd()) {
    return c->command_buffer_support_loop_unroll();
  }
  return thunk()->command_buffer_support_loop_unroll();
}

bool CmdOrThunk::command_buffer_force_update() const {
  if (auto* c = cmd()) {
    return c->command_buffer_force_update();
  }
  return thunk()->command_buffer_force_update();
}

CmdOrThunk::BufferUseVector CmdOrThunk::buffer_uses() const {
  if (auto* c = cmd()) {
    return c->buffer_uses();
  }
  return thunk()->buffer_uses();
}

std::shared_ptr<Resource> CmdOrThunk::token() const {
  if (auto* c = cmd()) {
    return c->token();
  }
  return thunk()->token();
...
void CmdOrThunk::add_resouce_use(ResourceUse resource_use) {
  if (auto* c = cmd()) {
    c->add_resouce_use(resource_use);
  } else {
    thunk()->add_resouce_use(resource_use);
  }
}

ResourceUseVector CmdOrThunk::resources() const {
  if (auto* c = cmd()) {
    return c->resources();
  }
  return thunk()->resources();
}

absl::string_view CmdOrThunk::profile_annotation() const {
  if (auto* c = cmd()) {
    return c->profile_annotation();
  }
  return thunk()->profile_annotation();
}

se::StreamPriority CmdOrThunk::command_buffer_priority() const {
  if (auto* c = cmd()) {
    return c->command_buffer_priority();
  }
  return thunk()->command_buffer_priority();
}

void CmdOrThunk::set_command_buffer_priority(se::StreamPriority priority) {
  if (auto* c = cmd()) {
    c->set_command_buffer_priority(priority);
  } else {
    thunk()->set_command_buffer_priority(priority);
  }
}

std::string CmdOrThunk::ToString(int indent) const {
  if (auto* c = cmd()) {
    return c->ToString(indent);
  }
  return thunk()->ToString(indent);
}

std::vector<const se::CommandBuffer::Command*>
CmdOrThunk::CommandBufferDependencies(
    const CommandBufferParams& record_params) const {
  if (auto* c = cmd()) {
    return c->CommandBufferDependencies(record_params);
  }
  return thunk()->CommandBufferDependencies(record_params);
}
...
class CommandOperation : public ExecutionGraph::Operation {
 public:
  explicit CommandOperation(CmdOrThunk::BufferUseVector buffers,
                            const CmdOrThunk* cmd)
  ...
  const CmdOrThunk* cmd() const { return cmd_; }
 ...
 private:
  std::string name_;
  CmdOrThunk::BufferUseVector buffers_;
  const CmdOrThunk* cmd_;
...
static std::vector<CommandOperation> CreateCommandOperations(
  ...
  if (synchronization_mode ==
    ...
    for (const CmdOrThunk& cmd : commands) {
      operations.emplace_back(cmd.buffer_uses(), &cmd);
    }
  }
  ...
  if (synchronization_mode ==
    ...
    for (const CmdOrThunk& cmd : commands) {
      operations.emplace_back(CmdOrThunk::BufferUseVector{}, &cmd);
    }
...
CommandBufferCmdExecutor::CommandBufferCmdExecutor(
  ...
  for (const CmdOrThunk& cmd : commands_) {
...
absl::Status CommandBufferCmdExecutor::Record(
  ...
  auto skip_command_update = [&](CommandId id) {
    ...
    const CmdOrThunk& command = commands_[id];
  ...
  for (CommandId id = 0; id < static_cast<CommandId>(commands_.size()); ++id) {
    ...
    TF_RETURN_IF_ERROR(command.Record(execute_params));
  }
...
CommandBufferCmdExecutor::SinkCommands(
  ...
  for (CommandId id : sink_ids) {
    auto* record_state = record_params.state.GetOrNull<RecordState>(
        commands_[id].cmd_or_thunk(), record_params.command_buffer,
...
CommandBufferCmdExecutor::Dependencies(const CommandBufferParams& record_params,
  ...
  for (CommandId dependency_id : dependencies_ids) {
    auto* record_state = record_params.state.GetOrNull<RecordState>(
        commands_[dependency_id].cmd_or_thunk(), record_params.command_buffer,
        record_params.unroll_iteration);
...
bool CommandBufferCmdExecutor::IsSource(
    std::variant<const CommandBufferCmd*, const Thunk*> cmd) const {
  for (CommandId id = 0; id < static_cast<CommandId>(commands_.size()); ++id) {
    if (cmd == commands_[id].cmd_or_thunk()) {
      return IsSource(id);
    }
  }
  return false;
...
std::vector<const se::CommandBuffer::Command*>
CommandBufferCmdExecutor::Dependencies(
...
absl::Status CommandBufferCmdExecutor::HandleCmdCreateOrUpdate(
  ...
  CommandBufferStateManager& state = record_params.state;
...
//===----------------------------------------------------------------------===//
// Thunk method implementations
//===----------------------------------------------------------------------===//

// These are implemented here rather than in thunk.cc to avoid circular
// dependencies between thunk and command_buffer_cmd_executor.

std::vector<const se::CommandBuffer::Command*> Thunk::CommandBufferDependencies(
    const CommandBufferParams& record_params) const {
...
absl::Status Thunk::HandleCmdCreateOrUpdate(
