...
bool IsConvertible(const Thunk& thunk, const CommandBufferConfig& config) {
  ...
  if (*cmd_type == DebugOptions::COLLECTIVES) {
    ...
    // (e.g. TP training). Ordinary (non-traced) kernels dereference the fixed VA
    // and the MMU redirects to the current physical page, so they are safe; only
    // traced collectives are affected. Keep collectives out of command buffers
    // on ROCm in these modes and run them eagerly. ALWAYS_UPDATE re-records the
    // collective each step and is unaffected. NVIDIA/CUDA behaviour is unchanged.
    if (config.device_description.gpu_compute_capability().IsRocm() &&
      ...
      VLOG(1) << "ROCm: not converting collective thunk "
