...
static absl::StatusOr<int> SetupAndExecute(
    int d, DeviceTestSlot* slots, CollectivePermuteThunk* thunk,
    const DeviceAssignment* device_assignment) {
...
TEST(CollectivePermuteThunkMultiGpuTest, ExecuteOnStream) {
  ...
  tsl::thread::ThreadPool pool(tsl::Env::Default(),
                               "collective_permute_execute", kNumDevices);
...
TEST(CollectivePermuteThunkMultiGpuTest, RecordCommandBufferCreate) {
  ...
  tsl::thread::ThreadPool pool(tsl::Env::Default(), "collective_permute_create",
                               kNumDevices);
  std::vector<tsl::Future<int>> futures(kNumDevices);
  for (int d = 0; d < kNumDevices; ++d) {
    futures[d] = tsl::MakeFutureOn<int>(
...
TEST(CollectivePermuteThunkMultiGpuTest, RecordCommandBufferUpdate) {
  ...
  {
    tsl::thread::ThreadPool pool(tsl::Env::Default(),
                                 "collective_permute_create", kNumDevices);
    std::vector<tsl::Future<int>> futures(kNumDevices);
    for (int d = 0; d < kNumDevices; ++d) {
      futures[d] = tsl::MakeFutureOn<int>(
          *pool.AsExecutor(), [d, &slots, &thunk, &device_assignment]() {
  ...
  {
    tsl::thread::ThreadPool pool(tsl::Env::Default(),
                                 "collective_permute_update", kNumDevices);
    std::vector<tsl::Future<int>> futures(kNumDevices);
    for (int d = 0; d < kNumDevices; ++d) {
      futures[d] = tsl::MakeFutureOn<int>(
          *pool.AsExecutor(),
