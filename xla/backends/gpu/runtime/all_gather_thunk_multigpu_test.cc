...
static absl::Status FillDestinationBuffer(se::Stream& stream,
                                          se::DeviceAddressBase buf,
                                          float value) {
  std::vector<float> data(kGatheredLength, value);
  RETURN_IF_ERROR(stream.Memcpy(&buf, data.data(), kGatheredByteLength));
  return stream.BlockHostUntilDone();
}

static absl::StatusOr<std::vector<float>> ReadDeviceBuffer(
    se::Stream& stream, se::DeviceAddressBase buf) {
  std::vector<float> data(kGatheredLength);
  RETURN_IF_ERROR(stream.Memcpy(data.data(), buf, kGatheredByteLength));
...
static absl::Status SetupDeviceSlot(int device_ordinal, DeviceTestSlot& slot,
  ...
  slot.dst1 = slot.executor->AllocateArray<float>(kGatheredLength, 0);
  slot.src2 = slot.executor->AllocateArray<float>(kLength, 0);
  slot.dst2 = slot.executor->AllocateArray<float>(kGatheredLength, 0);
...
static absl::Status RunExecuteOnStreamPhase(DeviceTestSlot& slot,
  ...
  return VerifyOutput(*slot.stream, slot.dst1, phase);
}
...
static absl::Status RunUpdatePhase(DeviceTestSlot& slot, AllGatherThunk& thunk,
  ...
  RETURN_IF_ERROR(slot.command_buffer->Submit(slot.stream.get()));
  RETURN_IF_ERROR(slot.stream->BlockHostUntilDone());
  return VerifyOutput(*slot.stream, slot.dst2, phase);
}

static absl::StatusOr<int> SetupAndExecute(
    int d, DeviceTestSlot* slots, AllGatherThunk* thunk,
    const DeviceAssignment* device_assignment) {
...
TEST(AllGatherThunkMultiGpuTest, ExecuteOnStream) {
  ...
  tsl::thread::ThreadPool pool(tsl::Env::Default(), "allgather_execute",
                               kNumDevices);
...
TEST(AllGatherThunkMultiGpuTest, RecordCommandBufferCreate) {
  ...
  tsl::thread::ThreadPool pool(tsl::Env::Default(), "allgather_create",
                               kNumDevices);
  std::vector<tsl::Future<int>> futures(kNumDevices);
  for (int d = 0; d < kNumDevices; ++d) {
    futures[d] = tsl::MakeFutureOn<int>(
...
TEST(AllGatherThunkMultiGpuTest, RecordCommandBufferUpdate) {
  ...
  {
    tsl::thread::ThreadPool pool(tsl::Env::Default(), "allgather_update",
                                 kNumDevices);
    std::vector<tsl::Future<int>> futures(kNumDevices);
    for (int d = 0; d < kNumDevices; ++d) {
      futures[d] = tsl::MakeFutureOn<int>(
