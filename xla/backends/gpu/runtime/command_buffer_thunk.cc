...
absl::Status CommandBufferThunk::Initialize(const InitializeParams& params) {
  ...
  // When VA remapping is enabled, use the device address of the first
  // allocation as the command buffer key to distinguish between different VA
  // ranges during interleaved execution. When disabled, all executions share
  // a single command buffer per executor (keyed by nullptr).
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      GetOrCreateCommandBuffer(params.executor, first_alloc_address));
  ...
  if (!thunks_ && !cmd_buffer->warmup_done) {
  ...
  if (cmd_buffer->warmup_done && (cmd_buffer->command_buffer->state() ==
    ...
    VLOG(3) << "Initialize command buffer on device #"
            ...
            << "; first_alloc_addr=" << first_alloc_address;
    ...
    TraceMe trace([&] {
      return TraceMeEncode(
          ...
          {{"device", params.executor->device_ordinal()},
           {"num_commands", commands_.size()},
           {"first_alloc_addr",
            reinterpret_cast<uint64_t>(first_alloc_address)}});
    });
    ...
    VLOG(3) << "skip Initialize command buffer on device #"
            << params.executor->device_ordinal()
            << " by recoding command buffer cmd sequence"
            << "; num_commands=" << commands_.size()
            << "; first_alloc_addr=" << first_alloc_address;
...
absl::Status CommandBufferThunk::ExecuteOnStream(const ExecuteParams& params) {
  ...
  if (!enable_command_buffer_va_remapping_) {
  ...
  } else if (cmd_buffer->command_buffer->state() ==
             se::CommandBuffer::State::kCreate) {
    // When VA remapping is enabled, a new command buffer may be created for a
    // different va_range_idx that was not initialized during Initialize().
    // We need to record the command buffer before we can submit it.
    VLOG(3) << "Recording command buffer on device #"
            << executor->device_ordinal()
            << " for first_alloc_addr=" << first_alloc_address
            << " (VA remapping enabled)"
            << "; num_commands=" << commands_.size();
    ...
    uint64_t end_micros = tsl::Env::Default()->NowMicros();
    VLOG(3) << "Recorded command buffer in " << (end_micros - start_micros)
            << " μs; num_commands=" << commands_.size();
    cmd_buffer->num_executions = 0;
    cmd_buffer->warmup_done = true;
  }
...
CommandBufferThunk::GetOrCreateCommandBuffer(se::StreamExecutor* executor,
  ...
  // Key includes both executor and first_alloc_address to support separate
  // command buffers for different buffer allocation VA ranges during
