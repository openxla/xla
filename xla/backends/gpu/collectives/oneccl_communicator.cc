...
static absl::StatusOr<onecclDataType_t> ToCclDataType(PrimitiveType dtype,
    ...
    switch (dtype) {
      ...
      case U16:
        ...
        // For collectives that just move data around, we can use onecclFloat16 for
...
Future<> OnecclCommunicator::GroupExecute(absl::AnyInvocable<absl::Status() &&> f) {
...
absl::Status OnecclCommunicator::GroupEnd() {
  if (group_nesting_level_ <= 0) {
        return absl::FailedPreconditionError(
            "There was no corresponding onecclGroupStart() for this onecclGroupEnd()");
    }
    group_nesting_level_--;
    if (group_nesting_level_ == 0) {
...
Future<> OnecclCommunicator::Broadcast(se::DeviceAddressBase send_buffer,
                                       se::DeviceAddressBase recv_buffer,
                                       PrimitiveType dtype, size_t count,
                                       RankId root, const Communicator::Executor& executor) {
    return Execute([send_buffer, recv_buffer, dtype, count, root,
                    &executor, this]() -> absl::Status {
      return LaunchBroadcast(send_buffer, recv_buffer, dtype, count, root, executor);
                    });
}
...
Future<> OnecclCommunicator::AllGather(se::DeviceAddressBase send_buffer,
                                       ...
                                       PrimitiveType dtype, size_t count,
                                       const Communicator::Executor& executor) {
    return Execute([send_buffer, recv_buffer, dtype, count,
                    &executor, this]() -> absl::Status {
      return LaunchAllGather(send_buffer, recv_buffer, dtype, count, executor);
                    });
}

Future<> OnecclCommunicator::AllToAll(absl::InlinedVector<se::DeviceAddressBase, 4> send_buffers,
                                       absl::InlinedVector<se::DeviceAddressBase, 4> recv_buffers,
                                       PrimitiveType dtype, size_t count,
                                       const Communicator::Executor& executor) {
    return Execute([send_buffers, recv_buffers, dtype, count,
                    &executor, this]() -> absl::Status {
...
absl::Status OnecclCommunicator::LaunchSend(
    se::DeviceAddressBase send_buffer, PrimitiveType dtype, size_t count, RankId peer,
    const Communicator::Executor& executor) {
  se::Stream* stream = ToStream(executor);
  void* stream_handle = stream->platform_specific_handle().stream;
  ::sycl::queue* sycl_queue = reinterpret_cast<::sycl::queue*>(stream_handle);
...
absl::Status OnecclCommunicator::LaunchRecv(
    se::DeviceAddressBase recv_buffer, PrimitiveType dtype, size_t count, RankId peer,
    const Communicator::Executor& executor) {
  se::Stream* stream = ToStream(executor);
  void* stream_handle = stream->platform_specific_handle().stream;
  ::sycl::queue* sycl_queue = reinterpret_cast<::sycl::queue*>(stream_handle);
...
absl::Status OnecclCommunicator::LaunchAllToAll(
    ...
    PrimitiveType dtype, size_t count, const Communicator::Executor& executor) {
      ...
      for (size_t i = 0; i < send_buffers.size(); ++i) {
        ...
        XLA_ONECCL_RETURN_IF_ERROR(onecclSend(
            send_buffer.opaque(), ToOnecclCount(dtype, count), ccl_dtype, i,
            comm_, sycl_queue));
        XLA_ONECCL_RETURN_IF_ERROR(onecclRecv(
