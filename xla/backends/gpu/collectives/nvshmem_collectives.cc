...
NvshmemCollectives* NvshmemCollectives::Default() {
  ...
  if (auto* nvshmem_collectives = tsl::down_cast<NvshmemCollectives*>(*collectives)) {
...
absl::Status NvshmemCollectives::Initialize(int process_id, size_t num_processes, size_t device_count_per_process,
  ...
  if (process_id_ == 0) {
    ...
    TF_RETURN_IF_ERROR(kv_store_set(kv_store_key_, nvshmem_id_str));
  } else {
    TF_ASSIGN_OR_RETURN(std::string id_str, kv_store_get(kv_store_key_));
  ...
  XLA_NVSHMEM_RETURN_IF_ERROR(nvshmemx_set_attr_uniqueid_args(
      process_id, num_processes, &nvshmem_id, &nvshmem_init_attr));
  XLA_NVSHMEM_RETURN_IF_ERROR(nvshmemx_hostlib_init_attr(
      NVSHMEMX_INIT_WITH_UNIQUEID, &nvshmem_init_attr));

  VLOG(3) << absl::StreamFormat(
      "Initialized NVSHMEM on process %d; num_processes=%llu", process_id,
      num_processes);
  return absl::OkStatus();
}

NvshmemCollectives::Finalize() {
  VLOG(3) << absl::StreamFormat(
