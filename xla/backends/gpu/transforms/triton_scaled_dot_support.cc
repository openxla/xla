...
bool IsTritonSupportedScaledDot(const HloScaledDotInstruction& dot,
  ...
  if (!lhs_fp4 && !rhs_fp4) return true;

  // Triton's verifier derives the contracting dimension size from each
  // operand's fp4 packing. Mixing fp4 and non-fp4 makes the two sides disagree.
  if (lhs_fp4 != rhs_fp4) return false;

  if (!cc.IsAtLeastHopper()) return false;
  ...
  // Triton's fp4 scaled-MMA path expects both scales to use the same encoding.
  if (lhs_scale != rhs_scale) return false;
  ...
  // On tcgen05 targets, the MXFP4 path accepts XLA's canonical operand layout:
  // lhs is K-packed and rhs is N-packed. Other Hopper-or-newer targets can
  // unpack fp4 operands before using the scaled dot.
  if (cc.HasTcgen05() && (!lhs_k_pack || rhs_k_pack)) return false;
