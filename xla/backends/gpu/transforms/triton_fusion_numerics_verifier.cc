...
absl::Status InlineModuleFusions(HloModule* hlo_module) {
  ...
  while (true) {
    ...
    ABSL_RETURN_IF_ERROR(hlo_module->entry_computation()->Accept(&visitor));
    if (!visitor.changed()) {
...
absl::Status ForAllTritonFusions(
  ...
  for (HloComputation* computation :
    ...
    for (HloInstruction* instruction : computation->instructions()) {
      ABSL_ASSIGN_OR_RETURN(auto triton_fusion, AsTritonFusion(instruction));
      if (triton_fusion != nullptr) {
        VLOG(2) << "processing fusion " << triton_fusion->name();
        ABSL_RETURN_IF_ERROR(fn(*triton_fusion));
      }
    }
  }
  return absl::OkStatus();
