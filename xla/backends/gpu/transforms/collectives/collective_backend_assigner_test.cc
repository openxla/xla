...
TEST_F(CollectiveBackendAssignerTest,
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(kHloText));
  // Default is PRIVATE_MEMORY — collectives_mode should not be set.
  ...
  EXPECT_THAT(
      GetCollectivesMode(permute),
      absl_testing::IsOkAndHolds(DebugOptions::COLLECTIVES_MEMORY_INVALID));
}
...
TEST_F(CollectiveBackendAssignerTest,
  ...
  // collectives_mode should remain PRIVATE_MEMORY (unaffected by permute flag).
  EXPECT_THAT(
