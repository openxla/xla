...
TEST_F(CollectiveFusionTest, AsyncCollectiveCreatorLiftsTritonCollective) {
  constexpr int64_t kNumElements = 32768;  // 128 KB -> OneShot
  Shape shape = ShapeUtil::MakeShape(F32, {kNumElements});

  const std::string hlo =
      absl::StrFormat(kAllReduceHloTemplate, shape.ToString());
  HloModuleConfig module_config = GetModuleConfigForTest(/*replica_count=*/8);
  ASSERT_OK_AND_ASSIGN(auto module,
                       ParseAndReturnVerifiedModule(hlo, module_config));
  AnnotateAndFusePipeline fuse_pipeline(*gpu_topology_);
  ASSERT_OK_AND_ASSIGN(bool merged, fuse_pipeline.Run(module.get()));
  EXPECT_TRUE(merged);
  // Wrap the fusion in async start/done.
  ASSERT_OK(xla::gpu::RunAsyncCollectivesConversionPasses(module.get()));
  static constexpr absl::string_view kExpected = R"(
    // CHECK: %[[FUSION_COMPUTATION:.*]] ({{.*}}: f32[32768]) -> f32[32768] {
    // CHECK:   %[[P0:.*]] = f32[32768]{0} parameter(0)
    // CHECK:   ROOT {{.*}} = f32[32768]{0} all-reduce(%[[P0]]),
    ...
    // CHECK:   %[[P0_ENTRY:.*]] = f32[32768]{0} parameter(0)
    // CHECK:   %[[ASYNC_START:.*]] = ((f32[32768]{0}), f32[32768]{0})
    // CHECK-SAME: fusion-start(%[[P0_ENTRY]]), kind=kCustom, calls=%[[FUSION_COMPUTATION]]
    // CHECK:   ROOT {{.*}} = f32[32768]{0} fusion-done(%[[ASYNC_START]])
  )";
  // NB: By default PrintOptions() will use syntactic sugar for
  // fusion-start/done.
  // This is also what RunAndFilecheckHloRewrite uses.
  ASSERT_OK_AND_ASSIGN(
      bool filecheck_matches,
      RunFileCheck(module->ToString(HloPrintOptions()), kExpected));
