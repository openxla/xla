...
ENTRY e {
  p0 = f16[32,3] parameter(0)
  p1 = f16[8,28] parameter(1)
  p2 = f16[8,28] parameter(2)
  c1 = f16[8,28] add(p1, p2)
  b1 = f16[32,7] bitcast(c1)
...
ENTRY e {
  ...
  br = f32[8,4,7] broadcast(p1), dimensions={0,1}
  b1 = f32[32,7] bitcast(br)
  ROOT d = f32[3,7] dot(p0, b1),
    lhs_contracting_dims={0}, rhs_contracting_dims={0}
})"));
  ASSERT_THAT(GemmFusion(gpu_version_).Run(module.get()), IsOkAndHolds(true));
...
ENTRY e {
  ...
  p1 = f32[32,14] parameter(1)
  d = f32[16,14] dot(p0, p1),
    lhs_contracting_dims={0}, rhs_contracting_dims={0}
  b1 = f32[8,2,14] bitcast(d)
  b2 = f32[8,2,7,2] bitcast(b1)
...
TEST_P(GemmFusionTestVersioned, TransposeFusesInThreeWayConcatGemm) {
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                       ParseAndReturnVerifiedModule(R"(
HloModule module
...
TEST_P(GemmFusionTestVersioned, TransposeDoesNotFuseInConcatOnContractingDim) {
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                       ParseAndReturnVerifiedModule(R"(
HloModule module
...
TEST_P(GemmFusionTestVersioned, BitcastSplitSliceAlignedFuses) {
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                       ParseAndReturnVerifiedModule(R"(
HloModule module
...
ENTRY main {
  ...
  if (std::get<0>(GetParam())) {
    EXPECT_THAT(module->entry_computation()->root_instruction(),
                GmockMatch(m::Tuple(m::Fusion(m::Parameter(), m::Parameter(),
                                              m::Bitcast(m::Parameter())),
...
TEST_P(GemmFusionTestVersioned, BitcastSplitSliceUnalignedDoesNotFuse) {
  ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                       ParseAndReturnVerifiedModule(R"(
HloModule module
...
ENTRY main {
  ...
  if (std::get<0>(GetParam())) {
    EXPECT_THAT(module->entry_computation()->root_instruction(),
                GmockMatch(m::Tuple(
                    m::Fusion(m::Parameter(), m::Parameter(), m::Bitcast()),
                    m::Convert(m::Bitcast()))));
  } else {
    EXPECT_THAT(module->entry_computation()->root_instruction(),
                GmockMatch(m::Tuple(
