...
absl::Status AllGatherMultiProcessTestBody(int node_id, int port) {
  ...
  if (desc.gpu_compute_capability().IsCuda() && !desc.gpu_compute_capability()
                                                     ...
                                                     ->IsAtLeastAmpere()) {
    LOG(INFO) << log_prefix
              << ": skipping one-shot all-gather test, requires Ampere+ for "
                 "CUDA.";
  ...
  *compile_options.executable_build_options.mutable_debug_options() =
  ...
  LOG(INFO) << log_prefix << ": compiling HLO module with one-shot all-gather";
    ...
    for (const HloInstruction* instr : comp->instructions()) {
      if (instr->opcode() == HloOpcode::kAllGather ||
        ...
        TF_RET_CHECK(gpu_config.ok())
            ...
            << gpu_config.status();
        LOG(INFO)
            << log_prefix << ": AllGather instruction " << instr->name()
        ...
        TF_RET_CHECK(
            ...
            << "Expected AllGather to use KERNEL_STRATEGY_TRITON_ONE_SHOT, but "
               "got: "
            << gpu::CollectiveBackendConfig::CollectiveKernelStrategy_Name(
                   gpu_config->collective_backend_config().kernel_strategy());
      }
  ...
  TF_RET_CHECK(found_all_gather)
      << "Expected to find an AllGather instruction in optimized HLO.";

  // Prepare input literal: rank 0 provides 1.0f, rank 1 provides 2.0f.
  const float input_val = (node_id == 0) ? 1.0f : 2.0f;
  ...
  LOG(INFO) << log_prefix << ": executing one-shot all-gather";
  ...
  Literal result_literal(ShapeUtil::MakeShape(F32, {256}));
  ABSL_RETURN_IF_ERROR(results[0][0]->ToLiteralSync(&result_literal));

  // Expected result is [1.0f x 128, 2.0f x 128] on all ranks.
  ...
  Literal expected_literal = LiteralUtil::CreateR1<float>(expected_data);
...
TEST(AllGatherMultiProcessE2ETest, OneShotAllGather2Processes) {
  ...
  for (int node_id = 0; node_id < kNumNodes; ++node_id) {
    std::vector<std::string> argv = {
        test_binary_name,
        "--test_to_run=AllGatherMultiProcessHelper",
        absl::StrFormat("--node_id=%d", node_id),
        absl::StrFormat("--port=%d", port),
        "--alsologtostderr",
        "--vmodule=gpu_executable=1,thunk_executor=1,all_gather_thunk=5,"
...
int main(int argc, char* argv[]) {
  ...
  if (test_to_run == "AllGatherMultiProcessHelper") {
    result = xla::AllGatherMultiProcessTestBody(node_id, port);
  } else {
