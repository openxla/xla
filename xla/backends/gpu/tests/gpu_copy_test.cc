...
constexpr char kRawDynamicSliceFusionModule[] = R"(
    raw_dynamic_slice {
    ...
    ENTRY main {
      p0 = s32[4] parameter(0)
      ROOT fusion = s32[1] fusion(p0), kind=kLoop, calls=raw_dynamic_slice
    })";

TEST_F(GpuCopyTest, UseDUSV2MemcpyForRawDynamicSliceFusion) {
  ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<VerifiedHloModule> hlo_module,
      ParseAndReturnVerifiedModule(kRawDynamicSliceFusionModule));

  ASSERT_OK(CompileAndVerifyIr(std::move(hlo_module),
                               "; CHECK-NOT: void @fusion",
                               /*match_optimized_ir=*/false,
                               /*run_optimization_passes=*/false));
  EXPECT_TRUE(
      RunAndCompareNoHloPasses(kRawDynamicSliceFusionModule, ErrorSpec{0, 0}));
}
...
constexpr char kRawDynamicUpdateSliceModule[] = R"(
    dynamic_update_slice {
      p0 = s32[4,8,8] parameter(0)
    ...
    add_slices {
      p0 = s32[1,8,8] parameter(0)
      ROOT sum = s32[1,8,8] add(p0, p0)
    }

    body {
      p0 = (s32[], s32[4,8,8], s32[1,8,8]) parameter(0)
      ivar = s32[] get-tuple-element(p0), index=0
      input = s32[4,8,8] get-tuple-element(p0), index=1
      input-copy = s32[4,8,8] copy(input)

      ivar_copy = s32[] copy(ivar)
      acc = s32[1,8,8] get-tuple-element(p0), index=2
      acc_copy = s32[1,8,8] copy(acc)

      updated = s32[4,8,8] fusion(input-copy, acc_copy, ivar_copy), kind=kLoop,
          calls=dynamic_update_slice
      next_ivar = s32[] fusion(ivar_copy), kind=kLoop, calls=add

      next_acc = s32[1,8,8] fusion(acc_copy), kind=kLoop, calls=add_slices
      ROOT result = (s32[], s32[4,8,8], s32[1,8,8])
          tuple(next_ivar, updated, next_acc)
    }
    ...
    ENTRY main {
      input = s32[4,8,8] fusion(), kind=kLoop, calls=input
      c7 = s32[] constant(7)
      init_acc = s32[1,8,8] broadcast(c7), dimensions={}
      c0 = s32[] constant(0)
      tuple = (s32[], s32[4,8,8], s32[1,8,8]) tuple(c0, input, init_acc)
      ROOT while = (s32[], s32[4,8,8], s32[1,8,8]) while(tuple),
          condition=condition, body=body,
          backend_config={"known_trip_count":{"n":"6"},
                          "known_init_step":{"init":"0","step":"1"},
                          "known_induction_variable":{"tuple_index":"0"}}
...
TEST_F(GpuCopyTest, UseDUSV2MemcpyForRawDynamicUpdateSliceFusion) {
  ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<VerifiedHloModule> hlo_module,
      ParseAndReturnVerifiedModule(kRawDynamicUpdateSliceModule));
  ...
  ASSERT_OK(CompileAndVerifyIr(std::move(hlo_module),
                               ...
                               /*run_optimization_passes=*/false));
  EXPECT_TRUE(
      RunAndCompareNoHloPasses(kRawDynamicUpdateSliceModule, ErrorSpec{0, 0}));
}
...
constexpr char kSliceMemcpyModuleUnfused[] = R"(
    body {
      p0 = (s32[], s32[4,8,1000000], s32[1,8,1000000]) parameter(0)
      ivar = s32[] get-tuple-element(p0), index=0
      ...
      c0 = s32[] constant(0)
      c1 = s32[] constant(1)
      slice = s32[1,8,1000000] dynamic-slice(input, ivar_copy, c0, c0),
          dynamic_slice_sizes={1,8,1000000}

      next_ivar = s32[] add(ivar_copy, c1)
      ROOT result = (s32[], s32[4,8,1000000], s32[1,8,1000000])
    ...
    ENTRY main {
      ...
      p1 = s32[1,8,1000000] parameter(1)
      c0 = s32[] constant(0)
      tuple = (s32[], s32[4,8,1000000], s32[1,8,1000000]) tuple(c0, p0, p1)
      ROOT while = (s32[], s32[4,8,1000000], s32[1,8,1000000]) while(tuple),
          condition=condition, body=body
    })";
