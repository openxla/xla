...
using RocblasBackendConfig = AutotuneResult::GemmKey;
...
const char kRocblasCustomCallHlo[] = R"(
...
class RocblasBackendTest : public HloHardwareIndependentTestBase {
 protected:
  DebugOptions debug_options_;
  AMDGPUCompiler compiler_;
  se::StreamExecutor* stream_executor_;
  Compiler::GpuTargetConfig target_config_;
  RocblasBackend backend_;
  ...
  RocblasBackendConfig ExpectedDefaultAlgorithm() {
...
TEST_F(RocblasBackendTest, CanCreateRocblasBackend) {
  ASSERT_NE(nullptr, &backend_);
}

TEST_F(RocblasBackendTest, GetSupportedConfigsFromRocblasCustomCall) {
...
TEST_F(RocblasBackendTest, HipblasLtCustomCall) {
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> hlo_module,
                          ParseAndReturnVerifiedModule(kHipblasLtCustomCallHlo));
  const HloInstruction* instr =
      hlo_module->entry_computation()->root_instruction()->operand(0);
  RocblasBackend backend(stream_executor_, &debug_options_, &compiler_,
...
TEST_F(RocblasBackendTest,
       GetSupportedConfigsReturnsEmptyVectorNonRocblasCustomCall) {
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> hlo_module,
...
TEST_F(RocblasBackendTest, GetDefaultConfigFromRocblasCustomCall) {
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> hlo_module,
                          ParseAndReturnVerifiedModule(kRocblasCustomCallHlo));
