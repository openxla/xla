...
class GpuCodegenBackend : public CodegenBackend {
 public:
  ...
  absl::StatusOr<std::unique_ptr<Executable>> Compile(
    ...
    bool is_tuple_output = hlo_instruction.shape().IsTuple();
    bool is_followed_by_get_tuple =
        hlo_instruction.users().size() == 1 &&
        hlo_instruction.users()[0]->opcode() == HloOpcode::kGetTupleElement &&
        hlo_instruction.users()[0]->tuple_index() == 0;
    bool also_extract_consumer = uses_last_output_for_scratch_ &&
                                 is_tuple_output && is_followed_by_get_tuple;
    if (also_extract_consumer) {
    ...
    HloInstruction* main_instruction = entry_computation->root_instruction();
    if (also_extract_consumer) {
      main_instruction =
          entry_computation->root_instruction()->mutable_operand(0);
    }
    TF_RETURN_IF_ERROR(ApplyConfig(*main_instruction, config));
