...
constexpr absl::string_view kArrayResultHlo = R"hlo(
  ENTRY e {
...
constexpr absl::string_view kTupleResultHlo = R"hlo(
  ENTRY e {
    p0 = f32[4] parameter(0)
    ROOT c = (f32[4]) custom-call(p0), custom_call_target="test.target"
  }
...
TEST(SingleResultArgumentPositionTest, ResultFollowsTheOperands) {
  ASSERT_OK_AND_ASSIGN(auto tester,
                       NativeCustomCallHandlerTester::Create(kTupleResultHlo));
  ASSERT_OK_AND_ASSIGN(emitters::KernelArguments kernel_args,
                       tester->context().CreateKernelArguments());

  // One operand, then the single array leaf of the tuple result.
  EXPECT_THAT(SingleResultArgumentPosition(tester->instruction(), kernel_args),
              IsOkAndHolds(1));
}
...
class MakeCustomKernelThunkSequenceTest : public ::testing::Test {
 protected:
  ...
  CustomKernelLaunchSpec ValidLaunchSpec() {
    return CustomKernelLaunchSpec{
        /*name=*/"fake_kernel",
        /*kernel_spec=*/FakeKernelSpec(2, ValidPackingSpec()),
...
TEST_F(MakeCustomKernelThunkSequenceTest, RejectsOutOfRangeRelocation) {
  ...
  EXPECT_THAT(MakeCustomKernelThunkSequence(
                  tester_->context(),
                  {/*name=*/"fake_kernel",
                   /*kernel_spec=*/FakeKernelSpec(2, std::move(packing_spec))},
                  *kernel_args_),
              StatusIs(absl::StatusCode::kInvalidArgument));
}
...
TEST_F(MakeCustomKernelThunkSequenceTest, RejectsArityMismatch) {
  EXPECT_THAT(MakeCustomKernelThunkSequence(
                  tester_->context(),
                  {/*name=*/"fake_kernel",
                   // The kernel takes three arguments but the packing spec
                   // only produces two.
                   /*kernel_spec=*/FakeKernelSpec(3, ValidPackingSpec())},
                  *kernel_args_),
              StatusIs(absl::StatusCode::kInvalidArgument,
                       HasSubstr("takes 3 arguments")));
}

TEST_F(MakeCustomKernelThunkSequenceTest, RejectsPackingFunction) {
  // A packing function cannot be serialized, so it would silently break AOT
  // compilation of the folded thunk.
  se::KernelLoaderSpec kernel_spec =
      se::KernelLoaderSpec::CreateOwningCudaPtxInMemorySpec(
          /*ptx=*/"", /*kernel_name=*/"fake_kernel", /*arity=*/2,
          se::KernelLoaderSpec::KernelArgsPackingFunc{});
  ...
  EXPECT_THAT(
      MakeCustomKernelThunkSequence(tester_->context(),
                                    ...
                                    *kernel_args_),
      StatusIs(absl::StatusCode::kInvalidArgument,
               HasSubstr("KernelArgsPackingSpec")));
}
