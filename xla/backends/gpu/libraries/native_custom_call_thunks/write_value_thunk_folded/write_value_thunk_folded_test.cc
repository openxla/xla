...
// Loads the AOT-compiled executable produced by
// write_value_thunk_folded_compile_main and runs it, verifying the thunk-folded
...
TEST(WriteValueThunkFoldedTest, LoadAndRunFoldedCustomCall) {
  std::string path = tsl::GetDataDependencyFilepath(
      "tensorflow/compiler/xla/backends/gpu/libraries/"
      "native_custom_call_thunks/write_value_thunk_folded/"
      "write_value_thunk_folded_executable_h100");
