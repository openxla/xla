...
/*static*/ absl::StatusOr<std::unique_ptr<SubprocessProfilingSession>>
...
absl::Status SubprocessProfilingSession::CollectData(
  ...
  if (response_.empty_trace() || response_.raw_xspace().empty()) {
    space->add_warnings(
        absl::StrCat("No XSpace data returned from subprocess: ",
                     subprocess_info_.DebugString()));
  } else if (!space->MergeFromString(response_.raw_xspace())) {
    space->add_errors(absl::StrCat(
        "Failed to merge XSpace data (", response_.raw_xspace().size(),
        " bytes) from subprocess: ", subprocess_info_.DebugString()));
  }
