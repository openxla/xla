...
void RocmTracer::EmitMarkerEvent(std::string label, uint64_t start_ns,
  ...
  // Markers correlate with nothing downstream; rocprofiler's marker
  // correlation id is 64-bit and would narrow into this 32-bit field.
...
std::string RocmTracer::GetCurrentRoctxLabel() {
        ...
        rocprofiler_configure_callback_tracing_service(
            ...
            [](rocprofiler_callback_tracing_record_t record,
              ...
              if (record.phase == ROCPROFILER_CALLBACK_PHASE_ENTER) {
                ...
                // Copied: the frame can be popped by the next roctx call on
                // this thread.
                std::string roctx = tracer.GetCurrentRoctxLabel();
