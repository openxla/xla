...
private:
    ...
    absl::flat_hash_map<uint32_t, CorrelationInfo> correlation_info_  TF_GUARDED_BY(events_mutex);
...
class RocmTraceCollectorImpl : public profiler::RocmTraceCollector {
 public:
  ...
  void OnEventsDropped(const std::string& reason, uint32_t correlation_id) override
  {
    LOG(INFO) << "RocmTracerEvent dropped (correlation_id=" << correlation_id << ",) : " << reason << ".";
  }
...
void RocmTraceCollectorImpl::AddEvent(RocmTracerEvent&& event, bool is_auxiliary)
{
...
const std::vector<RocmTracerEvent> RocmTraceCollectorImpl::ApiActivityInfoExchange()
{
...
std::unique_ptr<RocmTraceCollector> CreateRocmCollector(
    const RocmTraceCollectorOptions& options,
    const uint64_t start_walltime_ns, const uint64_t start_gputime_ns) {
