...
void ApplyCount(const ProfileOptions& options, absl::string_view key,
  ...
  Apply<int64_t>(options, key, keys, diagnostics, [&](int64_t value) {
    if (value < 0) {
      diagnostics.errors.push_back(absl::StrCat(
          ...
          "encoding for \"no limit\". Pass ",
          kMaxCount, " for the largest supported value. The key was ignored."));
      return;
    }
    if (value > kMaxCount) {
  ...
  for (const UnimplementedKey& key : kUnimplementedOnRocm) {
    ...
    switch (key.type) {
      case UnimplementedKey::kInt64:
        accepted = Apply<int64_t>(options, key.name, keys, diagnostics,
                                  [](int64_t) {});
        break;
