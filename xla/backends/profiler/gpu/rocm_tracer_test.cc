...
struct RoctxLib {
  ...
  static RoctxLib Load() {
    ...
    for (const char* path : candidates) {
      // RTLD_GLOBAL: the intercept table must see these symbols.
      // RTLD_NODELETE: rocprofiler's intercept table holds pointers into this
      // library's text. The process-wide tracer context outlives every test,
      // so unmapping it at teardown would leave those pointers dangling.
      lib.handle = dlopen(path, RTLD_LAZY | RTLD_GLOBAL | RTLD_NODELETE);
      if (lib.handle) break;
    }
  ...
  // Rule of three/five. The user-declared destructor below suppresses the
  // implicit move constructor, so `return lib;` in Load() would otherwise fall
  // back to a COPY whenever NRVO does not apply -- leaving two objects owning
  // one handle and dlclose'ing it twice. Copy is deleted and move nulls the
  // source so ownership is unique by construction rather than by NRVO luck.
  RoctxLib() = default;
  RoctxLib(const RoctxLib&) = delete;
  RoctxLib& operator=(const RoctxLib&) = delete;
  RoctxLib(RoctxLib&& other) noexcept
      : handle(other.handle),
        roctxRangePushA(other.roctxRangePushA),
        roctxRangePop(other.roctxRangePop),
        roctxMarkA(other.roctxMarkA) {
    other.handle = nullptr;
  }
  RoctxLib& operator=(RoctxLib&& other) noexcept {
    if (this != &other) {
  ...
  // Opened with RTLD_NODELETE, so this decrements the refcount without
  // unmapping; rocprofiler's intercept pointers stay valid.
  ~RoctxLib() {
    if (handle) dlclose(handle);
  }
};
...
TEST(RocmTracerTest, GetCurrentRoctxLabelReturnedCopyRemainsValidAfterPop) {
  ...
  std::string captured = tracer.GetCurrentRoctxLabel();
