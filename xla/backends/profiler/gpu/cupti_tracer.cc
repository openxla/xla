...
struct GraphResourceCreationInfo {
  uint32_t graph_id = UINT32_MAX;
  uint32_t orig_graph_id = UINT32_MAX;
...
void SetCudaGraphEventUponApiExit(
  ...
  if (cbid == CUPTI_DRIVER_TRACE_CBID_cuGraphClone ||
    ...
    // The time increment is set to 1nanosecond. This is to make sure that the
    // CudaGraphNodeMap events are not overlapping.
    ...
    for (const auto &[node_id, orig_node_id] : graph_id_info.node_id_map) {
      ...
      // Add a 2nanoseconds gap between the CudaGraphNodeMap events.
      // This is to make sure that the CudaGraphNodeMap events are not
      // overlapping and are not merged by the CUPTI collector.
      current_start_time += 2 * kTimeIncrementNs;
    }
