...
//
// collector_options.num_gpus must hold the number of GPUs present on entry
// (device_tracer_rocm.cc passes RocmTracer::NumGpus()). It is read as well as
// written: gpu_num_chips_to_profile_per_task uses it to resolve 0 and any
// over-large value to "every device", which is the CUDA disposition and the
// only one the collector can act on -- num_gpus==0 there means an empty
// profile, not a full one.
//
...
// diagnostics.errors, and the remaining keys are still applied. It has no
// opinion about what a non-empty error list means either; that is the caller's
// decision. A caller that wants CUPTI's hard-failure behaviour can inspect
// diagnostics.errors and honour ProfileOptions.raise_error_on_start_failure
// itself. `diagnostics` is appended to, never cleared, so a caller reusing one
// across sessions must reset it.
//
