...
// SFINAE helpers to detect and set padding on Riegeli Options.
...
template <typename T, typename = void>
struct HasSetPadToBlockBoundary : std::false_type {};

template <typename T>
struct HasSetPadToBlockBoundary<
...
void SetPadding(Options& options) {
  ...
  } else if constexpr (HasSetPadToBlockBoundary<Options>::value) {
