...
class LibzipInputStream : public google::protobuf::io::ZeroCopyInputStream {
 public:
  ...
  bool Next(const void** data, int* size) override {
    ...
    int64_t bytes_read = zip_fread(file_, buffer_, kBufferSize);
    if (bytes_read < 0) {
      buffer_size_ = 0;
      buffer_offset_ = 0;
      return false;  // error
...
class LibzipArchive : public ZipArchive {
 public:
  ...
  absl::StatusOr<std::vector<std::string>> GetEntries() override {
    ...
    int64_t num_entries = zip_get_num_entries(archive_.get(), 0);
    if (num_entries < 0) {
  ...
  absl::StatusOr<std::string> GetContents(absl::string_view entry) override {
    ...
    int64_t bytes_read = zip_fread(file, &content[0], st.size);
...
int64_t tsl_source_callback(void* userdata, void* data, uint64_t len,
