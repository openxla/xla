...
# Generates the following targets (all of the rule types are the standard ones):
# proto_library:    "{name}"
# cc_proto_library: "{name}_cc"
# alias:            "{name}_cc_impl", actual = "{name}_cc"
# py_proto_library: "{name}_py_proto"
# py_grcp_library:  "{name}_py_grpc_proto"
# py_library:       "{name}_py", deps = ["{name}_py_proto", "{name}_py_grpc_proto"]
# cc_grpc_library:  "{name}_cc_grpc_proto", if name.endsWith("proto") name = name[:-6]
...
def tf_additional_core_deps():
    return select({
        ...
        "//conditions:default": [
            clean_dep("@xla//xla/tsl/platform/cloud:gcs_file_system"),
        ],
    })
    ...
    strict_cc_test(
        ...
        deps = deps + if_tsl_link_protobuf(
            ...
            [
                ...
                # granularly
                clean_dep("@xla//xla/tsl/protobuf:error_codes_proto_impl_cc_impl"),
...
def tf_windows_aware_platform_deps(name):
    return select({
        clean_dep("@xla//xla/tsl:windows"): [
            clean_dep("@xla//xla/tsl/platform/windows:" + name),
        ],
        "//conditions:default": [
