...
ABSL_FLAG(std::vector<std::string>, strip, {},
          "Leading prefixes to strip off file names.");
ABSL_FLAG(bool, data_in_cc, false,
          "Whether the embedded data should be stored in the .cc file or the "
          ".o file.");
  ...
  for (const auto& initializer : initializers) {
    ...
    if (settings.flatten) {
      ...
      for (const auto& prefix : prefixes) {
        filename = absl::StripPrefix(filename, prefix);
      }
    }
