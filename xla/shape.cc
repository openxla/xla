...
void Shape::RemoveSharingForLiteral() {
  // Helper for recursive walk over tuple tree.
  std::function<void(Shape * s)> unshare = [&](Shape* s) {
    s->mutable_state();
    TupleState* state = s->if_tuple_state();
    if (state != nullptr) {
      for (Shape& sub : state->tuple_shapes) {
        unshare(&sub);
      }
    }
  };
...
void Shape::set_element_type(const PrimitiveType value) {
  ...
  const PrimitiveType old_type = element_type();
  if (old_type == TOKEN) {
    if (!if_token_state()) {
      CheckStateIsEmpty();
  ...
  if (old_type == BUFFER) {
    if (!if_buffer_state()) {
      CheckStateIsEmpty();
      *mutable_state() = BufferState();
    }
    return;
  }
  if (primitive_util::IsArrayType(old_type)) {
    if (!if_array_state()) {
      CheckStateIsEmpty();
  ...
  if (old_type != PRIMITIVE_TYPE_INVALID) {
    LOG(ERROR) << "Unsupported element type: " << old_type;
