 ...
 *   - Operand type must be an integer type or a quant::QuantizedType.
 *   - Result type must be a float type.
 *
...
createStablehloLegalizeQuantCompositePass() {
  return std::unique_ptr<OperationPass<ModuleOp>>(
      new StablehloLegalizeQuantCompositePass());
}
