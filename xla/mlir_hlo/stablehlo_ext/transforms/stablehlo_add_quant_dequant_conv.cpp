...
#define GEN_PASS_DEF_STABLEHLOADDQUANTDEQUANTCONVPASS
...
Type getQuantizedType(Location loc, PatternRewriter& rewriter,
  ...
  auto quantizedElementType = quant::UniformQuantizedType::getChecked(
      loc, flags, storageType, expressedType, scale, zeroPoint, storageTypeMin,
      storageTypeMax);
...
struct AddQuantDeQuantAfterConvolutionOp final
  ...
  LogicalResult match(stablehlo::ConvolutionOp op) const override {
    ...
    // 1. Its operands are defined by stabhelo.uniform_dequantize op,
...
class StablehloAddQuantDeQuantConvPass
    : public impl::StablehloAddQuantDeQuantConvPassBase<
          StablehloAddQuantDeQuantConvPass> {
 public:
