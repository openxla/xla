...
MlirAttribute mlirMhloSparsityDescriptorGet(MlirContext ctx, int64_t index,
                                            int64_t dimension, int64_t n,
                                            int64_t m) {
  return wrap(mlir::mhlo::SparsityDescriptorAttr::get(unwrap(ctx), index,
                                                      dimension, n, m));
}
...
int64_t mlirMhloSparsityDescriptorGetIndex(MlirAttribute attr) {
  return unwrap(attr).cast<mlir::mhlo::SparsityDescriptorAttr>().getIndex();
}
