...
func.func @batchNormInference_2D_inner_features(
  ...
  // CHECK-DAG: %[[STDDEV_BCAST:.+]] = mhlo.broadcast_in_dim %[[STDDEV]], dims = [1] : (tensor<256xf32>) -> tensor<4x256xf32>
  // CHECK-DAG: %[[SCALE_BCAST:.+]] = mhlo.broadcast_in_dim %[[SCALE]], dims = [1] : (tensor<256xf32>) -> tensor<4x256xf32>
  // CHECK-DAG: %[[OFFSET_BCAST:.+]] = mhlo.broadcast_in_dim %[[OFFSET]], dims = [1] : (tensor<256xf32>) -> tensor<4x256xf32>
  // CHECK-DAG: %[[MEAN_BCAST:.+]] = mhlo.broadcast_in_dim %[[MEAN]], dims = [1] : (tensor<256xf32>) -> tensor<4x256xf32>
...
func.func @batchNormTraining_2D_inner_features(
  ...
  // CHECK-DAG: %[[VARIANCE_EPS:.+]] = mhlo.add %[[VARIANCE]], %[[EPS_BCAST]] : tensor<256xf32>
  // CHECK-DAG: %[[STDDEV:.+]] = mhlo.sqrt %[[VARIANCE_EPS]] : tensor<256xf32>
  // CHECK-DAG: %[[EX_BCAST:.+]] = mhlo.broadcast_in_dim %[[EX]], dims = [1] : (tensor<256xf32>) -> tensor<4x256xf32>
...
// CHECK-SAME: %[[X:[^:]+]]
// CHECK-SAME: %[[SCALE:[^:]+]]
// CHECK-DAG: %[[SCALE_BCAST:.+]] = mhlo.broadcast_in_dim %[[SCALE]], dims = [2] : (tensor<256xf32>) -> tensor<3x4x256x6xf32>
...
func.func @batchNormTraining_4D_middle_features(
  ...
  // CHECK-DAG: %[[VARIANCE_EPS:.+]] = mhlo.add %[[VARIANCE]], %[[EPS_BCAST]] : tensor<256xf32>
  // CHECK-DAG: %[[STDDEV:.+]] = mhlo.sqrt %[[VARIANCE_EPS]] : tensor<256xf32>
  // CHECK-DAG: %[[EX_BCAST:.+]] = mhlo.broadcast_in_dim %[[EX]], dims = [2] : (tensor<256xf32>) -> tensor<3x4x256x6xf32>
  // CHECK-DAG: %[[X_CENTER:.+]] = mhlo.subtract %[[X]], %[[EX_BCAST]] : tensor<3x4x256x6xf32>
  // CHECK-DAG: %[[STDDEV_BCAST:.+]] = mhlo.broadcast_in_dim %[[STDDEV]], dims = [2] : (tensor<256xf32>) -> tensor<3x4x256x6xf32>
  ...
  // CHECK-DAG: %[[OFFSET_BCAST:.+]] = mhlo.broadcast_in_dim %[[OFFSET]], dims = [2] : (tensor<256xf32>) -> tensor<3x4x256x6xf32>
...
func.func @batchNormInference_dynamic_shape(
  ...
  // CHECK-DAG: %[[EPS_BCAST:.+]] =  mhlo.dynamic_broadcast_in_dim %[[EPS]], %[[VAR_SHAPE]], dims = []  : (tensor<f32>, tensor<1xindex>) -> tensor<?xf32>
  ...
  // CHECK-DAG: %[[STDDEV_BCAST:.+]] = mhlo.dynamic_broadcast_in_dim %[[STDDEV]], %[[X_SHAPE]], dims = [1] : (tensor<?xf32>, tensor<4xindex>) -> tensor<?x?x?x?xf32>
  // CHECK-DAG: %[[SCALE_BCAST:.+]] = mhlo.dynamic_broadcast_in_dim %[[SCALE]], %[[X_SHAPE]], dims = [1] : (tensor<?xf32>, tensor<4xindex>) -> tensor<?x?x?x?xf32>
  // CHECK-DAG: %[[OFFSET_BCAST:.+]] = mhlo.dynamic_broadcast_in_dim %[[OFFSET]], %[[X_SHAPE]], dims = [1] : (tensor<?xf32>, tensor<4xindex>) -> tensor<?x?x?x?xf32>
  // CHECK-DAG: %[[MEAN_BCAST:.+]] = mhlo.dynamic_broadcast_in_dim %[[MEAN]], %[[X_SHAPE]], dims = [1] : (tensor<?xf32>, tensor<4xindex>) -> tensor<?x?x?x?xf32>
...
func.func @batchNormTraining_dynamic_shape(
  ...
  // CHECK-DAG: %[[EPS_BCAST:.+]] = mhlo.dynamic_broadcast_in_dim %[[EPS]], %[[SCALE_SHAPE]], dims = []  : (tensor<f32>, tensor<1xindex>) -> tensor<?xf32>
  // CHECK-DAG: %[[X_SHAPE:.+]] = shape.shape_of %[[X]] : tensor<?x?x?x?xf32> -> tensor<4xindex>
  ...
  // CHECK-DAG: %[[REDUCE_SIZE_BCAST:.+]] = mhlo.dynamic_broadcast_in_dim %[[REDUCE_SIZE_RESHAPE]], %[[SCALE_SHAPE]], dims = [] : (tensor<f32>, tensor<1xindex>) -> tensor<?xf32>
  ...
  // CHECK-DAG: %[[EX_BCAST:.+]] = mhlo.dynamic_broadcast_in_dim %[[EX]], %[[X_SHAPE]], dims = [2] : (tensor<?xf32>, tensor<4xindex>) -> tensor<?x?x?x?xf32>
  // CHECK-DAG: %[[X_SUB_EX:.+]] = mhlo.subtract %[[X]], %[[EX_BCAST]] : tensor<?x?x?x?xf32>
  // CHECK-DAG: %[[STDX_BCAST:.+]] = mhlo.dynamic_broadcast_in_dim %[[STDX]], %[[X_SHAPE]], dims = [2] : (tensor<?xf32>, tensor<4xindex>) -> tensor<?x?x?x?xf32>
  // CHECK-DAG: %[[X_CENTOR:.+]] = mhlo.divide %[[X_SUB_EX]], %[[STDX_BCAST]] : tensor<?x?x?x?xf32>
  // CHECK-DAG: %[[SCALE_BCAST:.+]] = mhlo.dynamic_broadcast_in_dim %[[SCALE]], %[[X_SHAPE]], dims = [2] : (tensor<?xf32>, tensor<4xindex>) -> tensor<?x?x?x?xf32>
  // CHECK-DAG: %[[X_SCALED:.+]] = mhlo.multiply %[[X_CENTOR]], %[[SCALE_BCAST]] : tensor<?x?x?x?xf32>
  // CHECK-DAG: %[[OFFSET_BCAST:.+]] = mhlo.dynamic_broadcast_in_dim %[[OFFSET]], %[[X_SHAPE]], dims = [2] : (tensor<?xf32>, tensor<4xindex>) -> tensor<?x?x?x?xf32>
