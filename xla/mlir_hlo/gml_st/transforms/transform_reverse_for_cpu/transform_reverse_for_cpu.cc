...
FailureOr<TilingResult> tileReverse(PatternRewriter &rewriter,
                                    thlo::ReverseOp reverseOp,
                                    ArrayRef<int64_t> tileSizes,
                                    bool distribute) {
  TilingOptions opts;
  opts.setTileSizeComputationFn(tileSizes);
  opts.distribute = distribute;
  return tile(opts, rewriter, cast<TilingInterface>(reverseOp.getOperation()));
}
        ...
        for (auto *remParLoop : peelingResult) {
          remParLoop->walk([&](Operation *childOp) {
            if (isa<thlo::ReverseOp>(childOp)) {
              ...
              auto secondTiling =
                  tileReverse(rewriter, innerReverseOp,
                              getTileSizes(rank, vectorSize, false),
                              /*distribute=*/true);
              if (failed(secondTiling)) return;
              // Update the results if tiling occurred.
              if (secondTiling->loop != nullptr) {
                rewriter.replaceOp(innerReverseOp,
                                   secondTiling->loop->getResults());
                innerReverseOp =
                    cast<thlo::ReverseOp>(secondTiling->tiledOps.front());
              }
            }
          });
        }
