...
bool hasPrivateFeaturesNotInStablehlo(HloOpTy hloOp) {
  ...
  if (isa<mhlo::AddDependencyOp, mhlo::AsyncDoneOp, mhlo::AsyncStartOp,
          ...
          mhlo::FusionOp, mhlo::PartitionIdOp, mhlo::StochasticConvertOp,
  ...
  if constexpr (std::is_same<HloOpTy, mhlo::CustomCallOp>::value) {
    if (hloOp.getCustomCallScheduleAttr()) return true;
  }
  return false;
}
...
class HloToStablehloOpConverter : public OpConversionPattern<HloOpTy> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    // supported in StableHLO.
    //   1) Take converted result types.
    //   2) Take converted operands.
    //   3) Pack attributes into DictionaryAttr and store in backend_config.
    if (hasPublicFeaturesNotInStablehlo(hloOp)) {
      if (hloOp->getNumRegions() != 0) {
        ...
        // In principle, it should be straightforward to implement by converting
        // regions into functions and calling them out in "called_computations".
