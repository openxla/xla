...
FailureOr<MemRefLayoutAttrInterface> MemRefReshapeOp::inferResultLayout(
  ...
  if (const auto input_tiled_layout = dyn_cast<TiledLayoutAttr>(input_layout)) {
    ...
    const TiledLayoutAttr reshaped_tile_layout = [&]() -> TiledLayoutAttr {
      ...
      // representation, but performs a similar process by collapsing tile
      // dimensions and non-tile dimensions separately, and then expanding them.
      ...
      while (input_dim < input_rank && result_dim < result_rank) {
        if (input_dim_is_dynamic_or_padded(input_dim)) {
          if (input_shape[input_dim] != result_shape[result_dim]) {
            return nullptr;
          }
        ...
        while (non_tile_elements != 1) {
          ...
          if (non_tile_elements % result_shape[result_dim] == 0) {
            ...
            // non_tile_size holds the non-tile size for the current dimension.
            break;
          } else {
            // Cannot expand into the desired shape
            return nullptr;
        ...
        if (non_tile_elements != 1) {
          // Cannot expand into the desired shape
          return nullptr;
        }
        CHECK_EQ(tile_elements, 1);
      }
      CHECK_EQ(input_dim, input_rank);
      CHECK_EQ(result_dim, result_rank);
