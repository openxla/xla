...
template <typename Op>
LogicalResult verifyCompressStoreExpandLoad(Op op, VectorType ty,
                                            int32_t compress_expand_dim,
                                            StringRef vector_name,
                                            StringRef dim_name,
                                            StringRef dim_adjective) {
  MemRefType ref_ty = op.getBase().getType();
  ...
  for (int64_t i = 0; i < rank; ++i) {
    ...
    if (ty.getDimSize(i) > ref_ty.getDimSize(i)) {
      return op.emitOpError(dim_adjective)
...
LogicalResult VectorCompressStoreOp::verify() {
  return verifyCompressStoreExpandLoad(
      *this, getValueToStore().getType(), getCompressDim(),
      /*vector_name=*/"valueToStore", /*dim_name=*/"compress_dim",
      /*dim_adjective=*/"Non-compressed");
}

LogicalResult VectorExpandLoadOp::verify() {
  return verifyCompressStoreExpandLoad(
      *this, getResult().getType(), getExpandDim(),
      /*vector_name=*/"result", /*dim_name=*/"expand_dim",
      /*dim_adjective=*/"Non-expanded");
}
