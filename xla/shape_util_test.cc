...
TEST(ShapeUtilTest, ForEachIndex_Rank0) {
  Shape shape = ShapeUtil::MakeShape(F32, {});
  int64_t output = -1;
  auto set_func = [&](absl::Span<const int64_t> indexes) -> StatusOr<bool> {
    output = indexes.size();
    return true;
  };

  ShapeUtil::ForEachIndex(shape, /*base=*/{}, /*count=*/{},
                          /*incr=*/{}, set_func);

  EXPECT_EQ(output, 0);
}

TEST(ShapeUtilTest, ForEachIndex_Empty) {
  ...
  ShapeUtil::ForEachIndex(shape, /*base=*/{2, 3}, /*count=*/{5, 0},
                          /*incr=*/{3, 3}, set_func);
...
TEST(ShapeUtilTest, ForEachIndexParallel_Empty) {
  ...
  ShapeUtil::ForEachIndexParallel(shape, /*base=*/{2, 3}, /*count=*/{5, 0},
                                  /*incr=*/{3, 3}, set_func);
  EXPECT_FALSE(did_run);
}
