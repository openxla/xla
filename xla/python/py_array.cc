...
tsl::RCReference<ifrt::Array> CreateIfRtArrayFromSingleDeviceShardedPyArrays(
  ...
  tsl::RCReference<ifrt::DeviceList> device_list =
      device->client()->MakeDeviceList(devices);
...
// TODO(yashkatariya): Remove this check once we support non-default layouts
// for arrays with no local shards on some processes.
void check_sharding_with_device_subset_has_default_layout(
    ifrt::Array* ifrt_array, int num_devices) {
  auto layout = ifrt_array->layout();
  if (!absl::IsUnimplemented(layout.status())) {
    if (ifrt_array->client()->device_count() != num_devices) {
      auto default_layout = ifrt_array->client()->GetDefaultLayout(
          ifrt_array->dtype(), ifrt_array->shape().dims(),
          ifrt_array->sharding().devices()->devices().front(),
          ifrt_array->sharding().memory_kind());
      if (!default_layout.ok()) {
        throw nb::value_error(absl::StrCat("Failed to get default layout: ",
                                           default_layout.status().ToString())
                                  .c_str());
      }
      if (!layout.ok()) {
        throw nb::value_error(
            absl::StrCat("Failed to get layout: ", layout.status().ToString())
                .c_str());
      }
      if (*default_layout != *layout) {
        throw nb::value_error(
            "Arrays with non-default layout must be sharded over all devices.");
      }
    }
  }
}
...
void PyArray::PyInit(PyArray self, nb::object aval, nb::object sharding,
  ...
  if (skip_checks) {
    ...
    if (!py_arrays.empty()) {
      auto ifrt_array0 = py_arrays[0].ifrt_array();
      int num_devices = nb::cast<int>(sharding.attr("num_devices"));
      check_sharding_with_device_subset_has_default_layout(ifrt_array0,
                                                           num_devices);
    }
