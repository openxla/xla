...
struct PyClientConnectionOptions {
  ...
  std::optional<
      std::unordered_map<std::string, std::variant<std::string, bool, int64_t>>>
      initialization_data;
};
...
absl::StatusOr<nb_class_ptr<PyClient>> GetClient(
  ...
  if (py_options.initialization_data.has_value()) {
    ...
    for (const auto& [key, py_value] : *py_options.initialization_data) {
      if (std::holds_alternative<std::string>(py_value)) {
        attribute_map.insert(
            {key, AttributeMap::StringValue(std::get<std::string>(py_value))});
