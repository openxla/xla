...
BackendInterface::Response IfrtBackend::HandleInit(
  ...
  init_resp->set_platform_name(as_protobuf_string(client_->platform_name()));
  init_resp->set_platform_version(
      as_protobuf_string(client_->platform_version()));
  init_resp->set_platform_id(client_->platform_id());
  init_resp->set_runtime_type(as_protobuf_string(client_->runtime_type()));
  ...
  for (auto* device : client_->devices()) {
    ...
    d->set_device_kind(as_protobuf_string(device->device_kind()));
    ...
    d->set_debug_string(as_protobuf_string(device->DebugString()));
    d->set_to_string(as_protobuf_string(device->ToString()));
  ...
  for (const auto& [id, memory] : memories) {
    ...
    m->set_id(id);
    m->set_memory_space_kind(as_protobuf_string(memory->memory_space_kind()));
    for (const auto* device : memory->devices()) {
      m->add_device_ids(device->id());
    }
    m->set_debug_string(as_protobuf_string(memory->DebugString()));
    m->set_to_string(as_protobuf_string(memory->ToString()));
  }
...
Future<BackendInterface::Response> IfrtBackend::HandleCompileRequest(
  ...
  auto f = [this, request = std::shared_ptr<IfrtRequest>(
    ...
    compile_resp->set_name(as_protobuf_string(executable->name()));
