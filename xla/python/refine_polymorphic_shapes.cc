...
#include "third_party/tensorflow/compiler/mlir/tensorflow/utils/dump_mlir_util.h"
...
constexpr absl::string_view shapeAssertionName = "stablehlo.shape_assertion";
...
// represented via the `stablehlo.custom_call @stablehlo.shape_assertion`
...
//
struct ShapeAssertionsPass
    : public mlir::PassWrapper<ShapeAssertionsPass,
                               mlir::OperationPass<mlir::func::FuncOp>> {
 public:
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(ShapeAssertionsPass)

  explicit ShapeAssertionsPass(bool enable_shape_assertions = true)
      : PassWrapper() {
    this->enable_shape_assertions = enable_shape_assertions;
  }

  ShapeAssertionsPass(const ShapeAssertionsPass &pass) {
 ...
 private:
        ...
        if (assertWhat[0] != 0) {
          ...
          for (int i = 1; i < inputs.size(); ++i) {
            ...
            if (failed(mlir::hlo::matchInts(inputs[i], input))) {
              op.emitError("expected static error_message_input");
              signalPassFailure();
              return;
            }
  ...
  mlir::LogicalResult verifyShapeAssertion(mlir::stablehlo::CustomCallOp op) {
    ...
    if (!op.getCallTargetName().equals(shapeAssertionName))
      return op.emitError() << "expects @stablehlo.shape_assertion";
    ...
    if (!assertWhatType || !assertWhatType.hasRank() ||
      ...
      return op.emitError() << "expects assert_what (operand #1) "
                            << "to be a 0-dimensional tensor of type i1";

    // input[1:] (error_message_inptus) : tensor<i32> or tensor<i64>
    for (int i = 0; i < nrErrorMessageInputs; ++i) {
      ...
      if (!errorMessageInputType || !errorMessageInputType.hasRank() ||
        ...
        return op.emitError()
               ...
               << "to be a 0-dimensional tensor of type i32 or i64";
    }
    ...
    do {
      mlir::SmallVector<llvm::StringRef, 3> formatSpec;
  ...
  mlir::StringRef getArgument() const override { return "shape-assertions"; }

  mlir::StringRef getDescription() const override {
    return "Handle stablehlo.custom_call @stablehlo.shape_assertion ops.";
  }
...
absl::Status RefinePolymorphicShapes(mlir::ModuleOp module,
  ...
  pm.addNestedPass<mlir::func::FuncOp>(
      std::make_unique<ShapeAssertionsPass>(enable_shape_assertions));
...
absl::Status ValidateStaticShapes(mlir::ModuleOp module) {
  ...
  module->walk([&](mlir::Operation *op) {
    ...
    if (customCall &&
      ...
      op->emitOpError() << "has shape assertions";
    }
  });
