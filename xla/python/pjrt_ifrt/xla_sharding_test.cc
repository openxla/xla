...
TEST(HloShardingTest, IndexDomainWithReplication) {
...
TEST(HloShardingTest, IndexDomainWithTile) {
  auto device_list = CreateDummyDevices(2);
...
TEST(HloShardingTest, IndexDomainWithUnevenTile) {
  auto device_list = CreateDummyDevices(2);
  // 2-way sharded along axis 0, 1-way sharded along axis 1.
  auto xla_hlo_sharding = xla::HloSharding::Tile(xla::TileAssignment({2, 1}));
  std::shared_ptr<const Sharding> sharding =
...
TEST(HloShardingTest, IndexDomainWithPartialTile) {
...
TEST(HloShardingTest, IndexDomainWithSubgroupReplicated) {
  auto device_list = CreateDummyDevices(6);
  // 2-way sharded along axis 0, 1-way sharded along axis 1, each shard
  // replicated by 3 times.
...
TEST(HloShardingTest, DisassembleFailsWithUnsupportedSubgroupType) {
  auto device_list = CreateDummyDevices(2);
  // 2-way sharded along axis 0, 1-way sharded along axis 1, each shard
  // replicated by 3 times, device#2 in each replication is maximal.
  auto xla_hlo_sharding = xla::HloSharding::Subgroup(
      xla::TileAssignment({2, 1, 3, 2}),
      {xla::OpSharding::REPLICATED, xla::OpSharding::MAXIMAL});
  std::shared_ptr<const Sharding> sharding =
      HloSharding::Create(device_list, xla_hlo_sharding);
  ...
  EXPECT_THAT(
      ...
      StatusIs(tsl::error::INVALID_ARGUMENT,
               HasSubstr("sharding has unsupported non-replicated subgroup")));
}
