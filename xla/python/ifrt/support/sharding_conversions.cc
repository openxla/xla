...
std::vector<int> UpdatePermutation(const std::vector<int>& permutation,
  ...
  std::sort(subdim_indices.begin(), subdim_indices.end(), [&](int a, int b) {
    ...
    if (subdim_info[a].tile_dim_index != subdim_info[b].tile_dim_index) {
      return subdim_info[a].tile_dim_index < subdim_info[b].tile_dim_index;
    }
    return subdim_info[a].tile_sub_dim_index <
           subdim_info[b].tile_sub_dim_index;
  });
...
absl::StatusOr<ShardingInfo> GetShardingInfo(
  ...
  int replicated_size = replicated_and_unreduced_size / unreduced_size;
  ...
  for (int i = 0; i < perm_new.size(); ++i) {
    int ind = perm_new[i];
    if (sub_dim_info[ind].tile_dim_index < dims.size() - 1) {
      new_permutation.push_back(ind);
    } else {
      // The last element of dims represents replicated and unreduced axes.
      if (unreduced_axes_set.contains(sub_dim_info[ind].reshape_dim_index)) {
        new_unreduced_permutation.push_back(ind);
      } else {
        new_replicated_permutation.push_back(ind);
      }
    }
  }
...
absl::StatusOr<HloSharding> ToHloSharding(const ShardingParam& sharding_param) {
  ...
  if (sharding_info.last_tile_dims.empty()) {
    return HloSharding::IotaTile(sharding_info.dims, sharding_info.reshape_dims,
                                 sharding_info.permutation);
  }
  if (sharding_info.last_tile_dims.size() == 1 &&
      sharding_info.last_tile_dims[0] == OpSharding::REPLICATED) {
    return HloSharding::PartialTile(TileAssignment(sharding_info.dims,
