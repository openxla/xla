...
TEST_P(ShardingConversionsTest, OpShardingWithUnreduced) {
  ...
  op_sharding.add_tile_assignment_dimensions(1);
  op_sharding.add_tile_assignment_dimensions(1);
  op_sharding.add_tile_assignment_dimensions(2);
  op_sharding.add_tile_assignment_dimensions(5);
  op_sharding.add_tile_assignment_dimensions(3);
  op_sharding.add_iota_reshape_dims(5);
  op_sharding.add_iota_reshape_dims(3);
  op_sharding.add_iota_reshape_dims(2);
  op_sharding.add_iota_transpose_perm(2);
  op_sharding.add_iota_transpose_perm(0);
  op_sharding.add_iota_transpose_perm(1);
  op_sharding.add_last_tile_dims(OpSharding::UNREDUCED);
  op_sharding.add_last_tile_dims(OpSharding::REPLICATED);
  TF_ASSERT_OK_AND_ASSIGN(const HloSharding hlo_sharding,
                          HloSharding::FromProto(op_sharding));
  TF_ASSERT_OK_AND_ASSIGN(ShardingParam actual,
