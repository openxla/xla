...
std::unique_ptr<Sharding> ConcreteEvenSharding::Create(DeviceList devices,
  ...
  return std::unique_ptr<Sharding>(
      new ConcreteEvenSharding(std::move(devices), std::move(shard_shape)));
}
...
ConcreteEvenSharding::Disassemble(const Shape& shape) const {
  ...
  for (Device* device : devices_) {
    result.push_back({shard_shape_, SingleDeviceSharding::Create(device)});
  }
  return result;
}
...
std::string ConcreteEvenSharding::DebugString() const {
  DCHECK(this);
  return absl::StrFormat(
      "ConcreteEvenSharding(%s, devices: %s)", shard_shape_.DebugString(),
      absl::StrJoin(devices_, ",", [](std::string* out, const Device* device) {
        absl::StrAppend(out, device->ToString());
      }));
}
