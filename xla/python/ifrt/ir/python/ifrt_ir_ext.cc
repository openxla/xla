...
absl::Status RegisterIfrtIrDialect(MlirContext py_context) {
  mlir::MLIRContext* context = unwrap(py_context);
  context->loadDialect<IfrtDialect>();
  return absl::OkStatus();
}

PYBIND11_MODULE(_ifrt_ir_ext, m) {
  m.doc() = "IFRT IR main Python extension";

  //
  // Dialects.
  //
  m.def("register_dialect", xla::ThrowIfErrorWrapper(RegisterIfrtIrDialect));
  ...
  mlir::python::adaptors::mlir_attribute_subclass(
      ...
      .def_classmethod(
          ...
          "Creates a `DevicesAttr` attribute.");
