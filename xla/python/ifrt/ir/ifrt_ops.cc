...
mlir::LogicalResult VerifyGlobalLocalShapesEquivalent(
    ...
    llvm::StringRef calle_mnemonic, mlir::Type calle_type) {
  ...
  mlir::FailureOr<mlir::RankedTensorType> calle_shape =
      GetGlobalShapeFromLocal(calle_type, array.getSharding());
  if (mlir::failed(calle_shape)) {
    return op->emitOpError() << "fails to get global shape from "
                             << calle_mnemonic << ": " << calle_type;
  }
  if (*call_shape != *calle_shape) {
    return op->emitOpError()
           << "requires the same global shape. " << call_mnemonic << " "
           << *call_shape << " vs " << calle_mnemonic << " " << *calle_shape;
  }
  return mlir::success();
}
