...
class UnregisterFn {
 public:
  static UnregisterFn FromAutoFunctionRunner(
      util::functional::AutoFunctionRunner&& runner) {
    return UnregisterFn{std::move(runner)};
  }
  ~UnregisterFn() {
 ...
 private:
  explicit UnregisterFn(util::functional::AutoFunctionRunner&& runner)
      : runner_(std::move(runner)) {}
  util::functional::AutoFunctionRunner runner_;
};
...
NB_MODULE(_profiler, m) {
  ...
  m.def(
      ...
      [](int pid, int port) -> UnregisterFn {
        absl::StatusOr<util::functional::AutoFunctionRunner> unregister_fn =
            xla::profiler::subprocess::RegisterSubprocess(pid, port,
                                                          std::nullopt);
        xla::ThrowIfError(unregister_fn.status());
        return UnregisterFn::FromAutoFunctionRunner(std::move(*unregister_fn));
      },
