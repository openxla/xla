...
TEST(EigenUnaryTest, FastSinV2f64IsCorrect) {
  Vec2d x = {1.0, 2.0};
  Vec2d y = sin_v2f64(x);
  for (int i = 0; i < 2; ++i) {
    EXPECT_THAT(y[i], NearUlps(std::sin(x[i]), kSincosUlps));
...
TEST(EigenUnaryTest, FastCosV2f64IsCorrect) {
  Vec2d x = {1.0, 2.0};
  Vec2d y = cos_v2f64(x);
  for (int i = 0; i < 2; ++i) {
    EXPECT_THAT(y[i], NearUlps(std::cos(x[i]), kSincosUlps));
...
TEST(EigenUnaryTest, SinIsVectorized32) {
  ...
  EXPECT_THAT(ir, ContainsRegex("xla.sin.v2f64"));
...
TEST(EigenUnaryTest, CosIsVectorized32) {
  ...
  EXPECT_THAT(ir, ContainsRegex("xla.cos.v2f64"));
