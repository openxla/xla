...
function is_pytorch_xla_tpu_job() {
  [[ "$KOKORO_JOB_NAME" =~ pytorch/xla/linux/.*tpu.* ]]
}
...
docker run --name torchxla -w /root/ -itd --rm \
    -v "$KOKORO_ARTIFACTS_DIR/github/xla:/root/openxla" \
    "$DOCKER_IMAGE" \
    bash

    # -v "$KOKORO_ARTIFACTS_DIR/pkg:/torch/pkg" \
...
docker exec torchxla bazel \
        ...
        $RBE_FLAGS \
        $ADDITIONAL_FLAGS \
        -- //xla/... //build_tools/... @tsl//tsl/... $TARGET_FILTERS
