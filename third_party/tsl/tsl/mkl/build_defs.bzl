...
def if_mkl(if_true, if_false = []):
    ...
    """
    return select({
        clean_dep("//third_party/tensorflow/tsl/mkl:build_with_mkl_aarch64"): if_true,
        clean_dep("//third_party/tensorflow/tsl:linux_x86_64"): if_true,
        clean_dep("//third_party/tensorflow/tsl:windows"): if_true,
...
def if_mkl_ml(if_true, if_false = []):
    ...
    return select({
        ...
        clean_dep("//third_party/tensorflow/tsl/mkl:build_with_mkl"): if_true,
        "//conditions:default": if_false,
    })
...
def if_mkl_lnx_x64(if_true, if_false = []):
    ...
    """
    return select({
        clean_dep("//third_party/tensorflow/tsl/mkl:build_with_mkl_lnx_x64"): if_true,
        "//conditions:default": if_false,
    })
...
def if_enable_mkl(if_true, if_false = []):
    ...
    """
    return select({
        clean_dep("//third_party/tensorflow/tsl/mkl:enable_mkl"): if_true,
        "//conditions:default": if_false,
    })
...
def mkl_deps():
    ...
    """
    return select({
        clean_dep("//third_party/tensorflow/tsl/mkl:build_with_mkl_aarch64"): ["@mkl_dnn_acl_compatible//:mkl_dnn_acl"],
        clean_dep("//third_party/tensorflow/tsl:linux_x86_64"): ["@mkl_dnn_v1//:mkl_dnn"],
        clean_dep("//third_party/tensorflow/tsl:windows"): ["@mkl_dnn_v1//:mkl_dnn"],
        "//conditions:default": [],
    })
