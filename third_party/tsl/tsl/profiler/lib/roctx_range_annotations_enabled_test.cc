...
// Tests for roctx_range_annotations.cc with the emitter ENABLED.
//
// A separate binary from roctx_range_annotations_test.cc, not a --gtest_filter
// over it: DefaultProfilerDomain() latches its value in a function-local
// static on the first call, so a single process can only ever observe one
// configuration.
// Bazel sets XLA_ROCM_ENABLE_ROCTX=1 for this target (see the `env` attribute
// on roctx_range_annotations_enabled_test in BUILD), which makes the latch
// resolve to a non-null domain here and to null there.
//
...
TEST(RoctxRangeAnnotationsEnabled, DomainIsNonNullWhenEnvVarIsSet) {
...
TEST(RoctxRangeAnnotationsEnabled, DomainStaysLatchedAcrossCalls) {
...
// The enabled-side mirror of DomainIsLatchedAgainstLaterEnvChanges. Calling in
// a loop under a constant environment cannot tell a latched static from a
// getenv per call; changing the environment underneath it can.
//
// This direction is the one that matters in production: a per-call read that
// saw the variable disappear would return null from PopAnnotation's check after
// PushAnnotation had already emitted a roctx range, leaking that range and
// nesting every subsequent user range beneath it.
TEST(RoctxRangeAnnotationsEnabled, DomainIsLatchedAgainstLaterEnvChanges) {
  ProfilerDomainHandle before = DefaultProfilerDomain();
  ASSERT_NE(before, nullptr);

  ASSERT_EQ(setenv("XLA_ROCM_ENABLE_ROCTX", "0", /*overwrite=*/1), 0);
  EXPECT_EQ(DefaultProfilerDomain(), before)
      << "the handle must be computed once and latched; clearing the variable "
         "mid-process must not disable the emitter half way through a range";
...
TEST(RoctxRangeAnnotationsEnabled,
     ScopedAnnotationStillPopulatesAnnotationStack) {
...
TEST(RoctxRangeAnnotationsEnabled, NestedScopedAnnotationsStayBalanced) {
  AnnotationStackGuard guard;
  {
...
TEST(RoctxRangeAnnotationsEnabled, ScopedAnnotationEmitsARoctxRange) {
...
// enabled it must dual push the registered title, not silently do nothing
// while ~ScopedAnnotation goes on to pop.
TEST(RoctxRangeAnnotationsEnabled, DetailRangePushDualPushesRegisteredTitle) {
  AnnotationStackGuard guard;
...
// With the domain enabled the handle must be resolvable: detail::RangePush
// has nothing else to label the range with.
TEST(RoctxRangeAnnotationsEnabled, RegisterStringInternsRealAnnotation) {
  auto domain = DefaultProfilerDomain();
  ASSERT_NE(domain, nullptr);
  constexpr char kLabel[] = "Thunk:#name=fusion.3,hlo_op=fusion.3#";
  StringHandle handle = RegisterString(domain, std::string(kLabel));
  ASSERT_NE(handle, nullptr);
  EXPECT_STREQ(reinterpret_cast<const char*>(handle), kLabel);
}
