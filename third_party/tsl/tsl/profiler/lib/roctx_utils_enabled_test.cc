...
// enabled it must dual push the title text, not silently do nothing while
// ~ScopedAnnotation goes on to pop.
TEST(RoctxUtilsEnabled, DetailRangePushDualPushesTitleText) {
  AnnotationStackGuard guard;
  ...
  detail::RangePush(domain, /*title=*/nullptr, "hlo_op_title",
...
// Even with the domain enabled there is no string registration on roctx: the
// handle is inert and the text travels with the push. A non-null return would
// mean detail::RangePush had a handle it cannot resolve.
TEST(RoctxUtilsEnabled, RegisterStringReturnsNull) {
  auto domain = DefaultProfilerDomain();
  ASSERT_NE(domain, nullptr);
  EXPECT_EQ(RegisterString(domain, "Thunk:#name=fusion.3,hlo_op=fusion.3#"),
            nullptr);
}
