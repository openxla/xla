...
// (Pipeline B -- named ranges captured by MarkerCallback in rocm_tracer.cc;
// routed via kCuptiActivityNvtxPlaneName as a transient plane name, merged
// into /host:CPU in post-processing, and visible to users as
// "Host Threads/<tid>/ROCTX" timeline bands -- distinct from the plane
// name, which is an internal routing token and not the label users see).
//
...
void NameStream(StreamHandle stream, const std::string& name) {
  ...
  //
  // Contract: stream must have been cast from a hipStream_t value.
  // The assert below catches the most common violation (null handle when a
  // real stream was intended) without adding a HIP header dependency.
